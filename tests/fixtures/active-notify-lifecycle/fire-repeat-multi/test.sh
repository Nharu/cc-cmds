#!/usr/bin/env bash
# ARM(repeat) + FIRE × 3 → fire_count progresses 0→1→2→3, last_fire_at integer
# after first fire, flag preserved at every step. sed update regression guard.
set -euo pipefail

bash "$NOTIFY_SH" arm "iter" "iter" "repeat"

for i in 1 2 3; do
  bash "$NOTIFY_SH" fire "iter-$i" "step $i 완료"
  ...
  # mode + schema must remain stable through repeated sed updates.
  grep -q '"mode":"repeat"' "$FLAG_FILE" || { echo "FIRE $i: mode corrupted" >&2; exit 1; }
  grep -q '"schema":2' "$FLAG_FILE" || { echo "FIRE $i: schema corrupted" >&2; exit 1; }
  # quoted-string values must not be corrupted by the numeric-field sed.
