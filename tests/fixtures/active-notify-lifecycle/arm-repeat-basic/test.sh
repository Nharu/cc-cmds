...
grep -q '"arm_count":1' "$FLAG_FILE" || { echo "arm_count not 1 (default)" >&2; exit 1; }
...
grep -qv '"milestone"' "$FLAG_FILE" || { echo "milestone field should not exist in schema:3" >&2; exit 1; }
