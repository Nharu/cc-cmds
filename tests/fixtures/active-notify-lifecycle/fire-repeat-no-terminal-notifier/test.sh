#!/usr/bin/env bash
# ARM(repeat) + FIRE with terminal-notifier absent → stderr hint once + flag
# PRESERVED (asymmetric with single — repeat preserves for next-turn retry).
set -euo pipefail
...
err=$(bash "$NOTIFY_SH" fire "iter" "ping" 2>&1 1>/dev/null)
...
# Flag content must be unchanged (no fire_count increment because notifier never ran).
