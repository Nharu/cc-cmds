...
grep -q '"schema":3' "$FLAG_FILE" || { echo "step1: schema not 3" >&2; exit 1; }
grep -q '"mode":"single"' "$FLAG_FILE" || { echo "step1: mode not single" >&2; exit 1; }
grep -q '"arm_count":1' "$FLAG_FILE" || { echo "step1: arm_count not 1" >&2; exit 1; }
...
grep -qv '"milestone"' "$FLAG_FILE" || { echo "step1: milestone field should not exist in schema:3" >&2; exit 1; }
...
grep -q '"arm_count":1' "$FLAG_FILE" || { echo "step2: arm_count not 1 (default)" >&2; exit 1; }
...
grep -q '"arm_count":1' "$FLAG_FILE" || { echo "step3: arm_count not 1" >&2; exit 1; }
