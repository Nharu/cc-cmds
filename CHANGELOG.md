...
v2.1.0이 세운 계약 표면을 실제 동작과 맞춘다. 표면이 실제와 다른 말을 하는 것을 고치는 작업에서 같은 종류의 어긋남이 여러 새 형태로 재발했고, 대부분 v2.1.0이 새로 만든 것이다. 함께 훅의 자동 승인 경로를 좁힌다.

### Security

- **`notify.sh` 호출을 승인하는 훅 분기가 명령줄 전체를 승인하던 문제** — `active-notify` PreToolUse 훅에는 자동 승인 분기가 둘 있다. 그중 `notify.sh` 호출을 승인하는 분기는 승인이 명령줄 전체에 적용됐고, 이제 그 분기는 단독 단순 호출만 매치한다. 이전에는 자동 승인되던 복합 형태(예: `cd … && bash … notify.sh …`)는 이제 권한 다이얼로그를 거친다. 훅은 플러그인 설치만으로 모든 Bash 호출에 붙으므로 이 스킬을 쓰지 않는 세션도 대상이다. 다른 분기(`terminal-notifier` 권한 테스트)는 승인해야 할 형태가 다행 셸 블록이라 의도적으로 앵커하지 않았고, **여전히 명령줄 전체를 승인한다.**
- **훅이 함께 발행하던 세션 지속 승인 규칙 제거** — `notify.sh` 호출을 자동 승인할 때 훅은 그 호출만 승인한 것이 아니라, 세션 내내 남는 승인 규칙을 함께 발행했다. 그 규칙은 와일드카드 패턴이라 훅이 실제로 내리는 판정을 표현할 수 없었다 — 훅이 승인하는 형태 하나(`bash` 없이 스크립트 경로로 시작하는 호출)를 규칙은 담지 못했고, 반대로 훅이 거절하는 미지 서브커맨드는 규칙이 다시 받아들이는 형태였다. 이제 규칙을 발행하지 않는다. 이 호출이 자동 승인되는 것은 그대로다 — 훅은 플러그인 설치만으로 모든 Bash 호출에 붙으므로 호출마다 다시 판정한다.
- **승인 대상 경로의 형태 검사** — 훅은 명령어 단어가 `active-notify/scripts/notify.sh`로 끝나기만 하면 승인했다. 이제 절대경로여야 하고, `..`로 밖으로 나가거나 셸이 실행 시점에 확장하는 글로브를 담은 경로는 같은 리터럴로 끝나더라도 다이얼로그를 거친다. 남는 것은 공격자가 고른 루트 아래의 깨끗한 절대경로이며, 그것은 설치된 플러그인의 루트를 아는 것 없이는 구별할 수 없다.

### Fixed
- **요청하지 않은 알림을 등록하라고 지시하던 문단 제목** — 알림 요청이 없을 때의 처분을 적은 제목이 "등록한다"로 적혀 있었다. 같은 문단의 본문과 바로 아래 두 항목, 그리고 상시 로드되는 호출 규약은 모두 반대를 지시한다. 제목만 뒤집혀 있었고, 제목은 모델이 먼저 읽는 자리다.
- **예약 알림을 취소할 수 없던 문제** — 예약 작업 ID를 잃은 뒤의 회수 절차가 조회·삭제 도구를 이름으로 부르지 않아, 스키마를 먼저 적재해야 하는 이 도구들을 적재할 방법이 없었다. 게다가 대상을 예약 명령 문자열로 식별하라고 적었는데, 목록이 그 문자열을 잘라 보여 주므로 식별 키가 잘린 부분에 들어간다. 이제 도구를 이름으로 부르고, 목록이 온전히 보여 주는 예약 시각과 일회성·반복 표시를 식별 키로 쓰며, 후보가 여럿이면 사용자에게 묻는다.
- **반복 알림 요청이 단발로 라우팅되던 문제** — 여러 이벤트를 열거하면서 각 항목에 재발 한정사를 붙인 요청이 단발 여러 회로 확정되어, 정해진 횟수만 발송한 뒤 사이클이 조용히 만료됐다. 열거 안에 한정사 없는 유한한 순간이 섞여 있는 요청(`"시작할 때랑 각 커밋마다 알림 줘"`)도 같은 방향으로 바뀌며, 그 순간은 버려지지 않고 같은 사이클의 구성원으로 합류한다. 같은 문서 안에서 이 입력을 정반대로 처분하라고 적은 다른 절도 함께 정리해, 판단이 한 곳에서만 이뤄지게 했다. 반복 사이클의 배너는 그룹으로 묶이지 않아 알림 센터에 그대로 쌓이며 이 사이클에는 상한이 없다 — 원하지 않으면 `"알림 취소"`로 중단한다.
- **README가 규약이 금지하는 자동 종료를 약속하던 문제** — 사용자 대면 안내가 반복 알림의 종료를 "해당 이벤트가 더 나오지 않게 되면"이라는 예측으로 서술했는데, 이는 v2.1.0이 행위자 기준으로 교체한 바로 그 기준이다. 사용 예시 표의 한 행도 규약이 금지하는 경우에 자동 종료를 약속했다. 두 곳 모두 행위자 기준으로 다시 적었다.
- **제어 규칙 요약본이 본문 정본의 2차 전본이 되던 문제** — 규칙을 한 구간으로 모으면서 본문에만 있어야 할 근거와 비용 논증까지 옮겨 담았다. 같은 커밋이 그 비용을 두 자리에 적으면서 "이 비용은 한 곳에서만 서술된다"는 문장도 함께 새로 적어, 그 문장은 처음부터 참인 적이 없었다. 반대로 한 규칙은 옮겨지는 과정에서 정본의 판정 축 문장을 떨어뜨렸다. 담아야 할 것과 빼야 할 것을 바로잡았다.
- **드리프트 린트의 부정문 예외가 사실상 상시 열려 있던 문제** — 부정어 판정이 단어 경계 없는 부분 일치라 `note`·`notify`·`now`·`fire-now` 같은 흔한 단어가 전부 "이 줄은 부정문이다"를 만족시켰다. 이제 온전한 단어로만 부정을 인정하고, 부정어가 해당 문구 가까이 있을 것을 함께 요구한다. 영어는 앞에서, 한국어는 뒤에서 찾는다 — 한국어는 부정 어미가 문구 뒤에 오기 때문이다.
- **드리프트 린트가 한 가지 인용 표기만 인식하던 문제** — 틀린 절 번호를 담은 인용이라도 표기가 조금만 다르면 통과했다. 실제로 이 문서의 이력이 그렇다 — 수정 전 공유 문서의 참조 다섯 건 중 린트가 본 것은 셋뿐이었고, 못 본 둘이 정확히 그 표기였다. 이제 표기 변형을 표준형으로 접은 뒤 대조한다. 수정 전 트리 적발이 9건에서 11건으로 올라 사람이 세던 다섯 건을 전부 덮으며, 늘어난 2건은 인용 검사가 새로 본 것이다. 금지 검사는 별개로 `매 turn`·`per-turn` 두 표기에서 `every`/`each`/`per turn`·`턴마다` 계열까지 넓혔다.
- **드리프트 린트가 줄 단위로 판정하던 문제** — 문단을 다시 줄바꿈하는 것만으로 금지 문구와 부정 문구가 양쪽 다 검사에서 빠져나갔다. 이제 빈 줄로 나뉜 문단 단위로 판정한다.
- **드리프트 린트가 놓치던 나머지** — 등록된 공유 파일이 사라지면 조용히 건너뛰던 것을 위반으로 바꾸고, 제목 앵커에 최소 길이를 요구하며, 제목이 같은 헤딩이 둘 이상이면 경고를 남긴다.
- **한정사 없는 열거가 어느 분기에도 착지하지 못하던 문제** — `"커밋할 때랑 배포할 때 알림 줘"`처럼 재발 한정사 없이 두 시점을 열거한 요청이 배제 절에 걸려 다음 분기로 넘어갔는데, 이후 분기가 전부 한정사를 조건으로 삼아 어디에도 닿지 못했다. 두 번 요청하고 한 번 받는다. 이제 한정사가 없으면 열거를 유한한 것으로 보고 요청한 수만큼 발송한다. 항상 적재되는 호출 규약도 같은 판정을 내리도록 다시 구조화했다 — 두 표면이 다른 답을 내면 읽는 쪽이 그중 하나를 고르게 된다.
- **디스패처가 성공을 반환하며 아무것도 하지 않던 경로** — 락 예산을 파생하는 비용 상수가 유효한 정수가 아니면 파생이 실패한 채 스크립트가 계속 진행돼, macOS 시스템 bash에서 종료 상태 0으로 끝났다. 사용자는 알림이 걸렸다고 듣고 배너는 오지 않는다. 이제 첫 사용 전에 두 예산을 검증한다.
- **문자열이 아닌 세션 ID가 알림을 유실시키던 문제** — 세션 ID를 명령줄에 주입하는 경로에서 문자열이 아닌 값이 인용되며 갈라져, 대입문 뒤에 명령어 자리의 부유 단어가 하나 생겼다. 그 호출은 실행되지 못하고 알림이 사라진다.
- **명확화 질문에 답이 없을 때 마지막 배너를 잃던 절차** — 무엇마다 알릴지 물었으나 답이 오지 않은 채 작업이 끝나는 분기가, 이 사이클은 더 발사할 것이 없다는 근거를 세우고 취소만 지시했다. 그 근거는 참이 아니다 — 같은 문서가 사용자 작업 도구 호출이 있었던 턴의 종료를 발사 시점으로 규정하고, 작업이 끝나는 턴이 바로 그런 턴이다. 이제 발사한 뒤 취소한다.
- **예약 프롬프트의 줄바꿈 형태** — 예약된 알림 명령을 한 줄로 쓰라는 제약이 `&&`·명령 치환·리다이렉션만 금지하고 줄바꿈에는 침묵했다. 줄바꿈도 같은 이유로 자동 승인에서 빠지며, 그 실패는 아무도 보고 있지 않은 예약 턴에서 난다.
- **사용자 안내의 조건 없는 진술 셋** — README가 Bash 권한 다이얼로그는 뜨지 않는다고 단언했는데 위 Security 항목이 공시하는 사실과 부딪친다. 이제 어떤 호출에 한정되는지와 어떤 형태가 다이얼로그를 거치는지를 함께 적는다. 사용 예시 표 전체에 걸려 있던 자동 종료 전칭은 같은 표의 시계 행 둘이 반증하므로 모델 판단으로 발송되는 행으로 좁혔고, 두 이벤트를 한 사이클로 묶는 행에만 빠져 있던 중단 방법을 채웠다. 공유 절차 문서가 예약 틱에도 센티넬이 있다고 적은 네 자리도 실제 동작에 맞춰 고쳤다.

### Added

- **서브커맨드 열거 드리프트 검사** — 문서·훅·디스패처가 열거하는 서브커맨드 목록이 서로 어긋나면 CI가 잡는다. 기준 목록을 디스패처의 분기 코드에서 직접 읽어 내므로 목록이 늘어도 유지 비용이 들지 않으며, 이름 추가·이름 변경·삭제 세 방향을 모두 검출한다. 산문에 두 이름이 함께 나오는 것만으로 위반을 주장하는 네 번째 규칙도 만들어 보았으나, 현 트리에서 정상 문장 열 건을 위반으로 잡아 채택하지 않았다.
- **픽스처 확충** — 드리프트 린트 픽스처를 6종에서 35종으로 늘리고, 락 자세 픽스처를 더해 예산 소진 시 발화 분기가 배너를 내지 않고 플래그를 그대로 두는지와 두 예산이 실제로 분리돼 있는지를 못박았다. 각 픽스처가 무엇을 고정하는지는 픽스처 옆에 함께 출하되는 변이 목록과 그것을 돌리는 하네스에 실측으로 기록했다 — 목록만 두면 판정이 다음 사람이 만드는 기계에 달리므로 하네스가 산출물이다. 하네스는 잘못된 기대값을 거부하는지 스스로 점검할 수 있다.
- **잠금 자세를 고정하는 픽스처** — `arm`의 원자적 쓰기를 쓰는 도중에 다른 프로세스가 표집해 확인하고, 이름 바꾸기가 실패하는 경로에서 임시 파일이 회수되는지를 결정적으로 확인하며, 예약 틱이 배너를 내지 못한 이유를 매번 말하는지 확인한다. 이 성질들은 앞선 릴리스에서 고정한다고 적혀 있었으나 실제로는 어느 것도 고정되지 않았고, 그 사실을 적은 픽스처 헤더도 함께 정정했다.

### Changed

- **`arm`의 플래그 쓰기를 원자 교체로** — 락을 잡지 못한 `arm`이 플래그를 제자리에서 잘라 쓰던 것을 임시 파일 경유 교체로 바꿨다. 잘라 쓰는 순간을 발화 분기가 읽으면 방금 요청한 알림이 그대로 삭제될 수 있었다.
- **답 없는 명확화 질문 뒤의 퇴화 사이클** — 무엇마다 알릴지 되물었는데 답이 오지 않고, 그 사이 발화할 인스턴스도 나오지 않은 채 맡은 작업이 끝나면, 이제 모델이 그 사이클을 스스로 취소하고 그 사실을 답변에 적는다. 이전에는 발화할 것이 없는 사이클이 그대로 남았다.
- **예약 틱의 미설치 안내** — `terminal-notifier`가 없을 때 예약된 알림 틱은 안내를 한 번만 찍고 그 뒤로는 조용했다. 이제 틱마다 찍는다. 틱은 아무도 보지 않는 자리에서 돌고 사전 조건 검사는 예약 시점에 끝났으므로, 그 줄이 틱이 남길 수 있는 유일한 흔적이다.
- **공유 절차 문서의 §4 제목** — 본문이 신설한 동명 섹션과 이름이 겹쳐, 두 표면을 함께 든 모델이 나중의 참조가 어느 쪽을 가리키는지 판별할 수 없었다. 이 파일의 remit이 디스패처 사실이라는 점을 제목에 반영하고, 본문의 적재 지시에 있는 목록도 함께 맞췄다.

### Known limitations

- 폐기된 계약을 금지하는 검사는 그 계약을 가리키는 **표기**를 막는 것이지 계약 자체를 막는 것이 아니다. 같은 내용을 다른 문장으로 풀어 쓰면 통과한다.

## [2.1.0] - 2026-08-05

`active-notify`의 시계 기반 경로가 처음으로 실제 동작한다. 그리고 v2.0.0에 기재되지 않은 변경 하나를 여기서 함께 기록한다 — repeat 모드의 발화 축이 턴 경계에서 이벤트 클래스 인스턴스로 옮겨 갔고 모델 self-cancel이 신설됐다. 그 변경이 본문만 개편하고 상시 로드 표면을 손대지 않아 이번 작업의 절반이 그 정리다. 소급 기재를 택하지 않은 이유는, v2.0.0 시점의 계약을 그때 문서화하면 실제로는 표면 간에 어긋나 있던 상태를 정상으로 기술하게 되기 때문이다.

세 층이 각각 독립적으로 어긋나 있었고 그 상태에서 기존 픽스처 35종이 전부 통과했다. 즉 CI가 계약 드리프트를 한 건도 잡지 못했다.

### Added

- **`notify.sh fire-oneshot <workflow> <summary>`** — 플래그를 읽지도 쓰지도 않고 락도 잡지 않는 상태 무관 배너 서브커맨드. 살아 있는 ARM 사이클은 이 호출 전후로 바이트 동일하다. 전용 배너 그룹을 쓰는 것이 핵심인데, 같은 그룹 배너는 알림 센터에서 서로를 대체하므로 사이클 그룹을 재사용하면 틱 배너가 완료 배너를 지운다.
- **시계 기반 요청의 스케줄러 위임** — 반복 cadence와 일회성 지연 **양쪽 모두** 예약 작업으로 간다. 네 단계(전제조건 점검 / 예약 / 발신 / 고지)를 모두 수행해야 위임이 성립하며, 예약에 이르지 못하는 두 경로는 반드시 사용자에게 무엇을 못 했고 왜인지를 말한다. 침묵으로 끝나는 분기는 없다.
- **`## Control-Flow Invariants` 섹션** — 본문 여기저기 흩어져 있던 5개 제어 규칙(턴 종료 자동 발화 없음과 인스턴스 하한, fire-first, 의심 시 발화, self-cancel 범위와 순서, 벽시계 요청 ARM 불가)을 한 구간으로 모았다. `active-notify`는 이제 위치 린트의 면제 대상이 아니다 — 면제 사유였던 "종료가 훅·도구 주도 경계"가 self-cancel 도입으로 거짓이 되었다.
- **`scripts/lint-active-notify-drift.sh`** + 짝 테스트·픽스처 6종 — 상시 로드 표면과 공유 절차에서 폐기된 계약을 가리키던 두 표기(`매 turn`·`per-turn`)를 금지하고, 공유 문서가 표준 표기(`SKILL.md §N`)로 적은 절 인용을 **제목 앵커까지** 실재 헤딩과 대조한다. 존재 검사만으로는 이번 사고를 0건 적발한다(균일 +1 시프트는 항상 이웃한 실재 헤딩에 착지한다). 수정 전 트리에서 9건 적발.

### Changed

- **frontmatter 6행 교체** — 무조건적 `마다`→repeat 주장을 재발 한정사가 명사를 수식할 때로 좁히고, 벽시계 요청 ARM 금지·인스턴스 단위 발화·self-cancel과 fire-first 순서를 상시 로드 표면에 올렸다. `usage` 줄도 함께 고쳤다 — 예산 린트는 읽지 않지만 문구 린트 구간에는 포함된다.
- **self-cancel 허용 조건을 행위자 기준으로** — "더 나올 인스턴스가 없다"는 예측이며 모델은 자기가 만들지 않는 이벤트에 대해 그것을 말할 자격이 없다. 이제 발화한 인스턴스가 전부 모델 자신이 dispatch한 것의 완료였고 자기 계획에 인스턴스를 더 만들 단계가 없을 때만 허용하며, 사용자 본인의 커밋·CI·배포처럼 모델이 몰지 않는 계열이면 금지한다.
- **오탐 비용 서술 정정** — 잘못된 self-cancel은 배너 하나가 아니라 계열의 남은 배너 전부를 잃고, 자리를 비운 사용자에게는 손실 자체가 관측되지 않는다.
- **게이트 캐스케이드 재설계** — 절 분리와 벽시계 배제를 앞단에 두고, 재발 한정사가 명사를 수식하는 경우와 수식할 명사가 없는 경우를 분리했다. 열거 연결어에 `와`/`과`/`하고`/`이랑`을 추가하고 닫힌 목록이 아님을 명시한다.
- **인스턴스 하한** — 관측 가능한 경계가 없는 제네릭 클래스에 한해 user-task 도구 호출이 있었던 턴의 종료를 인스턴스 1회로 본다. 정밀 클래스에는 적용되지 않으며, 외부 스케줄러가 연 턴은 인스턴스가 아니다.

### Fixed

- **`cancel` 후 플래그 부활** — `cancel`과 `arm`이 fire 분기의 락에 참여하지 않아, 두 호출의 쓰기가 fire의 read-modify-write 창 안에 떨어지면 되돌려졌다. 취소한 사용자가 계속 배너를 받고, 모드를 좁히는 re-ARM은 넓은 모드로 복구됐다. 두 서브커맨드에 별도의 작은 예산(약 1초)과 fail-open 자세를 주고, trap을 락 보유 여부로 게이트한다.
- **락 루프 주석의 소요 시간** — fire 분기 상한의 실제 벽시계 시간은 주석이 주장하던 10초가 아니라 40초대다. 반복마다 `/bin/sleep`을 fork하므로 반복당 비용이 sleep 인자의 4배다.
- **훅 γ 폴백의 세션 ID 주입** — 값이 인용되지 않아 세션 ID의 셸 메타문자가 그대로 명령이 됐다. 이 경로는 명령을 재작성하면서 동시에 allow를 반환하는 유일한 자리라, `;` 하나가 `notify.sh` 호출을 다른 명령으로 바꿔 놓고 자동 승인까지 물려받았다.
- **공유 문서의 스테일 인용 2건** — 균일 +1 시프트로 각각 이웃 섹션을 가리키고 있었고 그중 하나는 의미가 반전돼 있었다. 남은 인용은 전부 제목 앵커를 달았다.
- **`_common` 적재 지시의 경로** — bare 상대 경로는 세션 작업 디렉토리에서도 스킬 디렉토리에서도 해석되지 않는다.
