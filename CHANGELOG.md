...
- **`scripts/lint-active-notify-drift.sh`** + 짝 테스트·픽스처 6종 — 상시 로드 표면과 공유 절차에서 폐기된 계약 문구를 금지하고, 공유 문서의 `SKILL.md §N` 인용을 **제목 앵커까지** 실재 헤딩과 대조한다. 존재 검사만으로는 이번 사고를 0건 적발한다(균일 +1 시프트는 항상 이웃한 실재 헤딩에 착지한다). 수정 전 트리에서 9건 적발.
