...
- **한 문자 클래스 시험이 두 역할을 겸직해 양쪽으로 틀렸고, 그래서 축소가 출하 상태에서 한 번도 발동하지 않았다.** 그 시험은 「이 토큰이 표면에 실릴 경로인가」와 「이 토큰이 해소돼야 하는가」를 동시에 정하는데 두 영역은 같지 않다. 표면 쪽으로는 진짜 저장소 상대 경로가 문자 클래스나 확장자 요건에서 탈락하면 검사되지도 계수되지도 않고 조용히 0 을 기여했다 — 확장자 없는 루트 파일과 명령이 한 코드 스팬에 함께 든 경로가 그것이고, 위트니스 63개에서 313 출현이다. 표면이 과소 유도되고 트리 상태 단언이 공짜로 통과한다. 트리거 쪽으로는 `gate.sh` · `/` · `2/2` · `paid/unpaid` 처럼 경로가 아닌 토큰이 전부 문자 시험을 통과하고 아무것도 해소하지 않아, 폴백이 63개 중 62개 파일에서 발화하고 표면이 매 런 트리 전체가 됐다 — 무관한 세션의 파일 수정이 매 런 단언을 깨뜨리고 자동 재감사가 0회이므로 그 밤의 감사가 거기서 끝난다. 이 토큰들은 경로가 아니므로 「경로를 저장소 상대로 적어라」는 생산 측 규칙으로 사라지지 않는다 — 규칙을 완벽히 지킨 리더도 산문에서 파일을 `gate.sh` 라고 부른다. 정량 영역을 두 갈래의 합집합으로 가른다. **해소되면 영역 안이고** — 접미사와 꼬리 구두점을 뗀 문면이 해소되면 문자를 묻지 않으므로 비ASCII 성분 경로·무확장자 경로·문장이 쉼표를 붙인 경로가 전부 여기로 들어온다 — **아니면 경로꼴일 때만 영역 안이다** — 구분자 `/` 를 보유하면서 확장자나 명시적 줄 접미사를 함께 가질 것. 토큰화에는 코드 스팬 내부의 공백 구분 런을 항으로 더한다. 좁힘이 경로를 하나도 잃지 않는 이유가 두 방향을 한 편집에 담을 수 있게 한다 — 구분자 없는 저장소 상대 경로는 루트 파일이고 루트 파일은 해소되므로 앞 갈래가 이미 붙들고 있다. 실측(위트니스 63개): 폴백 토큰 출현 3,208 → 303(−90.6%), 폴백이 발화하는 파일 62/63 → 38/63, 정량 영역 밖으로 떨어지던 실재 경로 313 → 0, 미결정 토큰 0 유지, 유도되는 표면 116 → 134 경로. 잔여 303 중 245(80.9%)는 실재 파일을 경로의 꼬리로만 부른 **부분 경로 인용**이며, 그것은 경로를 잘못 적은 것이므로 생산 측 규칙이 닫는 부류다. 이 코퍼스는 그 규칙보다 앞서 생산됐으므로 38/63 은 그 규칙을 지킨 런의 값이 아니다 — 부분 경로 인용만으로 발화하는 파일을 빼면 15/63 이고, 이 값은 상한 추정이지 실측이 아니다. 구분자 요건이 새 잔여를 하나 만든다 — 루트 파일의 깨진 인용은 구분자가 없어 뒤 갈래에 들지 않고 해소되지 않아 앞 갈래에도 들지 않는다. 구분자 요건을 빼면 비경로 부류가 그대로 돌아와 축소가 다시 죽으므로 그 교환은 성립하지 않으며, 노출은 저장소 루트 파일 수로 유계다. 계약이 그것을 잔여로 이름한다. (#565)
## [2.9.0] - 2026-09-06
