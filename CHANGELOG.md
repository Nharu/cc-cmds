...
- **유계 포화 읽기 크레딧** — 직전 진전 이후의 무진전 구간이 읽기 인가만으로 이루어졌고 그 개수가 여덟 미만이면 무진전 경계 호출을 건너뛴다. 그 구간의 시작점은 진전 다이제스트 자신이 정하고 어떤 행이 진전인지는 다시 열거하지 않으며, 판단·의무·인계처럼 읽기가 아닌 인가가 하나라도 섞이면 그 구간은 읽기 구간이 아니다. 교대는 대화 이력이 없는 새 프로세스라 세그먼트를 선언하기 전에 반드시 읽어야 하고 그 최소 개수가 경계의 문턱과 같았다 — 정상적으로 시작하는 경로가 곧 발화 조건이었다. 크레딧은 진전 벡터의 성분이 아니라 경계 안의 억제이므로 벡터도 스냅숏 다이제스트도 바이트 하나 바뀌지 않는다. 억제는 리셋이 아니라서 읽기가 끝나고 진짜 정체가 이어지면 다음 판정들에서 문턱에 도달하고, 읽기가 하나도 없는 빈 구간은 사면되지 않는다 (#689, #693, #682)
