...
- **훅이 함께 발행하던 세션 지속 승인 규칙 제거** — `notify.sh` 호출을 자동 승인할 때 훅은 그 호출만 승인한 것이 아니라, 세션 내내 남는 승인 규칙을 함께 발행했다. 그 규칙은 와일드카드 패턴이고 훅의 매처는 명령줄 전체에 앵커된 정규식이라, 둘은 서로 다른 기계가 서로 다른 규칙으로 읽는다. 두 형태가 각각 무엇을 받아들이는지 — 어느 쪽이 다른 쪽보다 넓은지 — 는 권한 엔진을 실제로 구동해 두 집합을 비교해야 정해지며, 그 측정은 하지 않았다. 규칙을 발행하지 않는 것은 그 관계를 알아서가 아니라 알 필요를 없애기 때문이다. 이 호출이 자동 승인되는 것은 그대로다 — 훅은 플러그인 설치만으로 모든 Bash 호출에 붙으므로 호출마다 다시 판정한다.
...
- **드리프트 린트가 한 가지 인용 표기만 인식하던 문제** — 틀린 절 번호를 담은 인용이라도 표기가 조금만 다르면 통과했다. 실제로 이 문서의 이력이 그렇다 — 수정 전 공유 문서의 참조 다섯 건 중 린트가 본 것은 셋뿐이었고, 못 본 둘이 정확히 그 표기였다. 이제 표기 변형을 표준형으로 접은 뒤 대조한다. 그때 이 검사로 잰 수정 전 트리 적발은 9건에서 11건으로 올라 사람이 세던 다섯 건을 전부 덮었고, 늘어난 2건은 인용 검사가 새로 본 것이다. 두 수치는 그 시점의 검사가 낸 것이며, 그 뒤 같은 릴리스에서 이 검사의 인식층이 교체됐으므로 지금 다시 재면 같은 수가 나오지 않는다. 금지 검사는 별개로 `매 turn`·`per-turn` 두 표기에서 `every`/`each`/`per turn`·`턴마다` 계열까지 넓혔다.
...
- **상시 적재 표면이 규약 본문이 거부한 기준으로 자기 취소를 허가하던 문제** — 모델이 반복 알림 사이클을 스스로 끝내도 되는 조건이 두 표면에서 달랐다. 본문은 「이 이벤트가 더 나오지 않을 것」이라는 예측을 명시적으로 거부하고 그 인스턴스를 모델 자신이 일으켰을 것을 요구하는데, 모델이 항상 먼저 읽는 표면은 정확히 그 예측 기준으로 허가하고 요구 조건이 없었다. 사용자 커밋이나 CI가 만드는 이벤트에 걸린 사이클이 모델의 잘못된 예측으로 조용히 닫힐 수 있었다. 이제 두 표면이 같은 조건을 적는다.
- **정본 호출 예시가 훅이 거부하는 형태를 보여 주던 문제** — 같은 릴리스가 자동 승인을 절대경로 호출로 좁혀 놓고, 모델이 실제로 베끼는 네 예시는 축약된 상대 경로를 그대로 보여 주고 있었다. 예시대로 쓰면 권한 다이얼로그를 거친다. 네 예시를 절대경로 형태로 바꿨다.
- **변이 하네스가 작업 중인 파일을 제자리에서 고쳤다 되돌리던 문제** — 픽스처의 판별력을 재는 장치가 추적되는 원본을 직접 고친 뒤 시작 시점 사본으로 되돌렸다. 그 사이에 같은 파일을 편집하면 편집이 흔적 없이 사라지고 `git status`는 깨끗하며 장치는 전부 통과를 보고한다. 이제 원본은 읽기 전용 스냅숏으로 한 번만 복사되고 변이는 별도 사본에만 적용되며, 실행 전후로 원본 해시가 같은지 단언한다.
- **공유 절차 문서의 조건 없는 진술 다섯** — 사전 조건 검사가 「모두 발화 전에 확인된다」고 적혀 있었으나 상태 무관 배너 경로는 넷 중 둘만 거치며, 두 가드의 플래그 처리 서술과 불변량의 전칭도 그 경로에는 성립하지 않는다. 미설치 안내가 「조용한 실패로 넘어가지 않는다」고 적은 문장은 바로 앞 절이 서술하는 센티넬이 정확히 그렇게 만든다. 다섯 자리 모두 참인 범위로 좁혔다.
...
- **픽스처 확충** — 드리프트 린트 픽스처를 6종에서 39종으로 늘리고, 락 자세 픽스처를 더해 예산 소진 시 발화 분기가 배너를 내지 않고 플래그를 그대로 두는지와 두 예산이 실제로 분리돼 있는지를 못박았다. 서브커맨드 린트 15종, 라이프사이클 37종, 훅 12종이 함께 선다(이 릴리스를 쓰는 시점의 트리에서 센 값이다). 각 픽스처가 무엇을 고정하는지는 네 변이 코퍼스에 행으로 적혀 있고, 각 행은 자기가 붉힐 픽스처의 집합을 실행 전에 선언한다 — 개수가 아니라 집합이라, 한 줄을 공유하는 두 편집이 각각 하나씩 서로 다른 픽스처를 붉히는 경우가 일치로 통과하지 않는다. 하네스는 잘못된 기대값을 거부하는지 스스로 점검하며, 그 통제군은 실재하는 픽스처를 과대·과소 선언해 거절이 벡터 비교에서 나왔음까지 단언한다.
