...
리뷰 리포트를 소비해 처분을 확정하는 경로를 신설한다. 지금까지 그 경로는 설계된 적이 없어 분류 지시문이 `design`에 자유 텍스트 인자로 밀입국하고 있었다.

문제는 리뷰 루프가 발산한다는 것이 아니었다. 사이클당 이득을 세 독립 경로로 재면 모두 1 미만으로 수렴하고 156개 체인이 전부 종료했다. 실제 결함은 **종료 조건이 이 과정이 도달할 수 없다고 증명된 상태를 요구했다**는 것이다 — 써 온 기준 `P0+P1+P2 == 0`이 156개 체인 중 81개(52%)에서 한 번도 성립하지 않았다. 리뷰어 민감도가 1 미만인 한 바뀌지 않은 코드에서도 새 발견이 계속 나오므로 고정점은 0이 아니고, 수렴하는 신호(P1, 6배 붕괴)와 수렴하지 않는 바닥(P3, 점유율 67%에서 평평)이 서로 다른 채널인데 종료 기준이 후자까지 읽고 있었다.

### Added

- **`/cc-cmds:review-remediate`** — 리뷰 리포트(및 PR 리뷰 코멘트)를 **1회** 소비해 발견별 처분을 확정하고, `반영` 항목의 완화 설계를 함께 저작하며, 보류분을 이슈로 등록한 뒤 문서를 `implement`에 인계하는 신규 스킬. 재리뷰 없음·에이전트 팀 없음의 단일 결정론적 패스이며, 동결은 **(입력 집합, base SHA, head SHA) 삼중쌍당 정확히 1회 처분**으로 얻는다(코드에서 바이트 동결은 아무것도 고치지 않았다는 뜻이라 모순이다). 게이트는 살아 있는 head에 대해 평가하고 리포트가 선언한 대상과 불일치하면 열리지 않는다 — 실측된 한 리포트는 발행 시점에 이미 커밋 하나만큼 낡아 있었다.
- **처분 5 arm 폐쇄 집합**(`반영`·`반증`·`승격`·`보류`·`제외`) — 차단 대역에서 `보류`·`제외`가 불가한 것은 심각도가 아니라 **종료 조건에서 파생**된다. 종료 술어가 그 대역을 읽는 이상 판단만으로 그것을 비우는 arm은 곧 종료 기울기다. `반증`이 남는 이유는 코드 관측에 게이트되어 「루프를 끝내고 싶다」는 동기로 도달할 수 없기 때문이고, `승격`이 남는 이유는 사이클을 끝내는 것이 아니라 설계 사이클을 시작하기 때문이다. **어떤 영구 처분 arm도 이연보다 비싼 증거를 요구할 수 없다** — 값싼 경로가 백로그뿐이면 예산 압박 아래 모든 것이 백로그로 흐른다.
- **비대칭 레벨 선택자 파서**(`scripts/parse-review-report.awk`) — 전수 374파일·826 심각도 섹션·5,507 발견에서 도출했다. 이전 규약은 33개 리포트 표본이 정했고 **정상 입력 7.1%에서 하드 스톱해 반증됐다**. 심각도 앵커는 이모지 정확히 하나와 대역 토큰의 합성이고, 발견 탐지는 형태 열거에서 **그룹 안의 레벨 선택**으로 강등된다 — 인식이 사라진 것이 아니라 한 단계 위로 옮겨 가 그 자리에서 훨씬 얕아졌으며, 그 대가로 남는 미계측 잔여(형제가 전부 비대괄호인 서브트리의 조용한 붕괴)를 계약 본문에 함께 적는다.
- **정지 팔 셋** — 섹션 단위 가드는 상시 가드로 두되 **「fail closed」라 이름 붙이지 않는다**(코퍼스에서 발화가 0건이며, 갖지 않은 속성으로 이름 붙은 가드는 건강한 입력에서 위반처럼 읽히고 위반처럼 읽히는 줄은 무시된다). 실제로 정지시키는 것은 구조적 검사 A1(섹션이 있는가)과 선언적 검사 A2(리포트 자신의 산술이 맞는가)이며, **어느 하나도 단독으로는 조용한 0건을 다 덮지 못한다.**
- **행 키 함수**(`scripts/row-key.awk`) — 서수를 폴백이 아니라 **무조건·섹션 스코프 성분**으로 둔다. 5,507 블록 전량에서 사이클 내 충돌 0건. 재출현 키는 줄 번호와 서수를 빼며(줄은 연속 버전 간 79.7% 표류하고 서수는 파싱의 속성이지 발견의 속성이 아니다), `claim_key`를 행 키에서 파생하는 것을 금지한다 — 파생하면 그 휘발성이 재출현 키로 조용히 재수입된다.
- **배수구 세 장치** — 쓰기 시점 상한(최근 30일 해결 종결 건수의 1배)·30일 무접촉 자동 만료·3개월 연속 저조 시 전면 중단. 상한은 트래커별로 독립 계산하고 자동정지는 전역인데, **전역 행동을 가진 레포별 장치는 어느 해상도에서도 비정합**이기 때문이다. 자동 만료로 닫힌 이슈는 상한의 분자에서 빠진다 — 포함하면 커맨드가 포기하는 속도만큼 입금하게 되고, **포기는 해결이 아니다.**
- **`scripts/lint-review-remediate-pins.sh`** + 테스트·픽스처 10종 — 상수 블록·`검증 시점` 닫힌 enum·산출물 H2 정확 일치·루프 기계 금지 목록을 CI에서 고정하고, 다섯 개의 구획 펜스(단일 질문 호출 지점·스폰 0·경계/후보 어휘의 진부분집합·예산 선택지·라우팅 단일 신호)를 마커 구획에 대해 검사한다. 전체 파일 검색은 정당한 산문에 발화하므로 구획화가 선행 조건이다.

### Changed

- **`_common/verification.md`의 작성자 열과 잔여 항목 탄생 경로에 새 커맨드를 추가** — 이 커맨드가 `반증`의 관측 기록과 저작한 수정에 대한 잔여 항목을 쓰려면 그 계약의 원소여야 한다. 편집 없이 저작하면 산출물의 항목이 계약이 인정하지 않는 작성자가 쓴 것이 되고, 소비자의 게이트가 출처 없는 잔여 항목 위에서 열린다. **교차 스킬 비용은 편집한 파일 수가 아니라 소비한 계약 수로 센다** — 남의 계약을 어기는 가장 흔한 방법은 그 파일을 고치는 것이 아니라 열어 보지 않은 채 그것이 허락하지 않는 일을 하는 것이고, 그 위반은 diff에 나타나지 않는다.

### Notes

- v1은 **`보류`가 사실상 도달 불가한 상태로 출하된다.** 배수구가 라벨 스코프 인구 위에 정의돼 있는데 그 인구가 비어 있어 상한이 모든 트래커에서 0이고, 라우팅이 헤더 리터럴 하나로 좁혀지며 이연 인구의 약 28.5%에서 `보류`가 떨어진다. **부트스트랩 값을 지어내지 않는 대가**이며, 커맨드는 그 0이 왜 0인지를 첫 사용자가 보는 자리에 인쇄한다.
- **팀 없음은 데이터 없이 출하된다.** 살아남는 근거는 리뷰어가 이미 선택지를 공급했다는 것뿐이고(차단 대역 존재율 99.6%), 그것은 종료할 것이라 기대할 근거이지 종료한다는 측정이 아니다. 산출물이 `완화 출처` 계측기로 그 빈칸을 메우기 시작한다.
## [2.1.0] - 2026-08-18
