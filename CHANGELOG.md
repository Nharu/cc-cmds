...
- **핀 커버리지를 측정 가능하게 만들고 실제로 올렸다.** 드라이버가 종료 코드만 비교하고 진단을 버려 FAIL 픽스처가 왜 실패했는지 아무도 검증하지 않았다. 진단 선언 형식을 신설하고, 핀을 영역·위치로 결속하고, 발화한 적 없는 규칙 하나를 삭제했다. **측정값은 두 지표로 나뉘며 2026-08-10, 이 릴리즈의 마지막 정정 커밋 직전 트리에서 측정했다** — **변조 탐지 80/80**(배열 원소를 하나 지우면 해당 드라이버가 실패하는가), **픽스처 행사 51/80**(그 제거로 빨개진 것이 OK 픽스처만이 아니라 FAIL 픽스처이기도 한가). 하나로 뭉치면 말없이 관대한 쪽을 고르게 되므로 이름을 붙여 따로 적는다. **변조 탐지가 전수인 것은 항등적으로 참이다** — 요약 줄이 모든 배열 크기를 나르고 그 줄이 OK 픽스처에 바이트로 핀돼 있어, 계수되는 원소는 추가되는 즉시 이 지표에 덮인다. 모집단 80은 두 스위트의 배열 원소 합(48 + 32)이고, 세 번째 스위트는 핀 배열이 없어 이 모집단 밖이며 단언 삭제 뮤턴트 10/10으로 따로 측정된다. **천장을 함께 적는다** — 이 지표들이 재는 것은 핀이 배열에서 사라졌을 때 CI가 빨개지는가이지 핀이 옳은 것을 가리키는가가 아니고, 이 릴리즈가 고친 결함 다수는 아홉 lint 어느 것도 읽지 않는 교차 파일 자연어 계약 텍스트에 있었다.
...
- **파리티 표면 6경로** — `scripts/lint-review-prompt-parity.sh`(358줄), 그 테스트 드라이버, 픽스처 20 디렉터리(87파일), `Makefile` 2줄. **삭제된 픽스처는 모두 98개이며 나머지 11개는 살아남은 펜스 두 루트(`lint-skill-invariants`·`lint-verification-literals`)에서 함께 빠졌다.** 이 회계가 필요한 이유는 처음 적힌 것과 다르다 — 그 두 루트는 **케이스를 하나도 잃지 않았고**(각각 10→10, 3→3), 빠진 11개는 제거된 파리티 표면을 겸해 들고 있던 디렉터리였다. 즉 이 줄이 막는 오독은 "어느 라이브 펜스가 입력을 잃었는가"가 아니라 **98과 109 중 어느 쪽이 이 개편의 제거 규모인가**이다. 이 lint는 대상 부재 시 조용히 통과하므로 남겨두어도 CI는 초록이었다 — 유지보수 비용을 근거로 삼은 개편에서 조용히 살아남기 좋은 형태였다.
