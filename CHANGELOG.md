...
- **조상 판정이 필드 경계에 앵커되고, 형식이 어긋난 다이제스트는 조상 판정에 들어가지 않는다.** 탐침이 원장 전체에 대한 앵커 없는 부분 문자열이라 셋이 통과했다 — 팁 half 가 빈 값이면 탐침이 「`prev=` 를 담은 행이 있는가」로 무너져 행이 하나라도 있으면 참이 되고, 실제 팁의 짧은 접두사도 참이 되며, 무엇보다 호출자가 자기 근거 문자열에 실어 보낸 리터럴이 인가 행 본문에 무변형으로 착지하므로 **감시 대상이 자기 감시를 통과할 자격증명을 스스로 주조**할 수 있었다. 주조된 값은 형태가 완벽하므로 길이·문자집합 검사로는 걸러지지 않는다. 행의 진짜 `prev` 는 항상 마지막 필드이고 구분자는 공급된 값에서 치환되므로, 그 경계에 앵커하는 것이 주조를 닫는다.
