...
**patch인 이유.** 사이드카 버전 범프는 호환 파괴가 아니라 **진단을 옳게 만드는 기전**이다. 수정 전에는 도달 가능한 인스턴스가 없었고(유일한 리더가 유일한 라이터가 쓰지 않는 경로를 봤다), 리더 측 경로가 고쳐져 도달 가능해지면 명명된 사유와 함께 스킵된다. 새 동작을 요구하는 항목 하나 — 구속 티어 승인 프롬프트 신설 — 은 의도적으로 이번 스코프 밖에 두었다. **새 durable 표면은 스코프 밖이 아니라 이번 릴리즈가 출하한다**: 정합 패스 완료 스탬프가 그것이고, 앞선 문면은 그것을 미출하로 적어 등급의 근거 문단 안에 검사 가능한 거짓 진술을 남겼다. 등급 자체는 그 사실 위에서도 서므로 바뀌지 않는다 — 스탬프는 하류가 읽는 기계 판독 표면이지 사용자에게 새 동작을 요구하는 것이 아니다. **커밋 종류로도 등급을 세우지 않는다.** 등급은 위 문단의 근거 위에서 **결정으로 유지된 것**이지 커밋 종류에서 유도된 것이 아니다. 무엇을 세는지부터 적는다 — 이 저장소는 릴리스 커밋 규약을 채택하며 그 커밋에 태그를 단다. 범위 안의 유일한 사용자 대면 기능은 이 등급을 정당화하지 않는다 — 그것은 반대로 자르는 증거다. 그 범위 `ca9a178..c98af92`(v2.0.0 릴리스 커밋부터 이 릴리즈의 마지막 커밋까지, 두 점 표기가 왼쪽 끝을 제외)의 원소는 17이고 종류는 `fix` 12 · `test` 1 · `feat` 1 · `chore` 1 · 머지 커밋 2다. **`feat` 1은 반대 증거이므로 숨기지 않고 적는다**: `d6858c5`가 그것이고, 그 하나가 minor를 요구한다고 읽는 규약도 성립한다. 이 문서는 그렇게 읽지 않기로 결정했고 그 결정을 여기 남긴다. **앞선 문면이 이 범위에 있다고 적은 `refactor`·`ci`는 이 범위에 없다** — 종류로 등급을 세우지 않는다는 결론은 그대로지만 근거로 든 열거가 틀렸었다. **경계 규약은 채택 결정일 뿐 아직 발효되지 않았다**: 이 저장소의 태그는 현재 0개이고, 릴리스 절차를 적은 파일이 없으며, CI 워크플로는 태그와 무관하게 돈다. 그래서 위 범위식은 오늘 손으로 재유도해야 하고, 태그가 붙기 전까지는 그 끝점 해시가 규약의 대체물이다.
