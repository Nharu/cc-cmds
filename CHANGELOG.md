...
감사의 동결 창이 감사가 소유하지도 읽지도 않은 표면을 재던 두 자리. 둘 다 리더 팬아웃이 끝난 뒤에 발화하므로 비용을 전액 지출한 다음 중단시키고, 무인 런에서는 자동 재감사가 0이라 그 밤의 감사가 거기서 끝난다. 그리고 그 축소가 재는 경로의 어휘가 생산 측에도 소비 측에도 규정되지 않아, 축소 자체가 아무 오류 없이 비어 버리던 자리.

그 어휘를 정하는 술어는 다섯 차례에 걸쳐 분석적으로만 다시 쓰였고, 그것이 분류해야 할 실물 위트니스에는 한 번도 돌려 본 적이 없었다. 이번에는 먼저 쟀다 — 위트니스 63개를 재니 오늘 술어를 통과하는 앵커 셀의 92.4%(105 중 97)가 코드 스팬에 감싸여 처분이 미결정이었고, 경로를 담은 표 행의 67.9%(327 중 222)가 「공백 없는 단일 토큰」 요건 때문에 정량 영역 밖에 있었다. 아래 세 항목은 그 관찰에서 유도한 것이지 그 반대가 아니다.

### Fixed

- **워크트리 경로 집합 단언의 예외 패턴이 면제해야 할 이름을 다 댈 수 없었다.** 그 기제는 남의 워크트리를 이름으로 면제하려 했으므로 예상하지 못한 이름이 나타나는 순간 실패한다. 한 저장소에서 이름에 `run` 이 들어간 워크트리 13개 중 선언된 인픽스에 걸린 것은 1개였고, 이름에 `run` 이 아예 없는 워크트리도 실재한다. 그래서 감사가 자기 파이프라인이 만든 워크트리 때문에 멈췄다 — 예외 선언이 존재하는 목적 자체가 달성되지 않았다. 극성을 뒤집어 「호출자가 선언한 소유 접두를 가진 항목의 집합」만 비교한다. 자기 것의 이름은 호출자가 언제나 댈 수 있다. 이 단언이 더 이상 잡지 않는 **두 가지**와 그 자리를 무엇이 덮는지를 계약이 같은 자리에서 적는다 — 무관한 워크트리가 늘었다는 통지가 하나이고, 호출자가 선언한 접두 밖에서 스스로 만든 워크트리가 다른 하나다. 뒤엣것은 소유 접두에 걸리지 않아 이 단언이 재지 않고 기제 소유 누수 단언도 그 접두만 세므로, 세 단언 중 어느 것도 대신 잡지 않는다. 기제 소유 누수 단언과 무엇이 다른지도 가른다 — 축은 기준선 의존성이고, 창이 열릴 때 이미 있던 실험 워크트리 하나에 대해 앞은 통과하고 뒤는 실패한다.
- **트리 상태 단언이 감사가 읽지도 않은 파일의 변화에 발화했다.** 그 기준선은 감사가 소유하지 않고 다른 세션들과 공유하는 워크트리 전체를 잰다. 관측된 사례에서 창 안에 움직인 것은 같은 저장소에서 진행 중이던 다른 작업의 파일 7개였고, 문서 해시와 나머지 단언은 전부 일치했는데도 감사가 끝났다. 재감사에는 「트리가 가라앉았다」는 사람의 판단이 필요한데 그 판단의 유일한 증거인 기준선이 방금 깨진 것으로 증명된 상태라, 이 지점은 무인 런에서 사실상 항상 멈춤이었다. 이제 관측된 차이가 가리키는 경로가 호출자의 측정 표면과 교집합이 없으면 불일치로 세지 않는다. 그 비교는 `git status --porcelain -z` 위에서 취하도록 고정한다 — 기본 출력은 비ASCII 경로를 C-인용하고(이 저장소에도 그런 추적 경로가 한 디렉터리에 12개 있다) rename 을 한 항목에 두 경로로 실으며 미추적 디렉터리를 `/` 로 끝나는 한 항목으로 줄이므로, 「차이가 가리키는 경로」를 산문으로만 두면 그 형태들에서 교집합이 비어 불일치를 놓친다. 측정 표면은 선택되지 않고 유도되며, 유도가 실제로 내놓는 것은 호출자가 **인용한** 경로 집합이다 — 읽은 집합보다 좁다(성공한 도달성 검사는 행을 만들지 않고 심볼 앵커는 경로를 싣지 않는다). 계약과 두 아암 모두 좁은 쪽만 주장한다. 닫히는 조건은 위트니스 파일이 아니라 유도에 대한 술어다 — 세 소스 중 하나라도 **닫는 porcelain 관측보다 먼저 봉인된** 지속 기록에서 열거되지 않으면 공집합이 아니라 트리 전체가 되며, 이 한 문장이 위트니스 부재·파싱 실패·세 번째 소스의 기록 부재·부분 유도를 함께 포섭한다. 그 시계는 호출자가 충족할 수 있는 유일한 값이다 — 위트니스는 창이 열린 뒤 스폰되는 리더가 쓰므로 창 개시 이전을 요구하면 트리거가 매 런 성립해 이 축소가 출하 상태에서 한 번도 발화하지 않고, 봉인이 닫는 관측보다 앞서기만 하면 diff 를 읽고 표면을 좁히는 해저드는 그대로 막힌다. 기준 트리도 같은 폴백을 받는다 — porcelain 관측을 취하는 트리가 표면의 기준 트리가 아니면 두 쪽은 경로 단위로 비교할 수 없으므로 표면은 트리 전체다. 「파싱된다」에는 구조 검사를 준다 — 앵커 대조표의 표제와 헤더 행이 있어야 하고, 없는 위트니스는 없는 것과 같은 폴백을 받는다. 아무것도 선언하지 않는 호출자의 이 단언은 바이트 단위로 불변이다. (#564, #565)
