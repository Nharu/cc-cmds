...
- **핀 커버리지를 측정 가능하게 만들고 실제로 올렸다.** 드라이버가 종료 코드만 비교하고 진단을 버려 FAIL 픽스처가 왜 실패했는지 아무도 검증하지 않았다. 진단 선언 형식을 신설하고, 핀을 영역·위치로 결속하고, 발화한 적 없는 규칙 하나를 삭제했다. **측정값은 두 지표로 나뉘며 2026-08-11, 이 릴리즈의 마지막 정정 커밋 직전 트리에서 두 독립 방법으로 재측정했다** — **변조 탐지 83/83**(배열 원소를 하나 지우면 해당 드라이버가 실패하는가), **픽스처 행사 53/83**(그 제거로 빨개진 것이 OK 픽스처만이 아니라 FAIL 픽스처이기도 한가). 하나로 뭉치면 말없이 관대한 쪽을 고르게 되므로 이름을 붙여 따로 적는다. **변조 탐지가 전수인 것은 항등적으로 참이다** — 요약 줄이 모든 배열 크기를 나르고 그 줄이 OK 픽스처에 바이트로 핀돼 있어, 계수되는 원소는 추가되는 즉시 이 지표에 덮인다. 모집단 83은 두 스위트의 배열 원소 합(51 + 32)이고, 계수 규약은 블록 스코프 배열 원소다 — 배열이 아닌 스칼라 핀 하나(참조 파일의 파일 끝 센티널)는 이 모집단 밖이고 arity 기전이 그것을 덮지 않는다. **세 번째 스위트에 대해서는 비율을 발행하지 않는다.** 모집단 규칙이 아직 합의되지 않았고, 이 레포에서 그 비율은 세 번 제안돼 세 번 무너졌다 — n분의 n은 계수가 아니라 «더 없다»는 주장이고, 매번 분모가 그 시점에 아직 발견되지 않은 생존자를 제외하고 있었다. 대신 계수와 상태를 적는다: **사이드카 쪽에는 어떤 계수도 공표하지 않는다.** 앞선 문면은 열여섯 멤버 모집단에 대해 한 오라클에서 사살 16 · 생존 0을 적었는데, 두 번째 독립 열거 — 구조적 술어 지점을 훑는 방식 — 은 서른셋 멤버에 생존자 열하나 이상을 낸다. 두 유도가 불일치하므로 이 불릿 자신의 규칙이 적용된다: **n분의 n은 계수가 아니라 「더 없다」는 주장**이고, 명명된 모집단에 대한 생존 0이 정확히 그 주장을 계수의 옷을 입혀 한 것이다. 모집단 규칙이 합의되기 전까지 이 축에는 수 대신 성질을 적는다 — **사이드카 스키마의 구조 핀은 오늘 두 오라클에서 서로 다른 모집단을 낳고, 어느 쪽이 옳은지는 아직 정해지지 않았다.** 오라클을 결과와 같은 문장에 적는 이유는 앞선 세 분모의 충돌 원인이 모집단이 아니라 이름 붙지 않은 오라클이었기 때문이다. **천장을 함께 적는다** — 이 지표들이 재는 것은 핀이 배열에서 사라졌을 때 CI가 빨개지는가이지 핀이 옳은 것을 가리키는가가 아니고, 이 릴리즈가 고친 결함 다수는 아홉 lint 어느 것도 읽지 않는 교차 파일 자연어 계약 텍스트에 있었다.
...
- **파리티 표면 6경로** — `scripts/lint-review-prompt-parity.sh`(358줄), 그 테스트 드라이버, 픽스처 20 디렉터리(87파일), `Makefile` 2줄. **삭제된 픽스처 디렉터리는 이 개편이 직접 들어낸 파리티 표면에서 20개(87파일), 살아남은 펜스 두 루트(`lint-skill-invariants`·`lint-verification-literals`)에서 함께 빠진 것이 13개(25파일), 합계 33개(112파일)다.** 앞의 둘은 소계이고 33은 총계다 — 20과 13을 더한 값이며, 이 문장에서 총계인 것은 그 하나뿐이다. **두 루트가 잃은 것은 디렉터리이지 검사 케이스가 아니다**: `lint-skill-invariants`는 3개, `lint-verification-literals`는 10개의 디렉터리를 잃었고 케이스 수는 각각 3→3, 10→10으로 그대로다. 단위를 적지 않은 채 「소계와 나머지」로 쓰면 두 수가 같은 것을 센다고 읽히고, 그러면 살아 있는 펜스가 입력을 잃은 것처럼 보인다 — 그것이 이 줄이 막는 오독이며, 처음 적힌 근거와는 다르다. **숫자를 다시 유도하려는 사람에게**: 세 값은 이 릴리즈가 갈라져 나온 지점의 첫 부모에서 재현된다. 발산한 팁을 기준으로 삼으면 다른 값이 나오고, 그 차이는 계수 규약이 아니라 기준 커밋의 차이다. 이 lint는 대상 부재 시 조용히 통과하므로 남겨두어도 CI는 초록이었다 — 유지보수 비용을 근거로 삼은 개편에서 조용히 살아남기 좋은 형태였다.
