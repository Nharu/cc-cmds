...
- **핀 커버리지를 측정 가능하게 만들고 실제로 올렸다.** 드라이버가 종료 코드만 비교하고 진단을 버려 FAIL 픽스처가 왜 실패했는지 아무도 검증하지 않았다. 진단 선언 형식을 신설하고, 핀을 영역·위치로 결속하고, 발화한 적 없는 규칙 하나를 삭제했다. 드롭 스윕으로 두 핀 스위트의 모든 리터럴이 단독 제거 시 스위트를 빨갛게 만드는 것을 확인했다.
...
- **파리티 표면 6경로** — `scripts/lint-review-prompt-parity.sh`(358줄), 그 테스트 드라이버, 픽스처 20 디렉터리(87파일), `Makefile` 2줄. **삭제된 픽스처는 모두 98개이며 나머지 11개는 살아남은 펜스 두 루트(`lint-skill-invariants`·`lint-verification-literals`)에서 함께 빠졌다** — 이 회계 없이는 CHANGELOG만으로 제거 범위를 재구성하는 독자가 어느 라이브 펜스가 입력을 잃었는지 잘못 그린다. 이 lint는 대상 부재 시 조용히 통과하므로 남겨두어도 CI는 초록이었다 — 유지보수 비용을 근거로 삼은 개편에서 조용히 살아남기 좋은 형태였다.
