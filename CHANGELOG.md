...
- **리포트 템플릿의 종료 술어 인터록.** 역할 중 하나라도 `checkpoint` 나 `absent` 로 해소된 복구 리포트는 드라이버가 매치하는 발견 요약 줄의 형상을 파일 어디에도 두지 않고, 계층을 밝히는 대체 줄을 낸다. 억제 대상은 자리가 아니라 형상이며, 머지 권고 루브릭의 `P0 + P1 = 0` 줄은 억제 대상이 아니다. 복구 리포트에는 역할별 계층·라운드·파일·`seq`·`nonce 미검증` 을 담는 `## 복구 프로버넌스` 절이 붙는다.
