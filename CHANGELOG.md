...
- **진전 벡터가 라우터 자신의 행위를 하나도 세지 않았다.** 커밋·push·PR·머지는 전부 이 게이트를 통해 인가되고 각각 원장 행을 쓰는데 그중 무엇도 벡터를 움직이지 않아, 한 시간 동안 결함을 잡아 올린 런이 정지 상태로 읽히고 정체 경계가 그 위에서 발화했다. 그 발화는 출력 한 줄로 끝나지 않는다 — 열려 있는 승인이 나머지 경계를 유예시키고 종료를 막는데 게이트는 라우터가 타이핑한 답을 받지 않으므로, 눈에 보이는 일을 하고 있는 런이 자고 있을지 모르는 사람을 기다리며 멈춰 선다. 읽기를 넘는 등급의 행위와 절 정산·런 스코프 막힘 해소를 벡터에 넣는다. 등급 필드가 아예 없는 행은 진전으로도 행위 예산으로도 세지 않는다 — 등급을 모르는 행위는 무엇이 바뀌었다는 증거가 아니다.
