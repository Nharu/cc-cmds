...
- **복구 파견과 원 리뷰 스테이지가 리포트 경로에 함께 쓸 수 있는 창이 남아 있다.** 그 창은 닫지 못했다. 파견 직전의 회수 호출은 도달 가능한 모든 경로에서 pid 파일이 이미 지워져 있어 첫 줄에서 반환하므로 아무 신호도 보내지 않으며, 신호가 갔다면 그것은 더 이른 시점이고 죽음을 확인할 pid 파일은 신호를 보낸 그 호출이 지웠다. 발행 전후 두 번의 술어 검사가 창을 좁힐 뿐 상한은 진술하지 않는다. 대신 회수가 `kill -0` 판정을 스탬프로 남기고 복구 행이 그것을 `원회수=alive|dead|미상` 으로 실어, 남은 창의 크기를 사후에 관측할 수 있게 했다.
...
- **인터록이 계층만 보고 라운드를 보지 않았다.** 부분 복구 줄이 최저 계층과 함께 **최저 라운드**를 싣고, 「원 스테이지가 발행했을 발견 집합과 같다」는 거짓 주장을 「살아남은 라운드까지의 발견 집합」으로 고쳤다. 그리고 라운드를 발행 조건 자체에 넣었다 — 전 역할이 `witness` 여도 그 해소 라운드가 원장 블록이 기록한 마지막 라운드에 못 미치면 정상 줄을 내지 않고 `최저 계층 라운드 미달` 인 부분 복구 줄을 낸다. 팔과 리포트 템플릿 양쪽이 같은 조건을 적는다.
...
- **로스터 행과 위트니스 파일을 잇는 조인 키가 정의돼 있지 않고 단사성도 요구되지 않았다.** 로스터를 원장 행으로 옮기면서 계층 해소는 파일명에 남겨 조인이 처음 필요해졌는데 그 키가 어디에도 없었다. 조인을 `role/scope` 선두 슬러그와 파일명 슬러그의 축자 일치로 못 박고, **한 파일은 한 행에만 배정한다** — 단사성이 없으면 접두가 겹치는 두 행이 한 파일로 둘 다 `witness` 가 되어, 아무것도 내지 못한 좌석이 다른 좌석의 산출물로 덮이고 전칭이 참이 된다.
