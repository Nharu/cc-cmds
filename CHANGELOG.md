...
- **검증 동결 리터럴 lint의 소비자 동기화 검사와 픽스처**: 공유 문서의 어휘가 소비자 쪽에서만 사라지는 회귀를 CI에서 잡도록 검사 분기를 추가하고, 그 분기를 실패·성공 양방향으로 실제 구동하는 픽스처를 함께 넣는다. 검사는 파일 전역이 아니라 판별 리터럴 기준으로 좁혀, 무관한 산문에 같은 토큰이 남아 있다고 해서 삭제가 가려지지 않게 한다.
