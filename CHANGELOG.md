...
v2.1.0이 세운 계약 표면을 실제 동작과 맞춘다. 표면이 실제와 다른 말을 하는 것을 고치는 작업에서 같은 종류의 어긋남이 여러 새 형태로 재발했고, 대부분 v2.1.0이 새로 만든 것이다. 함께 선재 보안 결함 하나를 닫는다.

### Fixed

- **훅 자동 승인이 명령줄 전체에 적용되던 문제** — `active-notify` PreToolUse 훅이 `notify.sh` 호출을 자동 승인할 때 그 승인이 명령줄 전체에 적용됐다. 이제 단독 단순 호출만 매치한다. 이전에는 자동 승인되던 복합 형태(예: `cd … && bash … notify.sh …`)는 이제 권한 다이얼로그를 거친다. 훅은 플러그인 설치만으로 모든 Bash 호출에 붙으므로 이 스킬을 쓰지 않는 세션도 대상이었다. `terminal-notifier` 권한 테스트 경로는 이번 변경 대상이 아니다.
...
- **반복 알림 요청이 단발로 라우팅되던 문제** — 여러 이벤트를 열거하면서 각 항목에 재발 한정사를 붙인 요청이 단발 여러 회로 확정되어, 정해진 횟수만 발송한 뒤 사이클이 조용히 만료됐다. 열거 안에 한정사 없는 유한한 순간이 섞여 있는 요청(`"시작할 때랑 각 커밋마다 알림 줘"`)도 같은 방향으로 바뀌며, 그 순간은 버려지지 않고 같은 사이클의 구성원으로 합류한다. 같은 문서 안에서 이 입력을 정반대로 처분하라고 적은 다른 절도 함께 정리해, 판단이 한 곳에서만 이뤄지게 했다.
...
- **드리프트 린트가 한 가지 인용 표기만 인식하던 문제** — 틀린 절 번호를 담은 인용이라도 표기가 조금만 다르면 통과했다. 실제로 이 문서의 이력이 그렇다 — 수정 전 공유 문서의 참조 다섯 건 중 린트가 본 것은 셋뿐이었고, 못 본 둘이 정확히 그 표기였다. 이제 표기 변형을 표준형으로 접은 뒤 대조하며, 금지 검사도 `매 turn`·`per-turn` 두 표기에서 `every`/`each`/`per turn`·`턴마다` 계열까지 넓혔다. 수정 전 트리 적발이 9건에서 11건으로 올라 사람이 세던 다섯 건을 전부 덮는다.
...
- **픽스처 확충** — 드리프트 린트 픽스처를 6종에서 19종으로 늘려 넓힌 검사들이 각각 무엇을 고정하는지 단독으로 붉어지게 했고, 락 자세 픽스처를 더해 예산 소진 시 발화 분기가 배너를 내지 않고 플래그를 그대로 두는지와 두 예산이 실제로 분리돼 있는지를 못박았다.
