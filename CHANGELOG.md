...
- **트리 상태 단언이 감사가 읽지도 않은 파일의 변화에 발화했다.** 그 기준선은 감사가 소유하지 않고 다른 세션들과 공유하는 워크트리 전체를 잰다. 관측된 사례에서 창 안에 움직인 것은 같은 저장소에서 진행 중이던 다른 작업의 파일 7개였고, 문서 해시와 나머지 단언은 전부 일치했는데도 감사가 끝났다. 재감사에는 「트리가 가라앉았다」는 사람의 판단이 필요한데 그 판단의 유일한 증거인 기준선이 방금 깨진 것으로 증명된 상태라, 이 지점은 무인 런에서 사실상 항상 멈춤이었다. 이제 관측된 차이가 가리키는 경로가 호출자의 측정 표면과 교집합이 없으면 불일치로 세지 않는다. 그 비교는 `git status --porcelain -z` 위에서 취하도록 고정한다 — 기본 출력은 비ASCII 경로를 C-인용하고(이 저장소에도 그런 추적 경로가 한 디렉터리에 10개 있다) rename 을 한 항목에 두 경로로 실으며 미추적 디렉터리를 `/` 로 끝나는 한 항목으로 줄이므로, 「차이가 가리키는 경로」를 산문으로만 두면 그 형태들에서 교집합이 비어 불일치를 놓친다. 측정 표면은 선택되지 않고 유도되며, 유도가 실제로 내놓는 것은 호출자가 **인용한** 경로 집합이다 — 읽은 집합보다 좁다(성공한 도달성 검사는 행을 만들지 않고 심볼 앵커는 경로를 싣지 않는다). 계약과 두 아암 모두 좁은 쪽만 주장한다. 닫히는 조건은 위트니스 파일이 아니라 유도에 대한 술어다 — 세 소스 중 하나라도 창이 열리기 전에 쓰인 지속 기록에서 열거되지 않으면 공집합이 아니라 트리 전체가 되며, 이 한 문장이 위트니스 부재·파싱 실패·세 번째 소스 부재·부분 유도를 함께 포섭한다. 「파싱된다」에는 구조 검사를 준다 — 앵커 대조표의 표제와 헤더 행이 있어야 하고, 없는 위트니스는 없는 것과 같은 폴백을 받는다. 아무것도 선언하지 않는 호출자의 이 단언은 바이트 단위로 불변이다. (#564, #565)
