...
- **원 리뷰 스테이지를 회수하지 않은 채 복구를 파견했다.** 대기 함수는 스테이지를 살려 둔 채 반환할 수 있고 그러면 리포트 경로에 쓰는 주체가 둘이 된다. 파견 직전에 회수하고, 발행 직전에 술어 줄을 한 번 더 검사하며, 그래도 남는 창을 팔 문면에 적었다.
...
- **인터록이 계층만 보고 라운드를 보지 않는 것을 리포트 문면에서 드러낸다.** 부분 복구 줄이 최저 계층과 함께 **최저 라운드**를 싣고, 「원 스테이지가 발행했을 발견 집합과 같다」는 거짓 주장을 「살아남은 라운드까지의 발견 집합」으로 고쳤다. 술어를 내는 조건 자체는 바뀌지 않았다 — 라운드 1 위트니스만 남은 복구는 여전히 정상 줄을 낸다.
