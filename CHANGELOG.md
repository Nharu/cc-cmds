...
**patch인 이유.** 사이드카 버전 범프는 호환 파괴가 아니라 **진단을 옳게 만드는 기전**이다. 수정 전에는 도달 가능한 인스턴스가 없었고(유일한 리더가 유일한 라이터가 쓰지 않는 경로를 봤다), 리더 측 경로가 고쳐져 도달 가능해지면 명명된 사유와 함께 스킵된다. 새 동작을 요구하는 항목 하나 — 구속 티어 승인 프롬프트 신설 — 은 의도적으로 이번 스코프 밖에 두었다. **새 durable 표면은 스코프 밖이 아니라 이번 릴리즈가 출하한다**: 정합 패스 완료 스탬프가 그것이고, 앞선 문면은 그것을 미출하로 적어 등급의 근거 문단 안에 검사 가능한 거짓 진술을 남겼다. 등급 자체는 그 사실 위에서도 서므로 바뀌지 않는다 — 스탬프는 하류가 읽는 기계 판독 표면이지 사용자에게 새 동작을 요구하는 것이 아니다. **커밋 종류로도 등급을 세우지 않는다.** 등급은 위 문단의 근거 위에서 **결정으로 유지된 것**이지 커밋 종류에서 유도된 것이 아니다. 무엇을 세는지부터 적는다 — 이 저장소는 릴리스 커밋 규약을 채택하며 그 커밋에 태그를 단다. 그 범위 `0278264..<이 릴리즈의 릴리스 커밋>`(v2.1.0 릴리스 커밋부터, 두 점 표기가 왼쪽 끝을 제외)의 원소는 78이고 종류는 `fix` 65 · `test` 3 · `docs` 3 · `chore` 3 · `refactor` 1 · `ci` 1 · 머지 커밋 2이며 **`feat`는 0이다.** **이 문단의 앞선 판본은 다른 범위를 세고 있었고 그 사실을 지우지 않고 적는다** — 이 릴리즈는 원래 `2.0.1`로 번호가 매겨져 `ca9a178..c98af92`(원소 17 · `feat` 1)를 세고 있었다. 기준 브랜치가 같은 조상에서 `2.1.0`을 먼저 출시해 번호를 다시 매기면서 왼쪽 끝점이 옮겨 갔고, 그러자 그 `feat` 하나(`d6858c5`)가 이 범위 밖으로 나갔다. **그 커밋은 사라진 것이 아니라 자기가 속한 릴리즈로 갔다** — 위 `2.1.0` 절의 `### Added`가 그것을 적는다. 앞선 판본은 그것을 「등급을 반대로 자르는 증거」라고 적었는데, 지금 이 범위에는 그런 증거가 없다. **등급이 그 때문에 쉬워진 것으로 읽지 말 것** — 바뀐 것은 이 범위가 무엇을 담느냐이지 이 릴리즈가 무엇을 바꾸느냐가 아니다. **경계 규약은 채택 결정일 뿐 아직 발효되지 않았다**: 이 저장소의 태그는 현재 0개이고, 릴리스 절차를 적은 파일이 없으며, CI 워크플로는 태그와 무관하게 돈다. 그래서 위 범위식은 오늘 손으로 재유도해야 하고, 태그가 붙기 전까지는 그 끝점 해시가 규약의 대체물이다.
...
- **`active-notify`의 반복 모드가 이벤트 스코프로 바뀌고 self-cancel이 생겼다.** 반복 요청이 사용자 취소 전까지 무한히 발화하던 것을, ARM 시점에 그 어휘가 수식하는 이벤트 클래스를 함께 포착해 **그 클래스의 인스턴스가 관측될 때마다** 발화하고, 그 계열이 끝났다고 판단되면 모델이 스스로 취소하도록 바꿨다. 사용자 취소는 그대로 상시 백스톱이다. 함께 들어간 것: 반복 트리거처럼 보이지만 실제로는 분배 한정사인 발화를 반복으로 오인하지 않도록 판별기에 열거 우선 파싱 순서를 두었고, 시계 기반 타이밍은 알림 반복이 아니라 인터벌 위임으로 라우팅하며, 이벤트 클래스를 특정할 수 없는 맨명사 발화는 확인질문으로 좁힌다.
  ...
  **이 항목은 뒤늦게 기재된다.** 해당 커밋은 `d6858c5`이고, 이 릴리즈가 나갈 때 이 문서에 0회 등장했다. 기록되지 않은 동작 변경은 이 문서가 막으려고 존재하는 실패이며 버전 등급과 무관하므로, 등급을 바꾸지 않고 기재만 늦게 착지시킨다.

  **어느 릴리즈에 속하는지는 범위로 정한다: `ca9a178..0278264`** — v2.0.0 릴리스 커밋에서 이 릴리스 커밋까지이며, 두 점 표기가 왼쪽 끝을 제외하므로 v2.0.0 릴리스 커밋 자신은 들어가지 않는다. 이 식을 그대로 실행하면 원소 9개에 `feat` 5개이고 `d6858c5`가 그중 하나다 — 나머지 넷이 위에 적힌 예산 항목들이다. 해시와 끝점을 함께 적는 이유는 **산문 서술로는 갈리지 않기 때문**이다. 실제로 독립 검토자 둘이 이 커밋을 두고 서로 다른 릴리즈를 지목한 전례가 있고, 그때 갈린 것은 계수가 아니라 어느 끝점을 쓰느냐였다.
