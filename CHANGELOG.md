...
- **팀 세대 인덱스 복구가 원인이 아니라 디스크 술어로 라우팅된다 — 잘라내기 금지.** 부분 존재로 도달하는 상태를 다섯으로 열거하고, 각 상태를 원장이 이미 나르는 두 컬럼(팀 스크래치 디렉토리 그룹 · 위트니스 논스의 라운드 태그)으로 구별한다. 그중 한 상태에는 복구 arm이 아예 없어 유일한 선택지가 중단이었고 재진입이 같은 메뉴를 재발화했다. **왜 뻔한 수리가 금지되는가**: 빠진 값을 「그냥 채우는」 백필은 현재 팀 행을 이전 세대로 떨어뜨려 `max(epoch)` 스코핑이 그 행들을 제외하게 만들고, 로스터가 다른 팀 행으로 통째 치환돼 타 역할 산출물이 이 단계의 산출물로 소비된다 — 게이트가 막으려던 절단이 그것을 고치려는 arm을 통해 재현된다. 그래서 백필은 「에폭 없는 행이 전부 종결 상태이고 이 단계보다 이른 라운드에 있으며 스탬프된 형제와 스크래치 디렉토리를 공유하지 않을 때」에만 선택된다.
