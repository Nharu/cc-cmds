...
이탈 기록 사이드카 스키마가 파일 종단자를 고정하지 않아, 공유 쓰기 루프가 읽는 종단자 변수의 **빈 값이 계약 적합값**이었다. 빈 종단자는 절단 검사와 완결성 검사를 동시에 단락시키고, 그 상태에서 carry-forward 규칙("파일의 마지막 비어있지 않은 줄까지 strip")이 센티넬 대신 이전 블록의 **필수 필드 줄을 지운다**. 쓰기 게이트는 이를 보지 못한다 — `diff`가 내용 기반이라 삭제된 필드 줄을 새 블록이 다시 내보내는 바이트 동일한 줄과 정렬하므로 removed 측에 아무것도 나타나지 않고, 두 블록의 마지막 필드 값이 다를 때만 거부되어 밖에서는 간헐적으로 보인다. 블록은 append 즉시 동결되고 삭제·절단이 금지되며 rewrite 형식이 없으므로 **복구 경로가 없다**. 세 하네스가 독립 재현했다.

### Added

- **`active-notify`의 반복 모드가 이벤트 스코프로 바뀌고 self-cancel이 생겼다.** 반복 요청이 사용자 취소 전까지 무한히 발화하던 것을, ARM 시점에 그 어휘가 수식하는 이벤트 클래스를 함께 포착해 **그 클래스의 인스턴스가 관측될 때마다** 발화하고, 그 계열이 끝났다고 판단되면 모델이 스스로 취소하도록 바꿨다. 사용자 취소는 그대로 상시 백스톱이다. 함께 들어간 것: 반복 트리거처럼 보이지만 실제로는 분배 한정사인 발화를 반복으로 오인하지 않도록 판별기에 열거 우선 파싱 순서를 두었고, 시계 기반 타이밍은 알림 반복이 아니라 인터벌 위임으로 라우팅하며, 이벤트 클래스를 특정할 수 없는 맨명사 발화는 확인질문으로 좁힌다.

  **이 항목은 이 릴리즈 범위 안에서 사용자 대면 동작을 바꾸는 유일한 기능 커밋인데 이 문서에 0회 등장했다** — 해당 커밋은 `d6858c5`다. 기록되지 않은 동작 변경은 이 문서가 막으려고 존재하는 실패이며 버전 등급과 무관하므로, 등급을 바꾸지 않고 기재만 뒤늦게 착지시킨다. **버전 번호는 그대로 `2.0.1`이다.**

  **범위의 두 끝점을 여기 적는다: `ca9a178..c98af92`** — 직전 릴리스 커밋에서 이 릴리스 커밋까지이며, 두 점 표기가 왼쪽 끝을 제외하므로 직전 릴리스 커밋 자신은 들어가지 않는다. 이 식을 그대로 실행하면 원소 17개이고 `d6858c5`가 그 안에 있다. 해시와 끝점을 함께 적는 이유는 **산문 서술로는 갈리지 않기 때문**이다 — 독립 검토자 둘이 서로 다른 커밋을 지목한 전례가 있고, 실제로 다른 끝점(`755fb4f`, PR 병합 커밋)을 쓰면 원소 14개에 `d6858c5`가 **빠지고** 범위 내 `feat`이 0건이 되어 정반대 결론이 나온다. 어느 쪽도 계산이 틀린 것이 아니라 **다른 규약**이며, 이 저장소가 채택한 것은 릴리스 커밋 범위 쪽이다.

### Changed

- **이탈 기록 스키마에 파일 종단자를 고정하고 스키마 버전을 v2로 올렸다.** 어떤 쓰기 게이트 술어로도 닫을 수 없어 수정은 종단자여야 한다. 종단자만 추가하면 생성 쓰기가 실패해 이후 모든 append가 실패하므로, 생성 디스패치·버전 검사 순서·형태 스레딩·구조 핀·CI 배선을 한 커밋에 함께 실었다.
- **v1 사이드카는 스킵되고 그 스킵이 무엇을 뜻하는지 보고된다.** v1 파일은 이 범프가 닫는 바로 그 경로에 이미 필드를 잃었을 수 있고 그 손실은 되돌아오지 않으므로, 마이그레이션은 자동이 아니라 사용자의 수기 전사다.
- **리뷰어가 이탈 사이드카를 존재할 수 없는 경로에서 조회하던 문제를 고쳤다.** 경로 토큰이 같은 스킬 안에서 이미 리뷰 slug에 결속돼 있어, 부재가 "미공시 이탈"로 흘러가 정상적으로 공시된 이탈이 전부 구현자에 대한 허위 지적으로 뒤집혔다. 부재는 이제 "채널 사용 불가"로 처분된다.
- **사이드카 경로 도출에 물리 경로 정규화를 넣었다.** 심볼릭 링크된 상위 경로로 저장소에 도달하면 같은 문서가 서로 다른 키를 얻어 writer와 reader가 다른 파일을 쓰고 읽는데, 두 쓰기가 모두 성공하므로 아무것도 이를 보고하지 않았다.
- **재소집 팀의 세대 인덱스 재스탬프를 2단계(전 행 clear → 전 행 write)로 바꿨다.** 행별 재스탬프가 중단되면 모든 행에 값이 있고 값만 달라 로스터가 균일 존재로 분류하고 팀을 조용히 절단한다. clear를 먼저 하면 같은 중단이 부재가 되어 fail-close한다.
- **팀 정리 절차에 진행 중 페이즈 경계 arm을 신설했다.** 워크플로 최종 완료에만 해야 할 정리를 진행 중에 수행하면 아직 발행되지 않은 위트니스가 사라지고 뒤 페이즈가 재개할 근거를 잃는다.
- **정합 패스 완료를 기계 판독 스탬프로 남기고 감사가 1회 읽는다.** 그 패스를 건너뛴 문서와 거친 문서를 하류가 구별할 수 없었다.
- **얇은 리더 위트니스에 전체 중단 말고 다른 출구를 주었다** — 격리 후 same-round respawn. 독립 리플리케이트 팬아웃에서는 resume이 도구 수준에서 막혀 있어 기존 에스컬레이션 경로가 닫혀 있었다.
- **핀 커버리지를 측정 가능하게 만들고 실제로 올렸다.** 드라이버가 종료 코드만 비교하고 진단을 버려 FAIL 픽스처가 왜 실패했는지 아무도 검증하지 않았다. 진단 선언 형식을 신설하고, 핀을 영역·위치로 결속하고, 발화한 적 없는 규칙 하나를 삭제했다. **측정값은 두 지표로 나뉘며 2026-08-11, 이 릴리즈의 마지막 정정 커밋 직전 트리에서 두 독립 방법으로 재측정했다** — **변조 탐지 83/83**(배열 원소를 하나 지우면 해당 드라이버가 실패하는가), **픽스처 행사 53/83**(그 제거로 빨개진 것이 OK 픽스처만이 아니라 FAIL 픽스처이기도 한가). 하나로 뭉치면 말없이 관대한 쪽을 고르게 되므로 이름을 붙여 따로 적는다. **변조 탐지가 전수인 것은 항등적으로 참이다** — 요약 줄이 모든 배열 크기를 나르고 그 줄이 OK 픽스처에 바이트로 핀돼 있어, 계수되는 원소는 추가되는 즉시 이 지표에 덮인다. 모집단 83은 두 스위트의 배열 원소 합(51 + 32)이고, 계수 규약은 블록 스코프 배열 원소다 — 배열이 아닌 스칼라 핀 하나(참조 파일의 파일 끝 센티널)는 이 모집단 밖이고 arity 기전이 그것을 덮지 않는다. **세 번째 스위트에 대해서는 비율을 발행하지 않는다.** 모집단 규칙이 아직 합의되지 않았고, 이 레포에서 그 비율은 세 번 제안돼 세 번 무너졌다 — n분의 n은 계수가 아니라 «더 없다»는 주장이고, 매번 분모가 그 시점에 아직 발견되지 않은 생존자를 제외하고 있었다. 대신 계수와 상태를 적는다: **사이드카 쪽에는 이 릴리즈의 측정으로서 어떤 계수도 공표하지 않는다.** 아래에 수가 나오지만 그것은 **인용**이다 — 서로 어긋나는 두 유도가 각각 무엇을 냈는지 보이려고 그 둘을 나란히 적는 것이고, 어느 쪽도 이 문서가 발행하는 값이 아니다. 앞선 배너는 「어떤 계수도 공표하지 않는다」로 끝나 자기 문단에 대해 거짓이었고, 뒤따르는 넷 중 셋이 한글 낱말로 적혀 숫자 감사에도 걸리지 않았다 — 그 셋을 아라비아 숫자로 바꿔 감사가 볼 수 있게 했다. 앞선 문면은 16 멤버 모집단에 대해 한 오라클에서 사살 16 · 생존 0을 적었는데, 두 번째 독립 열거 — 구조적 술어 지점을 훑는 방식 — 은 33 멤버에 생존자 11 이상을 낸다. 두 유도가 불일치하므로 이 불릿 자신의 규칙이 적용된다: **n분의 n은 계수가 아니라 「더 없다」는 주장**이고, 명명된 모집단에 대한 생존 0이 정확히 그 주장을 계수의 옷을 입혀 한 것이다. 모집단 규칙이 합의되기 전까지 이 축에는 수 대신 성질을 적는다 — **사이드카 스키마의 구조 핀은 오늘 두 오라클에서 서로 다른 모집단을 낳고, 어느 쪽이 옳은지는 아직 정해지지 않았다.** 오라클을 결과와 같은 문장에 적는 이유는 앞선 세 분모의 충돌 원인이 모집단이 아니라 이름 붙지 않은 오라클이었기 때문이다. **천장을 함께 적는다** — 이 지표들이 재는 것은 핀이 배열에서 사라졌을 때 CI가 빨개지는가이지 핀이 옳은 것을 가리키는가가 아니고, 이 릴리즈가 고친 결함 다수는 아홉 lint 어느 것도 읽지 않는 교차 파일 자연어 계약 텍스트에 있었다.
- **리뷰의 설계 정합 축을 여는 입력이 옵션 스키마에 노출된다.** 상류가 그 입력을 안내해도 문서화되지 않은 입력에 착지하고 있었다.
- **팀 세대 인덱스 복구가 원인이 아니라 원장 술어로 라우팅된다 — 잘라내기 금지.** 부분 존재로 도달하는 상태를 여섯으로 열거하고, 각 상태를 원장이 이미 나르는 두 컬럼(팀 스크래치 디렉토리 그룹 · 위트니스 논스의 라운드 태그)으로 구별한다. 판별자는 **기록된 값만 읽고 디스크를 조회하지 않는다** — 호스트가 살아 있는 팀의 임시 디렉토리를 수거하면 존재 검사가 뒤집혀 그 팀이 아래로 쓰는 arm으로 라우팅되므로, 수리 자체가 결함을 재도입하게 된다. **왜 뻔한 수리가 금지되는가**: 빠진 값을 「그냥 채우는」 백필은 현재 팀 행을 이전 세대로 떨어뜨려 `max(epoch)` 스코핑이 그 행들을 제외하게 만들고, 로스터가 다른 팀 행으로 통째 치환돼 타 역할 산출물이 이 단계의 산출물로 소비된다 — 게이트가 막으려던 절단이 그것을 고치려는 arm을 통해 재현된다. **앞선 판본의 백필 조건이 바로 그 재현을 허용했다**: 「에폭 없는 행이 전부 종결 상태이고 이 단계보다 이른 라운드에 있으며 스탬프된 형제와 스크래치 디렉토리를 공유하지 않음」은 두 패스 재스탬프의 **패스 사이** 상태에서 세 논항이 전부 참이 되는데, 그 상태의 팀은 살아 있다. 백필은 이제 **행이 스크래치 디렉토리도 위트니스 논스도 기록하지 않을 때**에만 선택된다 — 두 컬럼이 함께 제거되는 경로는 워크플로 정상 완료뿐이라 그 동시 부재가 은퇴한 행의 서명이고, 살아 있는 그룹은 대신 재스탬프로 간다. 아래로 세대 인덱스를 쓰는 arm은 여전히 이 하나뿐이며 표 전체의 안전성이 그 한 행의 판별자로 환원된다.
- **공개 블록의 반-공허 술어가 바뀌었다 — 잘라내기 금지.** `결손 수`의 상한이 `≤ 리뷰어 수`에서 **`< 리뷰어 수`로 엄격**해졌고, 총 결손(`결손 수 == 리뷰어 수`)은 선언할 수 있는 결손이 아니라 **감사가 일어나지 않은 상태**로서 블록을 쓰지 않고 중단·보고한다. **왜 뻔한 정리가 금지되는가**: 재척도된 하한 `원시 ≥ 리뷰어 수 − 결손 수`는 총 결손에서 `원시 ≥ 0`으로 무너지고, 그 상태의 `리뷰어 수 = 결손 수 = 3` · 나머지 0인 블록은 **네 검사를 전부 통과하면서 아무것도 감사되지 않았다고 진술한다**(손 평가로 확인). 이 절을 줄이면 그 상태가 다시 유효한 출구가 된다.
- **잔여 공개 블록이 14슬롯에서 15슬롯이 됐다.** 리더가 증언을 내지 못한 실행을 `리뷰어 수`를 낮춰 흡수하지 않고 **선언**하도록 결손 arm을 두었고, 그 arm이 쓰는 슬롯이 15번째다. 슬롯 수 정수 자체가 핀이라 arm의 조용한 제거는 개수 단언에 걸린다. 2.0.0 항목의 "고정 14슬롯"은 그 릴리즈 시점의 기록이며 이 줄이 그 뒤의 값을 잇는다.

### Removed

- **발화한 적 없는 리더 수 규칙.** 현실적 표기 6종 중 1종만 잡고 어떤 픽스처에도 매치된 적이 없다. 넓혀도 양성 증거가 0인 규칙은 여전히 아무도 동작을 보일 수 없으며, 이 릴리즈가 고치는 결함 클래스가 정확히 *"거기 있지만 아무것도 보장하지 않는 핀"*이다 — 같은 릴리즈에서 그 클래스의 인스턴스를 하나 더 남기는 것은 일관되지 않는다. 사용자 대면 표면은 아니나 lint 규칙의 제거는 기록되어야 한다.

### Why

`2.0.0`이 사이드카 공유 계약을 착지시키면서 이탈 기록 스키마를 그 위에 올렸는데, 그 스키마는 설계 선례 한 문장으로 출하됐고 리뷰 패스를 거치지 않았다. 이번 릴리즈는 그 결과를 정리한 것이며, 리뷰가 낸 것과 스키마를 다시 설계 테이블에 올려 나온 것을 함께 담는다.

**원인은 하나가 아니라 둘이고, 두 번째가 위 문단에 빠져 있었다.** 같은 릴리즈가 감사 파이프라인 쪽 항목도 담는다 — 정합 패스 완료 스탬프, 얇은 리더 위트니스의 격리 출구, 팀 세대 인덱스 재스탬프, 정리 arm 분리. 이들의 공통 원인은 사이드카 스키마가 아니라 **2.0.0이 신설한 감사 파이프라인이 리뷰를 거치기 전에 출하됐다**는 것이고, 두 원인을 한 릴리즈에 묶은 것은 둘 다 같은 커밋 범위의 파일들을 열기 때문이다. 한 원인만 적으면 나머지 절반의 항목이 어디서 왔는지 이 문서만으로는 복원되지 않는다.

**patch인 이유.** 사이드카 버전 범프는 호환 파괴가 아니라 **진단을 옳게 만드는 기전**이다. 수정 전에는 도달 가능한 인스턴스가 없었고(유일한 리더가 유일한 라이터가 쓰지 않는 경로를 봤다), 리더 측 경로가 고쳐져 도달 가능해지면 명명된 사유와 함께 스킵된다. 새 동작을 요구하는 항목 하나 — 구속 티어 승인 프롬프트 신설 — 은 의도적으로 이번 스코프 밖에 두었다. **새 durable 표면은 스코프 밖이 아니라 이번 릴리즈가 출하한다**: 정합 패스 완료 스탬프가 그것이고, 앞선 문면은 그것을 미출하로 적어 등급의 근거 문단 안에 검사 가능한 거짓 진술을 남겼다. 등급 자체는 그 사실 위에서도 서므로 바뀌지 않는다 — 스탬프는 하류가 읽는 기계 판독 표면이지 사용자에게 새 동작을 요구하는 것이 아니다. **커밋 종류로도 등급을 세우지 않는다**: 이 범위에는 `fix`와 `test` 외에 `refactor`·`ci`·`chore`가 있고, 「모든 커밋이 `fix`/`test`」는 이 브랜치 자신의 이력으로 반증된다.

### Post-install notes

- **기존 이탈 기록 사이드카는 읽히지 않는다.** 버전이 올라갔으므로 디스크의 v1 파일은 명명된 사유와 함께 스킵되며, 삭제되지도 재작성되지도 않는다. **그 파일이 이미 필드를 잃었다면 그 손실은 복구되지 않는다** — 블록은 append 즉시 동결되고 rewrite 형식이 없다. 옮길 값이 있으면 손으로 판단해 새 파일에 전사할 것.
- **이 릴리즈 이전에 동결된 설계 문서는 매 감사마다 「정합 패스 스탬프 없음」 발견을 받는다.** 스탬프는 이 릴리즈가 신설했으므로 그 이전 문서에는 존재할 수 없고, 감사는 부재를 발견으로 보고한다. **정합 패스가 아예 없는 파이프라인의 문서도 같다** — 문서의 바이트는 「패스가 없는 파이프라인」과 「패스를 건너뛴 파이프라인」을 구별하지 않으므로 원인을 판정할 수 없고, 감사는 그렇게 적는다. **조치**: 그 발견은 알려진 잔여이며 `## 미해결 이슈`로 라우팅된다(`기각`으로 보내면 기각 수에 영구 바닥이 깔려 triage 붕괴 탐지기가 자기 경보에 도달할 수 없게 된다). 문서를 다시 동결할 계획이 있으면 정합 패스를 한 번 태워 스탬프를 얻는 것으로 사라지고, 그럴 계획이 없으면 매 실행 반복되는 발견으로 남는다. 앞 항목의 v1 사이드카와 같은 부류의 안내다.
- **감사 보고서의 마지막 줄이 종단자가 된다.** 보고서 쓰기가 원자적 whole-file 쓰기로 바뀌었고 파일 종단자를 선언한다. 이 종단자를 강제하는 구조 핀은 공유 계약 파일만 스캔하므로 이 인스턴스에는 닿지 않는다 — 녹색 핀이 커버리지를 함의하지 않는다.
- **파이프라인 안내 문구에서 `다음 단계` 표현이 빠졌다.** 커맨드 이름과 하는 일은 그대로이며, 순서를 단정하지 않도록 표현만 바꿨다.
- **정리 호출부가 어느 arm을 취하는지 명시한다.** 진행 중 페이즈 경계에서는 위트니스 디렉터리와 행별 임시 필드가 **보존**된다. 판단이 서지 않으면 보존하는 쪽을 취한다.
...
- **파리티 표면 6경로** — `scripts/lint-review-prompt-parity.sh`(358줄), 그 테스트 드라이버, 픽스처 20 디렉터리(87파일), `Makefile` 2줄. **삭제된 픽스처 디렉터리는 이 개편이 직접 들어낸 파리티 표면에서 20개(87파일), 살아남은 펜스 두 루트(`lint-skill-invariants`·`lint-verification-literals`)에서 함께 빠진 것이 13개(25파일), 합계 33개(112파일)다.** 앞의 둘은 소계이고 33은 총계다 — 20과 13을 더한 값이며, 이 문장에서 총계인 것은 그 하나뿐이다. **두 루트가 잃은 것은 파일이지 디렉터리도 검사 케이스도 아니다**: 3과 10은 **삭제된 파일을 담고 있던 디렉터리의 수**이고, 그 디렉터리들은 그대로 있으며 케이스 수도 각각 3→3, 10→10으로 순 손실이 **0**이다. 「디렉터리를 잃었다」는 앞선 명사가 그 순 손실 0을 손실로 읽히게 했고, **값이 맞아서 재측정으로는 드러나지 않는 오류**다. 단위를 적지 않은 채 「소계와 나머지」로 쓰면 두 수가 같은 것을 센다고 읽히고, 그러면 살아 있는 펜스가 입력을 잃은 것처럼 보인다 — 그것이 이 줄이 막는 오독이며, 처음 적힌 근거와는 다르다. **숫자를 다시 유도하려는 사람에게 — 범위를 여기 적는다**: `6c47780..ca9a178`, 즉 v1.21.1 릴리스 커밋부터 v2.0.0 릴리스 커밋까지이며 두 점 표기가 왼쪽 끝을 제외한다. 이 범위에서 위 여섯 값이 전부 재현된다. 「갈라져 나온 지점의 첫 부모」만으로는 어느 릴리즈인지가 정해지지 않아 검토자 셋이 서로 다른 기준을 골랐다 — 차이는 계수 규약이 아니라 기준 커밋이고, 그것은 끝점을 적어야만 끝난다. 이 lint는 대상 부재 시 조용히 통과하므로 남겨두어도 CI는 초록이었다 — 유지보수 비용을 근거로 삼은 개편에서 조용히 살아남기 좋은 형태였다.
...
- **FE 파이프라인의 다음 단계 안내 문구가 바뀐다.** `design-prompt`·`design-apply`가 안내하던 검증 커맨드가 `design-audit`으로 바뀌었다. 기존 `--base` 호출 형태는 그대로 유지된다. **`--changes`는 후속 없이 제거됐다** — 편집된 문서를 다시 검증하려면 새 해시에 대한 전체 감사를 돌리는 것이 설계상 정답이며, 그것이 동결 논지가 겨냥한 자기 재리뷰 형태를 없앤 이유다.
