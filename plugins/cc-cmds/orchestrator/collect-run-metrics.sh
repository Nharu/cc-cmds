...
#                             없는 그 런은 다음 회차의 모집단에 그대로 다시 든다. 회차는 게이트가
#                             띄운 분리 자식에서 돌고 게이트가 이 수집기 전체에 한 시간 기한을
#                             건다 — 이 상한은 그 기한과 별개로, 한 회차가 새 수집에 쓰는 시간을
#                             나누어 적체를 여러 회차에 걸쳐 전진시키려는 것이다. 회차의 첫 새 수집은
