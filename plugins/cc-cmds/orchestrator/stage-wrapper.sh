...
SETTINGS=""; PLUGIN_DIR=""; SESSION_ID=""; MODE="A"; FIFO=""
while [ $# -gt 0 ]; do
  case "$1" in
...
[ -n "$SESSION_ID" ]  || { printf 'stage-wrapper: --session-id 는 필수입니다 — 트랜스크립트가 진행 오라클이고, 호출자가 고른 id 없이는 찾을 방법이 없습니다\n' >&2; exit 2; }
