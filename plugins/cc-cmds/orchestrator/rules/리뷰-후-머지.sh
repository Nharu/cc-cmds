...
# 세그먼트가 없으면 통과한다. 이 룰이 재는 것은 리뷰 의무와 리뷰 기록이고
# 둘 다 세그먼트 스코프이므로, 세그먼트가 없는 행위에는 판정할 대상 자체가
# 없다. 발동 칸을 머지 이상으로 넓히면서 이 자리가 필요해졌다 — 배포로 신고된
# 평범한 행위(디렉터리 생성·파일 읽기)가 사다리 위칸에 있다는 이유만으로 이
# 룰에 들어오기 때문이다. 세그먼트 없는 머지를 세우는 것은 이 룰이 아니라
# 발행 전 앵커 검사다.
