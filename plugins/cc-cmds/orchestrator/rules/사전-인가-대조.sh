...
# 저장소 대조 — 형태는 명령을 적지 저장소를 적지 않는다. 그래서 `gh pr` 행은
# 어느 저장소를 향한 `gh pr …` 이든 똑같이 맞았고, 인가는 이 런의 저장소에 대해
# 받은 것인데 행위는 남의 저장소를 향할 수 있었다. 게이트가 해소한 저장소를
# 따로 실어 주면 그것만 따로 대조한다.
#
# 실리지 않은 것은 「저장소를 적지 않았다」이고, 그때 저장소는 워크트리의
# 원격이라 대상 행이 이미 고정한다. 실렸는데 대상과 다르면 — 대상 값이 비어
# 있는 경우를 포함해 — 목록 밖이다.
if [ -n "${GATE_ARGV_REPO:-}" ] && [ "$GATE_ARGV_REPO" != "${GATE_TARGET_REPO:-}" ]; then
  [ "$probe" = "1" ] && { echo "P=0 형태=없음 Pd=0"; exit 0; }
  echo "사전-인가-대조: 행위가 가리키는 저장소 '$GATE_ARGV_REPO' 가 대상의 저장소가 아닙니다 — 승인 대기를 발행합니다" >&2
  exit 5
fi
