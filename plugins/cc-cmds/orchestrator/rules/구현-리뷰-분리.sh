...
[ -f "$GATE_LEDGER" ] || { echo "구현-리뷰-분리: cannot read the ledger" >&2; exit 1; }
...
for v in "$impl_sid" "$impl_par" "$rev_sid" "$rev_par"; do
  [ -n "$v" ] && [ "$v" != "미상" ] || {
    echo "구현-리뷰-분리: session lineage is not recorded, so separation cannot be judged — undecidable is not a pass" >&2
    exit 1
  }
done
...
for a in "$impl_sid" "$impl_par"; do
  ...
  for b in "$rev_sid" "$rev_par"; do
    ...
    if [ "$a" = "$b" ]; then
      echo "구현-리뷰-분리: the review session and the implement session share an ancestor ($a) — this is a review of its own work" >&2
      exit 1
    fi
  done
done
