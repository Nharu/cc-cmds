...
- **Early ledger stub**: before (or at) the first spawn, create `docs/{topic-slug}.md` as an early stub = an H1 title plus the durable ledger HTML-comment block (`<!-- cc-design-ledger v1 … -->`) placed right after the H1, before the first `##`. Record each member's returned `agentId` in the ledger immediately. Entry schema (per `_common/agent-team-protocol.md`): `agentId | state | round | role/scope | thinReturns | last-return summary`, `state ∈ {running, done, aborted}`. Update on every state change; **re-read the ledger from disk before any resume**; if the block is missing or unparseable, **fail closed via `AskUserQuestion`** (never silent-skip).
