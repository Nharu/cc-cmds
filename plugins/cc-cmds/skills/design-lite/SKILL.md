...
**Convergence has no step of its own.** From Round 2 on it rides that round's **closing declaration** (`_common/agent-team-protocol.md`'s `## Convergence`) — the Round-2 resume above already re-injects the consensus and the open conflicts, so it also requires the witness to end with either `no further input` or a named measurement the member has not yet run. Proceed to Step 4 when **every member's round witness is `witness_present` and its declaration reads `no further input`**; the dispatch-completeness gate still runs before collecting.

A third round opens only through the entry paths of `### Round budget` — a declaration naming an unrun, actionable measurement, or the lead's pre-save sweep returning a refuting verdict — and that section's ceiling is binding, so after it the team MUST converge regardless. Each round is at most one resume per member; the resume is the unit of lite's predictable cost (a resume-budget soft-cap sitting under the shared cap).
...
- **Lead pre-save sweep (lite)**: before the first save, the lead sweeps the synthesis draft (its 2 budget units) against the four pass conditions of `_common/verification.md` §10 (the shared sweep predicate) — do NOT restate them here; that section is the single definition and this skill cites it. **Sweep-failure path**: a refutation verdict from the sweep is the second entry path of `### Round budget` → route to Round 3 (blocker-only, within that section's ceiling) — no new machinery; the base "failures are never lead-solo" principle is preserved. If Round 3 is already spent, fall back to `AskUserQuestion` with 3 options (accept residual with refutation evidence / redirect to `/cc-cmds:design` / abort).
