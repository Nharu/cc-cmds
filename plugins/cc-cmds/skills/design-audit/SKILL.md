...
These six **assignment lines** are the only place any of these values is *written as a value* in this skill — the scoping is the accurate claim, and the unscoped one is refutable: the bare numerals also occur in ordinary prose, so "the only place any of these values appears" is false as written and a reader who checks it stops trusting the rest of the block. Every other mention — prose here, `references/*.md`, the spawn prompt, the disclosure block — refers to them **by name**. A hook reads `READER_COUNT` from this block directly and never from a spawn prompt, because a copy drifts.
...
The freeze window opens **immediately after the `## 결정론적 검사` write of Step 2 completes** — after, not at, because the window's own rule is that the tree does not change inside it and that write changes the tree; read as opening *at* the write, the write that opens the window violates it, and Step 2's baseline capture, which is taken after that write, would sit outside the window it is the baseline for — the last in-tree write this skill makes before the first reader spawns — and closes when the last reader witness is collected. **The opening point is named once, here, on purpose**: it was previously named twice, as *the last pre-spawn write of Step 1* in this paragraph and again at the write itself, and those are different steps, so a reader had to guess which baseline capture was the authoritative one. That write is the right anchor because it is the last thing that changes the tree before any reader sees it. Inside it, no `Edit`/`Write` may target the document and nothing may change in the tree. **Two** artifacts are recorded at the open — the **capture format** baseline (`${CLAUDE_SKILL_DIR}/../_common/verification.md` §6) and the `git worktree list --porcelain` baseline, together the two-command boundary gate of that file. `FROZEN_SHA256` is recorded earlier, at Step 1, and is not an artifact of this window; **all three are re-checked at the close.** The count said three because the two captures used to sit beside the hash in Step 1; moving them here to make the opening point single-named left the sentence counting a set it no longer describes.
...
Exactly `READER_COUNT` readers. **The permitted dispatches are a closed enumeration of two, and anything not on it is not one**: (1) the **initial spawn** of each reader, once each; (2) a **same-round respawn** of a reader whose witness came back thin or never arrived, per Step 4's Case-1 recovery — same round, byte-identical prompt, the recorded nonce reused. Each reader performs `ROUNDS_PER_READER` round, and a respawn does not advance it, which is what keeps (2) from being a second round under another name. **A resume is not on the list**, and it is not merely forbidden but tool-unavailable: `SendMessage` is deliberately not loaded in Step 0. Closing the enumeration is the load-bearing part — an open *"no resumes"* prohibition leaves every other dispatch shape unclassified, and the Case-1 recovery is precisely the shape that would otherwise have to be argued about at the moment it is needed, under pressure, by a lead holding a thin witness.

**Declared carve-out**: the shared team protocol's multi-round rule ("drive at least 2 rounds; a one-shot isolated `Agent()` per round is forbidden") is **inapplicable here, and this sentence is the explicit exception.** Cross-review IS the reproduction channel this command removes — a reader that has seen a peer's finding is no longer an independent replicate. **No reader is ever resumed**; `SendMessage` is deliberately not loaded in Step 0, so a resume is not merely forbidden but tool-unavailable. The protocol's same-round respawn on a death verdict is the only re-spawn permitted; it reuses `ROUND_TOKEN`/`PASS_TOKEN` unchanged and is bounded by the protocol's durable escalation counters, every one of which terminates in an `AskUserQuestion` rather than in another autonomous spawn.
...
None of the following may appear anywhere under this skill: `consecutive_no_major`, `COUNT_APPLIED`, `escalate_applied`, `INNER_EXIT_REASON`, `inner_round`, `outer_iter`, `outer_log.md`, `ack_items.md`, `pending_applies.md`, `INNER_TEMP_DIR`, `convergence_table.md`, a severity max-aggregation ratchet, and any auto-approve or auto-reject predicate used as a **termination input**. This command still triages, but no counter anywhere depends on the triage outcome, so the incentive gradient that collapsed the predecessor's filter does not exist.
...
4. **FREEZE (hash only — the git baselines are NOT captured here).** Record `FROZEN_SHA256` (`shasum -a 256`). The two boundary baselines are captured at the Step-2 `## 결정론적 검사` write, which CFI-1 names as the one opening point; capturing them here as well would put the naming back in two places, and the two places are different steps with in-tree writes between them — including this one. **The exemption that made a duplicate capture harmless is gone**: the shared contract no longer says a capture may sit on either side of an exempt write, so a second capture is now a contradiction of this skill's own invariant rather than a redundancy it blesses. Following the one order also puts all three of this skill's exempt writes outside the window, which is what leaves it owing zero bracket operations. Create `<base>/docs/design-audit/` and early-stub the report at `<base>/docs/design-audit/{slug}.md`: an H1 title, the machine header `<!-- cc-design-audit v1; writer=design-audit; reader=design-audit; owner-doc=<document key>; NOT a design doc; mechanism-local, never staged by a skill -->`, the `<!-- cc-design-ledger v3 … -->` block (this stub is the ledger's home, so no fallback location is ever needed), and a `## 감사 개시` block carrying `frozen-sha256`, `frozen-at`, `reader-count`, `round-token`, `pass-token`. The `frozen-sha256` recorded here is never rewritten — it is the second hash that anti-vacuity check (i) compares against. **The report's last non-empty line is the fixed terminator `<!-- cc-design-audit: end -->`**, emitted by this write and by every later write to the report; a report whose last non-empty line is not that sentinel is truncated, and a writer fails closed on it rather than extending it. **Every write to this report is a Tier-1 atomic write** per the shared sidecar contract — same-directory `mktemp` plus a whole-file `mv`, plain `mv` and never `mv -n`, never truncating, never `git add`. `Write`/`Edit` do not satisfy Tier-1. This skill is its own writer and reader, so it owes Tier-1 and not Tier-2; that is the whole of what it owes.
...
Apply the protocol's `witness_present` completion predicate and its reconcile ladder verbatim. Flip each row to `done` with a last-return derived **from the witness content**. No reader is resumed for any reason (CFI-2).

**Case 1 is the one escalation this fan-out does NOT take verbatim, and the exit is stated here.** The protocol's Case-1 recovery is a resume, and the independent-replicate carve-out this skill claims is exactly what makes a resume tool-unavailable — so "apply it unchanged" would name a closed route and leave a full abort as the only way out of a thin witness. Instead:

1. **Quarantine the thin witness.** `mv` it to a `quarantine/` subdirectory of the same `scratchDir`. That path is outside the witness key namespace, so it can never satisfy `witness_present` and a compaction-recovered re-derivation cannot read it back as this reader's product. Relocating is neither an overwrite nor an authorship, and the protocol says so affirmatively.
2. **Same-round respawn** into the now-vacant canonical path, byte-identical prompt, recorded nonce reused per the mint-vs-reuse predicate. **Never a re-scoped prompt**: re-scoping changes the lens, and a re-lensed reader is no longer a replicate, which silently destroys the reinforcement multiplicity Step 5 derives from agreement.
3. **If the respawn also returns thin**, that is a declared shortfall, not a silent one: report it through the `결손 수` slot and its adjacent reason line rather than lowering `리뷰어 수` to match.

**The quarantine step is what makes the respawn work at all.** `mv -n` returns exit 0 on an occupied path, so a producer respawned onto a still-occupied canonical path cannot detect that it accomplished nothing, and its temp is left orphaned on disk. Vacating the path first means the canonical location always holds exactly one file, so a naive post-compaction re-derivation reads the right one.
