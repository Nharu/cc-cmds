...
- **Witness scratch dir (parameters for the protocol)**: before the first spawn, create it by the protocol's two-branch form under **Spawn** with slug `{slug}` — `WITNESS_DIR="${CC_PIPELINE_RUN_DIR}/team-witness-{slug}"` when `CC_PIPELINE_RUN_DIR` is set, otherwise `WITNESS_DIR=$(mktemp -d "${XDG_STATE_HOME:-$HOME/.local/state}/cc-cmds/design/team-witness-{slug}.XXXXXX")` — recorded as each reader's `scratchDir`. The witnessed phase is `PASS_TOKEN` (colon-free, as the protocol requires) and the witness path is `${WITNESS_DIR}/reader-<k>.fanout.md`. The witness dir is out-of-tree, leaving the boundary gate untouched.
