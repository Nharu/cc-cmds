...
These six lines are the **only** place any of these values appears in this skill. Every other mention — prose here, `references/*.md`, the spawn prompt, the disclosure block — refers to them **by name**. `scripts/lint-design-audit-pins.sh` and `scripts/lint-team-budget-pins.sh` read these values from this block directly and never from a spawn prompt, because a copy drifts.
...
The freeze window opens at the **last pre-spawn write**, which is Step 2's `## 결정론적 검사` write into the report, and closes when the last reader witness is collected. **There is exactly one opening point and Step 2 names it**: an earlier form of this sentence put the open at the last write of Step 1, which left Step 2's own report write inside the window under one reading and outside it under the other, with no rule saying which. Inside the window, no `Edit`/`Write` may target the document and nothing may change in the tree. `FROZEN_SHA256` is recorded at Step 1's FREEZE and the two boundary baselines at the open — the two-command boundary gate of `${CLAUDE_SKILL_DIR}/../_common/verification.md` — and all three are re-checked at the close.

**The worktree half is the gate's three scoped assertions, not a whole-output equality** — `2a`, `2b`, `2c`, and assertion `1` within the surface it declares, all exactly as `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6 defines them. Do not re-derive any of them here, and do not gloss them: that divergence is what made this gate mean three different things in three skills, and a gloss that reads as correct is the one that survives an edit after the definition beneath it has changed.
...
3. **Read `${CLAUDE_SKILL_DIR}/../_common/sidecar.md` `## 1`** and derive, from the **document's own directory** and never the cwd: `CODE_ROOT` (`git rev-parse --show-toplevel`, falling back to the document's directory), the **document key**, and `{slug}`. **Record `CODE_ROOT` and the cwd the boundary observations will be taken in as a pair, in the report.** That pair is **one input** to the caller-checks-it duty of `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6.0 and not its discharge: Step 5's first sub-step discharges the duty by running the four checks on the derivation itself, and the recorded pair is the diagnostic that lets a morning reader see why a whole-tree surface came about.
4. **FREEZE.** Record `FROZEN_SHA256` (`shasum -a 256`) and, **from that same read**, `FROZEN_BYTES` (`wc -c`). The two are one pair of observations of one set of bytes and no write may fall between them — a size measured at any later point is a different measurement, because Step 6's reconciliation pass edits the document and the frozen size then survives nowhere on disk. The two boundary baselines are **not** captured here — Step 2 captures them after its report write, which is the single window opening CFI-1 names. Create `<base>/docs/design-audit/` and early-stub the report at `<base>/docs/design-audit/{slug}.md`: an H1 title, the machine header `<!-- cc-design-audit v1; writer=design-audit; reader=design-audit; owner-doc=<document key>; NOT a design doc; mechanism-local, never staged by a skill -->`, the `<!-- cc-design-ledger v3 … -->` block (this stub is the ledger's home, so no fallback location is ever needed), and a `## 감사 개시` block carrying `frozen-sha256`, `frozen-bytes`, `frozen-at`, `reader-count`, `reader-model`, `round-token`, `pass-token`. The `frozen-sha256` and `frozen-bytes` recorded here are never rewritten — the hash is the second hash that anti-vacuity check (i) compares against, and the byte count is the only surviving record of the frozen size, which Step 7 copies into the disclosure block instead of re-measuring.
...
**Three custody duties, and they are the arm's rather than the contract's.** The contract delegates each of them explicitly, so an arm that leaves one unstated has an unchecked field where a record should be:

1. **Where the two boundary baselines live** — persist them out-of-tree at `${WITNESS_DIR}/baseline.status` and `${WITNESS_DIR}/baseline.worktree` rather than holding them in context across the whole fan-out. An in-tree record changes the very `git status` assertion 1 states, so it has no fixed point; and a halt record's baseline value becomes re-derivable from a file rather than from a context window that no longer exists.
...
**Before spawning, Read `${CLAUDE_SKILL_DIR}/../_common/agent-team-protocol.md`** for the spawn / witness / ledger v3 / reconcile-ladder contract and the task-assignment header, **Read `${CLAUDE_SKILL_DIR}/../_common/team-model-tier.md`** for the reader model, and **Read `${CLAUDE_SKILL_DIR}/references/01-reader-prompt.md`** for the reader prompt.

Readers are **nameless background tasks** (`Agent` with `subagent_type:"claude"`, `run_in_background:true`, **no `name`**), self-terminating on return; each reader's result is delivered by its **witness file** and the return text is only an early-wake hint. The `Agent()` call passes the model on the tier file's `audit-reader` row, and every reader of one audit gets the same one. Stating it beats inheriting it: a later change to how stages are launched would otherwise move the readers with it, and the record would say only "whatever the session was".

- **Witness scratch dir (parameters for the protocol)**: before the first spawn, run the protocol's `## Spawn` command — `<plugin root>/orchestrator/cc-team-witness-init.sh {slug}` (no `bash` in front — see the protocol's note) — and record the **printed path, literally** (not `$WITNESS_DIR`) as each reader's `scratchDir`. The witnessed phase is `PASS_TOKEN` (colon-free, as the protocol requires) and the witness path is `${scratchDir}/reader-<k>.fanout.md`. The witness dir is out-of-tree, leaving the boundary gate untouched.
...
- **MUST — byte-identical prompts, and one model.** The `READER_COUNT` rendered prompts differ **only** in `{WITNESS_PATH}`, `{WITNESS_NONCE}`, and `{role-slug}` — and the `model` is the same for all of them. Mixing models across the readers of one audit breaks the same statistic lens differentiation breaks, for the same reason: the count means something only while the readers are interchangeable. Differentiating readers by lens silently destroys the reinforcement statistic the `미보강 잔여 수` slot reports: with disjoint lenses every finding is trivially unreinforced and the slot becomes noise.
...
1. **Run the four checks on the derivation, record the surface — and only then take the closing observation.** First act of this step, in this order: (1) apply the parsing predicate of `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6.0 — the anchor comparison table's heading present and the table's header row following it — to every collected witness; (2) derive assertion 1's measurement surface from the collected witnesses and the report's `## 결정론적 검사` section; (3) apply that same file's §6.0 totality predicate to those sources, reading the predicate from there rather than restating it here — a restatement is a parity obligation, and a partial one ships a predicate that appears in no document at all; (4) check whether the derived set is empty. If (1), (3) or (4) fails, assertion 1's surface is the whole tree, and **which of them fired is written into the report** — the report says which condition, not merely that a fallback happened. Then **write the surface into the report**, and record beside it, as a diagnostic line, the `CODE_ROOT` / observation-cwd pair Step 1 took. The closing observation is taken after that write and not before. Every witness is sealed at the end of Step 4, so this order is satisfiable; requiring the surface to be *recorded* is what makes the order observable in the artifact instead of merely asserted.
2. `등급 2` **Re-hash and re-compare both baselines** — the worktree half as the gate's **2a/2b/2c**, and assertion 1 taken within the measurement surface item 1 recorded (the derived set, or the whole tree when item 1's fallback fired). A mismatch is fail-closed: report in Korean **which** assertion diverged, the baseline value, the observed value, and — on an assertion 1 mismatch — **the intersecting paths**, since that line is the only thing telling a later reader what ended the window; then `AskUserQuestion` with `재감사(새 동결로 재실행) ← 추천` — which emits the exact re-invocation command line and **stops** — or `중단`. There is deliberately no "continue anyway" option: that is the seam through which a positive induced-defect rate re-enters.

    **On a 2b divergence the report names which component moved** — `worktree`, `branch` or `HEAD-tree` — because a subset is now compared and "the block is the value" no longer holds. On a `HEAD-tree` divergence it also carries the two **tree** shas as the compared baseline/observed pair, the changed path count with up to five names from `git diff --name-only <baseline tree> <observed tree>`, and the two `HEAD` commit shas as reported-never-compared diagnostics. The path list rather than a commit range is deliberate: a tree-to-tree diff needs only the two tree objects 2b already holds, while a range needs every commit in it to still be reachable — and the remedy this repository applies to a polluted local branch is a hard reset, which is exactly what makes a baseline commit unreachable.

    **On a PASSING run where 2b's `HEAD` moved but its tree did not, write one line into the report** naming both commit shas (12 hex is enough) and stating that the tree was unchanged, so the window held. Without it this narrowing makes that class invisible: there is nothing to report on a passing run, and the only symptom the class ever had was the stop this change removes.
3. **Dedup into unique defects** (identity = same anchor or section AND same root cause) and record each defect's **reinforcement multiplicity** — how many readers independently raised it. Merge the Step-2 deterministic findings in: they count toward `원시` and `고유` and are **excluded from `미보강`** by definition, since reinforcement is not a meaningful notion for a saturated channel. That exclusion is what keeps the `원시 ≥ 고유 ≥ 미보강` invariant sound.
4. **Copy each collected reader report in-tree** to `<base>/docs/design-audit/{slug}.reader-<k>.md`, each carrying the same `owner-doc=` machine header, **after** the freeze verdict. These are the durable artifacts anti-vacuity check (ii) counts; the out-of-tree witness stays where the protocol put it.
