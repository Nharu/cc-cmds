...
**The worktree half is the gate's three scoped assertions, not a whole-output equality** — 2a (path set), 2b (this audit's own entry), 2c (`cc-design-exp-` count is 0), exactly as `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6 defines them. **Assertion 1 is scoped too**: its measurement surface is derived from the reader witnesses' anchor comparison tables and `path:line` citations plus the paths the deterministic checks cited, so a `git status --porcelain` change touching no path a reader **cited** is not a mismatch, unless the four checks put the surface at the whole tree. Cited is narrower than read — a file a reader opened without quoting it, or one a tree-wide search swept for a cited symbol, contributes no path — and this scoping claims only the narrower set, which is what the derivation yields. **In measured runs the whole-tree fallback is the standing case and the narrowing is the exception** — it fired in 0 of the 15 complete runs in this repository's audit corpus, because every one of them carried at least one token that forces it, so plan for whole-tree equality and treat a narrowed surface as a windfall rather than the default. Do not re-derive any of them here; that divergence is what made this gate mean three different things in three skills.
