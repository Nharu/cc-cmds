...
- **Convergence (witness collection, no dispatch of its own)**: convergence is **not** a separate resume. From Round 2 on it rides that round's **closing declaration** (`_common/agent-team-protocol.md`'s `## Convergence`) — the round's own resume already re-injects the current consensus and open conflicts verbatim, and requires the witness to end with either `no further input` or a named measurement the reviewer has not yet run. A reviewer's round is converged-and-collected only when its round witness is `witness_present` and carries that declaration; the dispatch-completeness gate still runs before collecting. Round count follows `### Round budget` (Round 1 = produce; Round 2 = cross-review with peer findings quoted verbatim; Round 3 only on that section's entry paths). This is a **full** review — what separates it from `review-lite` is roster breadth and review scope, not a larger round allowance.
