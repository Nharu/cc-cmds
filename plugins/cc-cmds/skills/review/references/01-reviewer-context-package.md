...
17. **(Conditional) Design document + drift sidecar** — include only when the user supplied a design-document path as a review directive; **omit entirely otherwise**, and omit `design-conformance` from item 14 with it. Absence is the normal case, not a defect: a design document is user-local and typically untracked, so it does not exist in a clone. The channel this item opens therefore works when reviewing **your own** work and is structurally unavailable when reviewing someone else's PR — do not construct a fallback that guesses at a document.
    ...
    - **The drift sidecar** `<base>/docs/design-drift/{design-doc-slug}.md`, when present, records divergences the implementer already declared. **Derive that path from the design document, never from this review's `{slug}`** — that token is bound to the review slug elsewhere in this skill, so naming it here would address a file that cannot exist and make its absence look like a finding. Derive it explicitly: `cd` to the design document's own directory, take `pwd -P`, run `git rev-parse --show-toplevel` there to obtain `<base>` and the **document key**, and fold that key (`.md` stripped, every `/` replaced by `-`) into `{design-doc-slug}`. `<base>` is needed independently of the slug because a sidecar is anchored under `<base>/docs/<kind>/` and this item's own premise is that the design document is user-local and typically untracked, so outside the reviewed repo is an ordinary case rather than an edge one.
