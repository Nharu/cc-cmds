...
3. **Round 2 — Cross-validation**: Resume each reviewer by its `agentId`, re-injecting the other reviewers' findings verbatim. Explicitly request: validate severity assessments, identify missed issues in overlapping areas, flag false positives, and note findings that interact with their own. The reviewer publishes its cross-validation pass as its witness. This resume publishes a witness, so it is a round — step number and round number agree here by the counting rule in `### Round budget`. Require that round's **closing declaration** in the witness: either `no further input`, or a named measurement the reviewer has not yet run that would settle a contested peer claim.

4. **Round 3 — Refinement (as needed)**: Resume original authors with cross-validation feedback (quoted verbatim). Request severity revision, missed issue additions, and challenge responses. Each round's result is the reviewer's witness, confirmed via `witness_present`. Repeat as needed **within the ceiling of `_common/agent-team-protocol.md`'s `### Round budget`** — the entry condition here stays `as needed`, because that section's narrowed third-round trigger is written for discussion rounds and a refinement round reflecting cross-validation feedback would never satisfy it. Gating this round on the narrowed trigger would make refinement vanish for want of a qualifying measurement and let the review converge on feedback it never reflected. This round carries the same closing declaration.

**Convergence has no step of its own.** From Round 2 on it rides each round's closing declaration (the **Convergence** section of `_common/agent-team-protocol.md`): the review has converged — and Step 5 begins — when every reviewer's current-round witness is `witness_present` and its declaration reads `no further input`. The dispatch-completeness gate still runs before collecting, so a reviewer dropped from the round's own fan-out is not read as converged.
