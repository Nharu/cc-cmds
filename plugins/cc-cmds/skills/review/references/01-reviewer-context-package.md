...
17. **(Conditional) Design document + drift sidecar** — include only when the user supplied a design-document path as a review directive; **omit entirely otherwise**, and omit `design-conformance` from item 14 with it. Absence is the normal case, not a defect: a design document is user-local and typically untracked, so it does not exist in a clone. The channel this item opens therefore works when reviewing **your own** work and is structurally unavailable when reviewing someone else's PR — do not construct a fallback that guesses at a document.
    - **What binds.** The **binding tier** is the conformance baseline: `## 합의된 아키텍처`, the decision sentences of `## 주요 결정사항과 근거`, entries of `## 미해결 이슈 / 트레이드오프` whose `상태` is `해결`, `## 구현 시 검증 항목`, and a `## 재현·근본원인` whose `근거 등급` is `확인됨(재현·관측)`. A `design-conformance` finding is a divergence from **that** material.
      ...
      **Which `상태` renderings this consumer reads — declared, because the key has two and the shared contract requires every consumer of it to say.** Read **both**: the sub-section form `**상태**: 해결` (bold key, colon, one ASCII space, value to end of line — and its legacy renderings: leading `- `, or the key unbolded) **and** the table form, a `상태` column whose cell is the value. Do **not** loosen to a bare `상태.*해결`, which matches the rationale prose beside the entry as readily as the field. **An entry whose `상태` neither rendering matches is undetermined, and undetermined is not `해결`**: treat it as outside the binding tier and say so in the finding, rather than defaulting it in either direction. The lenient default is the dangerous one here — it would pull an entry into the conformance baseline on the strength of a line nobody could parse.
    ...
    - **The drift sidecar** `<base>/docs/design-drift/{design-doc-slug}.md`, when present, records divergences the implementer already declared. **Derive that path from the design document, never from this review's `{slug}`** — that token is bound to the review slug elsewhere in this skill, so naming it here would address a file that cannot exist and make its absence look like a finding. Derive it explicitly, **in these four steps, and note that the third is not optional**:

      1. `cd` to the design document's own directory and take `pwd -P`, so what follows is keyed to the physical path with every symlinked parent resolved.
      2. Run `git rev-parse --show-toplevel` there.
      3. **Branch on whether that produced a root at all.** The command yields rc≠0 and an **empty string** when the directory is not in a repository — which is this item's own stated ordinary case, the design document being user-local and untracked. Take the repo-relative arm **only** when the returned root is non-empty *and* the document sits under it: then that root is `<base>` and the **document key** is the root-relative path. Otherwise — no root, or a root that does not contain the document — `<base>` is the document's own directory and the key is its absolute path with only the leading `/` removed. Testing containment before testing for a non-empty root is the specific error to avoid: the natural idiom degenerates to a test against `/*`, which matches every absolute path, so the no-repo case silently takes the repo arm, `<base>` becomes the empty string, and the sidecar is looked for at `/docs/design-drift/`. The slug still comes out right, which is what makes the wrong path hard to notice.
      4. Fold the key (`.md` stripped, every `/` replaced by `-`) into `{design-doc-slug}`.

      `<base>` is needed independently of the slug because a sidecar is anchored under `<base>/docs/<kind>/`. The branch above is why this is four steps rather than one sentence: without it the ordinary case produces no path at all, and a reviewer with no path reports the channel unavailable for a sidecar that is sitting on disk.
      Read the file under **`## 1` + `## 3`** of `_common/sidecar.md` — `## 1` for the derivation above and the `owner-doc=` read guard (a mismatch, or an absent `owner-doc=`, means the file belongs to another document: do not apply it, and proceed as if no sidecar existed), `## 3` for the block schema being read (8 fields in fixed order, frozen from the instant a block is appended). Per `## 3`'s reader predicate, a block that is **malformed** under that schema is surfaced to the user rather than consumed — never silently skipped. This is the only reader-side path by which a corrupted drift record becomes visible at all, so it is not optional detail.
      **Absence is not evidence, and this is where the bullet fails unsafely if read loosely.** No sidecar, an unresolvable path, or a file the read guard rejected all mean the channel is **unavailable** — not that nothing was declared. Report the channel as unavailable and stop; do not convert it into findings. Only once a sidecar has actually been read does the rest apply: a divergence recorded there is **disclosed, not thereby justified** — judge it on the merits, and note that a `참고`-tier divergence needed no approval while a `구속`-tier one should carry `승인: 사용자 승인`. An undisclosed binding-tier divergence — one you find in the code with no matching block **in a sidecar you actually read** — is itself a finding.
