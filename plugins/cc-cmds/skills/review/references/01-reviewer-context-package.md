...
- **Round budget**: how many rounds this protocol drives is defined once in `_common/agent-team-protocol.md`'s `### Round budget` — the default, the hard cap, and the two entry paths that open a third round. There is no extension path: reaching the cap surfaces the impasse to the user rather than buying more rounds.
