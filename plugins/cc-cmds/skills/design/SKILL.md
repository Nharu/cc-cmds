...
- **Synthesis fidelity pass (team-driven, inside the synthesis-terminal window, after the full draft is synthesized and immediately before the pre-save sweep)**: before the lead's solo pre-save sweep, the still-alive Step 3 team (not yet cleaned up) checks draft↔discussion fidelity — the lossy-synthesis class of defect that only an original-context holder can catch, and that downstream `design-review` (which discards context each iteration and re-reads only the saved doc) can never reach. Emit once, inline at entry (one-way notice, not a question): *"팀이 설계 초안의 충실도를 점검하고 있습니다."* **Ordering rationale**: running the pass before the sweep makes the lead's fidelity re-reflection Edits "assertions the lead introduced during synthesis," so they fall into the pre-save sweep's unique coverage; reversing the order would let fidelity Edits pass the sweep unverified. **Not a CFI** (same basis as the pre-save sweep — it rides the existing "clarify before the first save" window, adds no new turn-yield semantics; a skipped pass degrades to a quality loss, not a mis-transition; nothing is added above the `## Control-Flow Invariants` heading).
    ...
    - **Completion contract**: each teammate replies via SendMessage with a `[COMPLETE]` body — either `[COMPLETE] Fidelity confirmed — all my contributions accounted for.` (clean) or `[COMPLETE]` + one discrepancy block per finding:
        ```
        ...
        ```
        `[COMPLETE]` means "check done," not "no issues" — a discrepancy report is a completed check. The "await all `[COMPLETE]`" gate is satisfied by either form. `[IN PROGRESS]` always waits — never proceed to save on `[IN PROGRESS]`.
    ...
    - **No-response — H1/H2 guards**: `[IN PROGRESS]` always waits; only genuine zero-DM silence escalates. Fidelity-pass non-response is a **channel-liveness failure** handled by the existing `_common/agent-team-protocol.md` machine — channel-remediation template + `remediation_count`/Bound A (N=3 nudge round-trip; **any valid DM, including a late `[IN PROGRESS]`, resets the counter**). No new counter.
        - **(H1)** never infer no-delivery from a single static interval — declare it only when DMs are 0 across all 3 nudge round-trips.
        - **(H2)** if a valid DM arrives while a Bound-A `AskUserQuestion` is open, its premise is void → auto-cancel the escalation (discard the unapplied answer), reset `remediation_count`, resume per DM type (`[IN PROGRESS]`→wait, `[COMPLETE]`→process verdict). **H2 is scoped to the Bound-A no-response prompt only** — never applied to the re-convergence 2nd-failure prompt (a substantive decision prompt that a late liveness DM does not resolve).
        - **Bound A prompt re-stated in fidelity scope**: ATP's verbatim Bound A prompt ("proceed without their input / abort workflow") is inaccurate here (synthesis-exclusion meaning — the contribution is already in the draft, only the self-check is missing). Present the fidelity-scope variant — (a) "save contributor X's already-reflected contribution without a fidelity check" (→ the emit-only coverage-gap path below), (b) one more remediation retry, (c) abort save/workflow. Counter reuse (`remediation_count`/N=3) is unchanged; only the user-facing option wording adapts to land on the emit-only path.
        - **Stall path (ip_count/Bound B) also reused**: a fidelity-pass assignment is a "new substantive prompt," so ATP reset-rule (b) resets `ip_count` (clean baseline); subsequent content-free `[IN PROGRESS]` repeats then ride the existing stall machine — hard prompt at `ip_count==3`, Bound B at `ip_count==5`. Both Bound A (channel-liveness) and Bound B (content-free stall) apply to fidelity rounds; content-ful `[IN PROGRESS]` always waits. Bound B's user prompt is likewise re-stated in fidelity scope — option-(a) "proceed with what's secured" → "save contributor X's contribution without a fidelity check" (same emit-only landing as Bound A), option-(b) "re-scope/different question" has no analogue (the fidelity task is fixed/single) so it collapses to "re-prompt once more (content-ful re-request)," (c) abort save/workflow.
        - **No implicit-confirm** — silence is never read as "no objection."
    - **Exclusion is emit-only**: when a Bound A/B option-(a) user decision excludes a teammate, do **not** apply that bound's document-metadata convergence-exclusion marking — the teammate already converged in Step 3 (substantive input is in the doc, only self-verification is missing), there is nothing to stamp, and stamping would violate the no-trace invariant. Instead, at save time, emit **once per excluded contributor** (Korean, mandatory — "no silent caps"): *"기여자 X의 기여 내용은 충실도 점검 없이 저장됩니다."* ATP's Bound-option-(a) document-metadata marking is restricted to **genuine Step-3 synthesis-convergence exclusion**.
