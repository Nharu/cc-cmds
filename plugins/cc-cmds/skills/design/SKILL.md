...
- **Reproduction-first (execution by reproduction)** — a third mode of the interview↔exploration loop, for issue/bug tasks. Reproducing the actual symptom *before* team composition keeps the interview questions sharp and anchors the Step 2 team on the real root cause instead of a code-reading guess.
    ...
    - Reproduction runs only the unmodified app/tests and routes all artifacts/logging out of tree — see `## Constraints` → "Reproduction carve-out" (do not restate it here).
...
- NO code modifications allowed. Design discussion only.
  - (reproduction carve-out: running the unmodified app/tests to reproduce a bug is not a modification — see `## Constraints` → "Reproduction carve-out".)
...
### Reproduction carve-out (running ≠ modifying)

The "NO code modifications" rule above and in Step 3 is **absolute and unweakened** — but "modification" has a precise definition that makes reproduction (running the unmodified app or test suite to observe a bug) fully compatible with it. This carve-out is a *definition*, not an exception: the two literal "NO code modifications" strings stay literally true everywhere.

- **What "modification" means**: a change **persisted to the working tree**. Running the app/test suite as-is — without persisting any source change — is NOT a modification. Reproduction is therefore always allowed; it never touches the prohibition.
- **ALLOWED**: executing the unmodified app, running the existing test suite, attaching a debugger, raising verbosity via existing flags/log levels, and importing project modules from an out-of-tree driver to observe internal state.
- **FORBIDDEN**: editing any tracked source file — even transiently, even "I'll revert it after." There is no in-tree print-logging exception. All reproduction artifacts, drivers, and logging route to a `/tmp` sink or existing verbosity/CLI flags — strictly outside the working tree.
- **Single verifiable invariant**: at every team-discussion boundary, `git status --porcelain` (compared against the pre-workflow baseline) is empty. A new file inside the repo breaks the invariant even when untracked (`??`). **Scope**: this invariant governs the *reproduction-artifact* boundaries only. Step 4's design-document save (`docs/{slug}.md`) is the skill's normal sanctioned tracked-file write and is NOT a reproduction boundary, so it is not subject to this invariant.
- **Cleanup boundary — "before findings leave their producer"**: whoever produces reproduction findings (the lead, or the Step 3 reproduction agent) must, *before* the findings leave in any form (whichever comes first: a Round 0 broadcast, an inter-agent SendMessage quoting findings, the reproduction agent returning `[COMPLETE]`, or a doc Edit quoting findings), revert all instrumentation, delete every out-of-tree artifact, and verify `git status --porcelain` == baseline. There is no window in which the tree is dirty while findings are visible to another agent. The lead is the backstop: re-verify once more before the Step 4 save.
- **Environment vs. working tree (lockfile checklist)**: the invariant is scoped to tracked source only. Reproduction may install dependencies and produce build output: (1) gitignored build output (`node_modules`/`dist`/`.next`/`build`) is out of scope — no deletion obligation, never appears in porcelain; (2) an install that rewrites a **tracked lockfile** (`package-lock.json`/`poetry.lock`) or tracked codegen path must NOT persist — use a non-rewrite form (`npm ci`, `npm install --no-save`, venv) or revert before the boundary; (3) the single pass/fail gate at every boundary is `git status --porcelain` == baseline.
