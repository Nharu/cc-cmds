...
- **Early ledger stub**: before the first spawn, create the output doc `docs/{slug}-fe/impl-design.md` as a stub — its H1 title plus the ledger HTML-comment block `<!-- cc-design-ledger v2 … -->` placed right after the H1 and before the first `##`, per `_common/agent-team-protocol.md`'s Role↔agentId ledger. Entry schema: `agentId | state | round | role/scope | thinReturns | last-return summary | scratchDir`, with `state ∈ {running, done, aborted}` (`scratchDir` = the member's out-of-tree witness dir, recorded at spawn, transient — stripped from terminal rows on normal completion). This ledger comment CO-EXISTS with the DS-manifest-version HTML comment added at Step 4 (both are HTML comments near the doc top; they do not conflict — keep both). Update the ledger on every state change; re-read it from disk before any resume; if it is missing or unparseable, fail closed via `AskUserQuestion` (never silent-skip).
