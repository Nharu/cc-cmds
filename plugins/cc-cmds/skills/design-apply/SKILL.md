...
**Convergence has no step of its own.** From Round 2 on it rides each round's **closing declaration** (`_common/agent-team-protocol.md`'s `## Convergence`) — the resume that opens the round already re-injects the current consensus and open conflicts, and requires the witness to end with either `no further input` or a named measurement the member has not yet run. The team has converged, and Step 4 begins, when every member's current-round witness is `witness_present` and its declaration reads `no further input`. The dispatch-completeness gate still runs before collecting, so a member dropped from the round's own fan-out is not read as converged.
