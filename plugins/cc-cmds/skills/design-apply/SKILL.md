...
- The `{slug}` serves two single-purpose roles, both reconstructable from the input path on every invocation without any in-session memory:
    ...
    - **Ledger key**: the slug locates that output doc, whose HTML-comment ledger holds the agentId roster used for resume and cleanup recovery (see `_common/agent-team-protocol.md`'s Role↔agentId ledger v2). agentId recovery comes from the ledger — there is no harness team name in this model.
...
- **Early ledger stub**: before the first spawn, create the output doc `docs/{slug}-fe/impl-design.md` as a stub — its H1 title plus the ledger HTML-comment block `<!-- cc-design-ledger v2 … -->` placed right after the H1 and before the first `##`, per `_common/agent-team-protocol.md`'s Role↔agentId ledger v2. Entry schema: `agentId | state | round | role/scope | thinReturns | last-return summary | scratchDir`, with `state ∈ {running, done, aborted}` (`scratchDir` = the member's out-of-tree witness dir, recorded at spawn, transient — stripped from terminal rows on normal completion, **done and aborted alike**). This ledger comment CO-EXISTS with the DS-manifest-version HTML comment added at Step 4 (both are HTML comments near the doc top; they do not conflict — keep both). Update the ledger on every state change; re-read it from disk before any resume; if it is missing or unparseable, fail closed via `AskUserQuestion` (never silent-skip).
