...
4. **Round 3+ — Refinement (as needed)**: Resume the original authors with the cross-review feedback re-injected verbatim. Confirm each member's refinement witness via `witness_present`, then read it (never the return) before assessing convergence. Repeat as needed **within the hard cap of `_common/agent-team-protocol.md`'s `### Round budget`** — the entry condition here stays `as needed`, because that section's narrowed third-round trigger is written for discussion rounds and a refinement round reflecting cross-review feedback would never satisfy it.
