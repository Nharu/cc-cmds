...
- **Witness scratch dir** → on **normal workflow completion**, `rm -rf` each team's witness directory (path-guarded to the recorded `cc-team-witness-<slug>` `scratchDir`, never a bare or computed path) **and** per-row strip `scratchDir` from **every** terminal ledger row (`done` and `aborted` alike — including a member left `aborted` by a Case-2 partial synthesis). On a **workflow-level abort**, the witness directories and their `scratchDir` fields are **retained** for audit/recovery. The cleanup axis is the workflow result, not the per-row state. The witness dir is out-of-tree (`${TMPDIR:-/tmp}`), so neither retaining nor removing it touches the two-command boundary gate.
