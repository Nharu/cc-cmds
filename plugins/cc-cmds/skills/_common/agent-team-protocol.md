...
- **Idle ≠ Done; idle+text-output ≠ Delivered**: A teammate going idle is normal — it does NOT mean they are done. Teammates may go idle while still processing (e.g., during sequential-thinking). Only trust the `[COMPLETE]` / `[IN PROGRESS]` signal in an actual `SendMessage` DM. If a teammate goes idle without sending a DM (including the case where their idle notification quotes `[COMPLETE]`-prefixed session text), send a follow-up via `SendMessage` with this exact remediation template:
  ...
  **Escalation bounds** (three parallel triggers for auto-escalation to `AskUserQuestion`, referenced as **Bound A**, **Bound B**, and **Bound C** throughout this file):
  ...
  3. **Bound C — Dispatch-failure bound**: see **Per-recipient `SendMessage` dispatch** rule (Facilitator Rules 섹션) for full semantics (retry counter, option (a)/(b)/(c), and cleanup caveat).

  Do not loop indefinitely. In Bound A, Bound B, and Bound C, option (a) overrides the Convergence Check "ALL teammates must confirm" rule — that is intentional; user decides. **Bound A option (a) / Bound B option (a) / Bound C option (a) downstream handling** (all equivalent to "exclude teammate from synthesis"): the lead (i) marks the excluded teammate in the synthesized design document's metadata section (e.g., `**Note**: Teammate X was excluded from convergence per user decision (via {Bound A|B|C}) at {round}`), (ii) during the **cleanup phase** (Step 4 post-synthesis), `shutdown_request` IS sent to the excluded teammate — T5 synthesis-terminal boundary applies only to **synthesis-phase** messaging (forwards / cross-review / convergence-check / clarification), not to cleanup-phase protocol messages (`shutdown_request`, idempotency-guard handling). The excluded teammate's absence is treated as a synthesis data gap; the cleanup contract applies uniformly to all teammates regardless of exclusion status. **For Bound C option (a) only**: cleanup-phase `shutdown_request` is attempted on a best-effort basis — if the original dispatch failure persists on the excluded recipient, the lead proceeds without the shutdown acknowledgement and does NOT re-enter Bound C during cleanup.
...
- **Per-recipient `SendMessage` dispatch**: The `SendMessage.to` field accepts exactly one recipient name per call; `to: "*"`, array values, or any other multi-recipient form are unsupported and error at the harness level. Whenever the same or parallel prompt must reach multiple teammates (Convergence Check issuance, cross-review and refinement-round forwards, cleanup-phase `shutdown_request` fan-out), you MUST issue N separate `SendMessage` calls — one recipient name per call — **dispatched as parallel tool calls inside a single response block**, not serialized across turns. Retries happen in a subsequent response block — parallel-dispatch applies to the initial N-way send and to any simultaneous multi-recipient retry batch (same response block per retry batch), not across retry-to-retry turns. **Bound C — dispatch-failure bound**: if any individual call fails, retry that single recipient only, up to 3 total dispatch attempts per recipient (1 initial + 2 retries); on continued failure, surface via `AskUserQuestion` with options (a) proceed without that recipient's response, (b) retry once more, (c) abort workflow. Bound A/B/C option (a) downstream handling applies — synthesis-metadata marking applies verbatim; Bound C's cleanup-phase `shutdown_request` carries a best-effort caveat (see **Bound A option (a) / Bound B option (a) / Bound C option (a) downstream handling** clause for the canonical wording). Do NOT collapse any retry into a broadcast or array attempt.
...
- **Convergence Check template**: After each round, ask ALL teammates via `SendMessage` per the **Per-recipient `SendMessage` dispatch** rule above — one call per teammate, dispatched in parallel. Send the following prompt to each teammate (teammate-facing; `you` = teammate):
