...
Out-of-tree placement makes the two-command boundary gate (`verification.md` §6: main `git status --porcelain` == baseline ∧ `git worktree list --porcelain` == baseline) self-evidently satisfied — even in a user project where `docs/` is tracked, the witness adds **0 surface** to `git status`. Each **fresh** team (the `design` Step-5 walkthrough team, the Step-6 refinement team) gets its **own** nested `mktemp -d` directory, anchored per-row in the ledger (see §F) so it survives compaction.
...
- **Write discipline (MUST).** Build the full round product **plus the trailing sentinel** in a **single hidden per-attempt temp**:
  ...
  ```
  T=$(mktemp "${scratchDir}/.{role-slug}.round-N.XXXXXX")
  ```
  ...
  ```
  mv -n "$T" "${scratchDir}/{role-slug}.round-N.md"
  ```

  `mv -n` (no-clobber) makes **first-COMPLETE-wins** a structural property. A losing temp self-deletes; teardown sweeps any remainder. A member `TaskStop`-ed mid-write cannot self-delete, so an orphan `.{role-slug}.round-N.*` temp may linger in an abort-preserved `scratchDir` — harmless, because floor-read and recovery only ever look at the non-temp key path (`{role-slug}.round-N.md`).
- **Sentinel + nonce.** The last line of the published file is the sentinel carrying the lead-injected per-round nonce:

  ```
  <!-- cc-witness: {role-slug} round N complete {nonce} -->
  ```
...
- **No same-round rewrite.** A published witness is **never** overwritten. A revision is a new round (a new file) — this is what makes `mv -n` safe: a published path is immutable, so the bytes the lead synthesizes from are preserved until audit time.

## Completion predicate `witness_present(member, N)`

Holds **iff all three conjuncts hold**:

1. `test -f {WITNESS_PATH}` — the non-temp `(role, round-N)` key file exists (fast pre-gate).
2. The file's **last non-empty line** (strip trailing blank lines / newlines before comparing) == `<!-- cc-witness: {role-slug} round N complete <nonce> -->`. The sentinel rides inside the atomic publish, so a file that exists but is torn or direct-written lacks this line and fails. (Trailing-blank tolerance avoids a false-negative respawn from a stray final newline.)
3. The filename's N == the sentinel's N (key consistency) **AND** the sentinel's nonce == the lead-injected per-(member, round) `{WITNESS_NONCE}`.
...
- **3-conjunct death predicate.** A member is declared dead **iff**: `reentry_count ≥ K (=3)` ∧ `witness_present` still false ∧ (`output_file` absent **OR** `b == last_output_bytes`, the two most recent transcript byte-counts unchanged). The liveness conjunct reads the member's harness **`output_file` byte-count**, NOT the witness bytes — the witness appears in one atomic rename and has no growth curve, so it cannot be a liveness signal. **All three conjuncts are required**; dropping byte-stability would kill a member still writing. `reentry_count` and `last_output_bytes` are **lead-LOCAL ephemeral** liveness-tracking state — **not** ledger v2 columns (§F deliberately omits counters; they are not durable state). They re-zero after a compaction, but the disk-first check (witness floor-read precedes the count) is the durable completion check, so re-zeroing only *delays* a death declaration (bounded extra wait), never makes it wrong.
...
| **Witness** (`${scratchDir}/{role-slug}.{round/phase}.md`) | CONTENT (full product, sentinel-terminated) | **member** (sole, atomic rename) | /tmp out-of-tree | deleted on normal workflow completion (all terminal rows); retained on workflow-level abort |
