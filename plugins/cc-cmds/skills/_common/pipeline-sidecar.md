...
| `checks.observed.draining.<pid>` | gate (`gate.sh`) | `checks.observed` renamed aside for the duration of one drain, so the poller's next append goes to a fresh file and nothing observed mid-drain is lost. Removed when the drain finishes; the poller seeds its transition table from any that survive a drain that died |
