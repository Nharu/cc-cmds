...
**`절단점` on a `승인` row is not always a cutpoint token.** Three shapes share the series because they share the lifecycle: an **act** approval carries a `CUTPOINTS` token and a binding tuple of `(대상 별칭, 슬러그, 행위 토큰, argv 다이제스트, 브랜치, head_sha, base_sha, PR 번호, 리뷰 리포트 다이제스트, 열린 P0·P1)`; a **judgment** approval carries the literal `판단` and a tuple of `(스테이지 id, 질문 문면 다이제스트, 선택지 집합 다이제스트, 스냅숏 다이제스트)`; a **boundary** approval — issued by B1–B5, which have no act at all — carries the literal `경계` and a tuple of `(경계 이름, 발동 시점 H, 관련 세그먼트 집합)`.

**B5 is the disposition volume boundary and it is evaluated OUTSIDE the pending-approval guard**, beside B4 rather than beside B1–B3. Those three are skipped whenever any act approval is waiting; B5 is not, and that placement is the decision rather than an accident of where it was pasted. Its threshold is low, so inside that guard a single open approval would switch it off for the rest of the night — with the boundary's existence standing as the reason nobody re-measures the surface it was meant to bound.

**It counts all three dispositions, exemption included.** Exemption writes no row, and counting only the two that do was measured to invert the incentive: the row-free path stays at zero while the honest one is what trips the boundary. The exemption component is also **latched** in the run directory, because unlike the two append-only row counts it is a predicate re-derived from segment state, and segment state is reversible — unlatched, walking a parked segment back and forth resets it. Staleness is re-derived at execution against whichever tuple the row carries, so the three do not need three series.
