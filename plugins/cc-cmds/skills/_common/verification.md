...
**Which tree the gate measures, named once.** The `cwd`-pinning principle of `### 6.2` binds a **Surface-2 caller only** — it is a rule about not standing in `$WT` while running a gate check, and a caller that creates no experiment worktree has no `$WT` for it to be about. Independently of that scope, and for every caller: where assertions 1 and 2b say "main tree" or "this caller's own worktree", they mean **the tree the caller's boundary baselines were captured in**. That phrasing is what makes the subject well-defined rather than inherited from whatever dispatched the caller, and it stays correct if a caller is ever relocated to a linked worktree — at which point "the main tree" and the tree the caller actually measures are different strings. A caller takes both boundary observations in that one tree.
...
**Assertion 2 is scoped, never a whole-output equality.** `git worktree list --porcelain` is a **repository-global** command: run from any worktree it emits every worktree of the shared repository, each with its own `HEAD <sha>` line. Comparing that output wholesale fails the gate whenever an unrelated sibling worktree commits — a tree no caller of this gate measured, and one whose movement changes no byte of anything the caller did measure. All three of these must hold, and each of them catches something the other two and assertion 1 cannot: 2a is the only check that sees a worktree this caller created and leaked, 2b is the only check that sees a content change that was **committed** inside the window, and 2c is the only check that needs **no baseline at all** and therefore still works after a compaction destroyed every baseline string.

- **2a — own creation record.** Not a set equality over the repository's worktrees. The caller keeps an **append-only creation record**: at the moment it adds a worktree, in the same Bash call as the `git worktree add`, it appends that worktree's absolute path — spelled as `git worktree list` will spell it, resolved — together with the owner token and a creation timestamp. 2a asserts that **every path the record claims is absent from `git worktree list --porcelain` at the close**, and that the caller's own primary entry (2b's subject) is still present. A worktree neither created by this caller nor claimed by its record is **outside 2a entirely** — created or removed, it is not a mismatch. A caller that creates no worktrees has an empty record and 2a holds vacuously: **a checked assertion over a provably empty domain, not an unchecked field.**

    > **옛 표제**(인용 재유도용): 「- **2a — path set.**」 — 이름 패턴을 폐기하고 호출자 자신의 생성 기록으로 바꾸면서 짧은 이름이 함께 바뀌었다.

- **2b — own entry.** This caller's own worktree entry is compared as three things, and the third is a tree and not a commit: its `worktree` line equals the baseline; its `branch` / `detached` line equals the baseline; and the **tree object its `HEAD` resolves to** — `git rev-parse "<the entry's HEAD sha>^{tree}"` — equals the baseline's. Catches a content change under the tree the work actually measured **even when that change was committed**, which is the one thing assertion 1 cannot see: porcelain is clean at both endpoints of a modify-and-commit. The commit sha is recorded in the baseline as a **diagnostic that is reported whether or not there is a mismatch and never compared**, so a halt record can still name which commit moved and a passing run can still say that one did; a `HEAD` that moves without changing the tree is **not** a mismatch.

    > **옛 표제**(인용 재유도용): 「- **2b — own entry.**」 — 짧은 이름은 같고 비교 대상이 커밋 sha 에서 그 커밋이 가리키는 트리로 바뀌었다.

- **2c — mechanism-owned leak, scoped to this caller.** The count of worktree entries whose path carries **this caller's owner token** under the `cc-design-exp-` prefix is **0**. Still an absolute predicate and still not a baseline comparison — which is the whole of its value: it is the one assertion that survives a compaction that destroyed every baseline string, because the owner token is re-derived from the caller's own session identity rather than read back from a record. It never condemns the user's own pre-existing worktrees, and — new — never condemns a **concurrent caller's live experiment worktree**, because that worktree carries a different owner token.

    > **옛 표제**(인용 재유도용): 「- **2c — mechanism-owned leak.**」 — 절대 술어는 그대로이고 그 정의역이 접두 전체에서 이 호출자의 소유 토큰으로 좁혀졌다.

**Foreign worktree churn is deliberately not a mismatch — creation, removal and `HEAD` movement alike.** None of it changes a byte of the caller's own tree or of any reviewed text, so whatever the gate protects — an induced-defect rate, a frozen document, a claim's measurement root — is untouched, and the callers that hash their input hash it directly. Under 2a's record form this is structural rather than a carve-out: a worktree the caller's record does not claim is outside the assertion, so there is nothing to excuse.

Three measured false positives rather than hypotheticals, each of which the old whole-output comparison produced:

- An audit halted at `freeze-mismatch` because a **sibling worktree committed** inside the window, while the document hash, every `git status --porcelain` baseline, and every measurement root were unchanged.
- The declared exception pattern `-run-` covered only one of this pipeline's two segment-worktree spellings, so a sibling run **moving a worktree from one spelling to the other** was guaranteed to break 2a for something the audit neither did nor could see. Measured on one repository: of 46 live entries, 19 matched `-run-`, 1 matched `/run/` and not `-run-`, and a third naming family matched only by accident of the repository's name. The same set was observed at 45, 46 and 47 by three readers on one day with none of them creating a worktree (`Nharu/cc-cmds#724`).
- An audit halted on **2b** because an empty commit — zero changed files, tree byte-identical to the baseline's — moved the main worktree's `HEAD` inside the window, after the reader fan-out had already been paid for in full. `FROZEN_SHA256` re-hashed identical and assertions 1, 2a and 2c all passed. On that repository 25 such commits were counted across three days, every one of them empty (`Nharu/cc-cmds#768`).

**2a and 2c are split by RECORD-DEPENDENCE, and that is what keeps them from saying the same thing.** 2a ranges over the caller's **own creation record** and compares it against the close; it is the only check that can condemn a leaked worktree the caller created **outside** the mechanism namespace, and it is **blind if the record is lost**. 2c ranges over the **machine's worktree list** filtered by the **owner token**, needing no baseline and no record; it is the only check that still works **after the record and every baseline are gone**. They are complements along one axis and neither restates the other, so nothing has to be merged and no merge disclosure is owed.

**Why the prefix cannot be 2a's domain, stated because the obvious narrowing is a tautology.** Defining 2a as set-equality over the `cc-design-exp-` namespace looks like the natural way to scope it to what the caller owns. It is not: 2c is the absolute predicate *count == 0*, so whenever the baseline count was zero — the normal case for a caller that declares no experiment worktrees — 2c **implies** that 2a holds, and 2a cannot fail without 2c failing first. That yields one assertion and one tautology, which is worse than a duplicate because a tautology reads as coverage. The record form avoids it by ranging over something 2c does not see at all: paths the caller created, whether or not they carry the prefix.

**Why 2c stays absolute while 2a is record-relative.** 2c's whole value is that it needs no baseline, so it survives the one failure mode that defeats every comparison — a compaction that destroys the caller's in-context baselines. Making it baseline-relative would buy nothing and cost that. What made its old whole-namespace form unusable was not the absoluteness but the domain: the count ranged over entries **whichever** caller minted, and nothing in a worktree entry records a caller (across 47 live entries the field keys were exactly `worktree`, `HEAD`, and `branch`/`detached` — no `locked`, no `prunable`, nothing caller-identifying). So a window that opened while a concurrent caller's experiment worktree was live opened with 2c already violated, and the caller that got condemned was whichever one's window happened to be closing. That needs no create-and-destroy coincidence: **existence at close is enough**, which is the ordinary state of any concurrent session mid-experiment. Putting the owner token in the path fixes the domain and leaves the absoluteness intact.

Measured: a `design-audit-unattended` run halted at `freeze-mismatch` on exactly this shape — 2a went 20 paths to 19, the vanished entry was a `cc-design-exp-` worktree the audit neither created nor removed, 2c was 1 at open and 0 at close, and the frozen document's sha256 was byte-identical across the window.

**The creation record of assertion 2a — and every name pattern is retired.** A caller that legitimately creates and removes worktrees while a gated window is open keeps an append-only record of what it created, and 2a reads that record. There is no declared pattern any more, and no spelling of one is admissible.

> **옛 표제**(인용 재유도용): 「**The declared exception pattern of assertion 2a.**」 — 패턴을 폐기하고 생성 기록으로 바꾸면서 이 표제가 가리키던 기전 자체가 사라졌다.

**No pattern can work, and the reason is structural rather than a spelling.** The worktree names are chosen by a **producer the gate does not constrain**, so a pattern is a contract term whose satisfaction is controlled by a party the contract does not bind — and the declaration has to happen *before* the window opens, so the caller cannot even observe the spellings it would need to cover. Two live spellings and a third accidental family were measured on one repository (see the false positives above), and the global set moved three times in one day unaided. A set-equality assertion over a continuously churning global set cannot be made to pass.

The record is bounded by three things, and it is not safe without all three:

- **It is recorded, not inferred.** The append happens **at creation time**, in the same Bash call as the `git worktree add`, which is unconditionally before the close. An append made **after the closing observation is not admissible** — this reuses the clock `### 6.0` already owns rather than inventing a second one. The hazard the old "declared before the window opens" rule closed is closed harder here: there is no term to choose after seeing the diff.
- **It reaches 2a and nothing else.** A recorded path excuses nothing about tree contents (assertion 1), nothing about the caller's own entry (2b), and nothing about the mechanism namespace (2c). Those are different failure modes with different owners.
- **The teardown guard survives and becomes primary.** Matching a name must never by itself authorize removing a worktree; the caller removes a path only when its **own record** shows that path was created by this run. The record was already the primary condition and the pattern a second one — this removes the second and gives its anti-lookalike function to the owner token, so a hand-made lookalike is excluded because it is neither in the record nor carries the token.

**Where the record lives, and who writes it.** The agent that runs `git worktree add` writes it, in the same Bash call — not the lead, not a later step, or there is an interval in which a worktree exists unclaimed. It lives **out-of-tree**, under the caller's existing scratch root (for the audit arms that is the witness directory; for a team it is the per-team `mktemp -d` anchored as each row's `scratchDir`). In-tree is forbidden for the same reason the witnesses are out-of-tree: an in-tree record changes the very `git status` assertion 1 states, so it has no fixed point. Each line carries the absolute worktree path as `git worktree list` will spell it, the owner token, and the creation timestamp. It must outlive the creating agent and must not be deleted before the close.

**What this gives up, named as a loss.** A worktree the caller's own team creates **without** appending it to the record and **without** an owner token is now invisible to all five checks — previously 2a's global set equality caught it, buried in foreign churn. The bound is that such a creation is a contract violation rather than an accident (the `mktemp` + `--detach` form is already a MUST and the append is specified into the same Bash call), and there is **no executable bound**. Closing it would require bringing the worktree-naming producer under this contract, which is out of scope here.

This parameterization is what lets the gate's own promise — that it **never condemns the user's own pre-existing worktrees**, and now also never condemns a concurrent caller's live one — hold for a caller whose normal operation transforms the worktree set, without weakening it for anyone else.

### 6.0 Assertion 1's declared measurement surface

**This narrowing has not been observed to fire in a shipped run, and the rate belongs at the head of the section rather than in the reader's assumptions.** Measured on this repository's audit corpus on 2026-09-07 — 61 reader witnesses grouped into the 15 complete runs they belong to — it fired in **0 of 15 runs**, both under the predicate defined below and under the predicate that one replaced. Every one of the fifteen retained at least one token forcing the whole-tree fallback. **This section reports three corpora, and naming their relation is part of reading the figures.** The 61 reader witnesses and 15 runs here and the 71 witness and report files counted further down are the same repository audit corpus on the same date, counted in different units — the first counts reader witnesses and the runs they belong to, the second counts witness files and report files together. **The one arithmetic bridge those units admit does not hold, and this contract records the gap rather than picking a side**: at one report file per run, the 71 leaves 10 report files above the 61 witnesses against 15 runs, five short, and nothing printed here resolves that difference — so the two figures must not be read as a single enumeration. The 11 of 63 witnesses and 5 of 6 runs reported on axis 1 below is a third corpus, earlier and smaller and taken on a different instrument as that sentence already says, and it is neither a subset nor a superset of the other two. **The token-level reduction reported further down is a different quantity and does not imply this one**: the fallback is an existential quantifier over a run's tokens, so that count can fall by an order of magnitude while the run-level result does not move at all. What holds the rate at zero is the residual classes this section ends with, and **one of them is produced by the audit's primary duty rather than by a misspelling**, so no production-side spelling rule closes it. Until a measurement shows the rate moving, a caller should expect assertion 1 to be whole-tree equality in practice and should treat a narrowed surface as the exception.
...
**The default is the whole tree**, so a caller that declares nothing gets today's whole-porcelain equality byte for byte. Empty assertion 1's surface by default and the gate stops measuring the thing it exists for.
...
An anchor entry that is a symbol, a count, or a numeric claim rather than a path contributes nothing. The frozen document needs no entry: a caller that hashes its input covers those bytes with that hash directly, so this scoping loses nothing there. **The caller's own output paths are a different case, and the hash does not reach them** — nothing hashes a caller's output, and the derivation above subtracts nothing, so an output path a witness cites does land on the surface. That widens the equality check rather than narrowing it, which is the safe direction; what it costs is a caller that writes that path inside its own window seeing its own write at the close.
...
- **Mechanism (normative)**: `WT=$(mktemp -d "${TMPDIR:-/tmp}/cc-design-exp-<owner-token>-<slug>.XXXXXX")` then `git worktree add --detach "$WT" HEAD`. `mktemp` is a MUST (uniqueness = concurrency safety; TMPDIR root).

    The **owner token** is the caller's already-existing phase nonce — for the audit arms `PASS_TOKEN`, which this contract already requires to be colon-free; for a team, the phase `witnessNonce` anchored per-row in the ledger. A caller with no such nonce mints one at its first experiment-worktree creation and appends it to the same record 2a uses, so 2c is defined for a caller that is neither an audit arm nor a team.

    **Two markers, two consumers, and they are not the same marker.** `cc-design-exp-` is the **machine-wide** marker meaning "some instance of this mechanism made me" — it is what a cleanup sweep would key on. `<owner-token>` is the **gate's** marker meaning "*this* instance made me". 2c is a gate and uses the second; conflating them is what made 2c condemn a concurrent caller's live worktree.

    **EnterWorktree is forbidden** (its only-if-unchanged auto-cleanup guarantees a leak for a changed mini-implementation worktree); the Agent-tool isolation form is forbidden (unavailable to teammates; it fragments the cleanup inventory). `--detach` is mandatory (no branch-namespace pollution; safe under concurrent creation). A teammate may create a worktree under the same `mktemp` + `--detach` duty.
...
- **Lifetime**: exists only between claim pre-registration and that claim's **verdict broadcast**. A mid-experiment `[IN PROGRESS]` observation share is not a kill event (avoids the pathological per-message teardown-rebuild). Hard kill-points (the two-command boundary gate of §6 == baseline required — assertion 1 plus the scoped 2a/2b/2c, never the whole-output `git worktree list` equality this section retires above): verdict broadcast / Quality Gate / producer `[COMPLETE]` / pre-save sweep / abort / Step 6 entry. **No persistence across re-convergence** — a re-convergence experiment is regenerated from the recipe (which already exists as a broadcast precondition; regeneration is deterministic because the main tree is frozen for the whole session). One experiment = one worktree, never shared across agents.
...
- **Cleanup**: the producer runs `git worktree remove --force "$WT"` + `git worktree prune` + the §6 boundary gate before the verdict ships. **2c is the assertion that condemns a leaked experiment worktree of this caller here** — of this caller specifically, because the count is now filtered by the owner token. Lead backstop: the pre-save sweep and the Step-6-entry state-check add the two-command gate. `prune`'s incidental cleanup of a user's stale worktree is accepted residual (the prefix naming mitigates; remove-own-paths takes priority).

    **What this gives up: an ORPHAN is now condemned by nobody, and that is a recorded decision rather than an oversight.** An orphan is an experiment worktree minted by a caller that has since died. The old whole-namespace 2c caught every one regardless of owner; the token-scoped form catches only its own. After this change nothing in `make check`, nothing in the gate, and nothing in any skill detects one — the "cleanup sweep" this section names as the prefix's consumer **does not exist as code** in this repository, so deferring orphan detection to it would be a deferral to nothing. No sweep is specified here: a sweep is not an assertion and would be miscategorized in this section, and specifying a mechanism that cannot be tested is the defect class this contract keeps removing.

    The exposure is bounded by two mechanisms that already exist, and the bound is why the decision is acceptable rather than merely stated. The worktree **directory** dies on the host's own schedule, because it is minted under `${TMPDIR:-/tmp}`. The worktree **record** outlives that: measured in a throwaway repository, `rm -rf` of the directory leaves the entry in `git worktree list --porcelain` unchanged, and `git worktree prune` removes it — and that `prune` is already in this bullet's own teardown, run by every producer. So the window in which an orphan is visible and condemned by nothing is the interval between the host's sweep and the next producer's prune, not an unbounded accumulation.
