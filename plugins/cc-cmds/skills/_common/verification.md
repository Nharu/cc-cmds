...
- **2a — path set.** The set of worktree paths (`git worktree list --porcelain | grep '^worktree '`) equals the baseline, **modulo the `cc-design-exp-` prefix and the caller's declared exception pattern**, both removed from *both* sides. Catches a worktree created or removed inside the window.
...
**A `cc-design-exp-` entry appearing or disappearing is likewise not a 2a mismatch, and the reason is ownership rather than leniency.** That prefix is the namespace 2c already owns: 2c asserts the count is **0**, so an entry carrying it is either absent — in which case it is in neither side of 2a — or present and already condemned by 2c. Leaving it in 2a's comparison therefore adds no detection and subtracts correctness, because the entries under that prefix belong to *whichever* caller made them, and a caller's declared exception pattern names only its own. The failure that follows is a false stop on an uninvolved session: a second session creates and tears down an experiment worktree entirely within a first session's window, and the first session — whose own tree, own entry and reviewed bytes are all unchanged — cannot pass 2a and cannot widen its pattern after the fact.

**The interaction with 2c's absolute form is what makes this mandatory rather than a convenience.** 2c is not a baseline comparison; it is the absolute predicate *count == 0*. So a window that opens while some other caller's experiment worktree exists opens with 2c already violated, and if that owner tears it down before the window closes, 2c passes at close **and 2a is guaranteed to fail** — the set lost an entry. Under the previous wording no caller could satisfy both, and nothing in the contract said which one should give. Excluding the prefix from 2a resolves it in the direction that keeps every detection: 2c still condemns any such entry that is present at close, and 2a stops reporting a change it was never the owner of.
...
**The declared exception pattern of assertion 2a.** A caller that legitimately creates and removes worktrees while a gated window is open declares, **before the window opens**, a single path-substring pattern naming the worktrees it owns. Assertion 2a then compares the worktree path set with the entries matching that pattern removed from *both* sides. **The default is the empty pattern**, under which 2a compares the whole set less the `cc-design-exp-` prefix above — so a caller that declares nothing gets the ordinary set equality, minus the one namespace 2c already owns.
