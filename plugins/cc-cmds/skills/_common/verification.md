...
- **One working tree resolves both sides.** Porcelain entries and surface entries are repository-relative paths of the caller's **main working tree**, and the `-z` observation is taken there and nowhere else; a difference and a surface resolved against two trees are not comparable at all.
