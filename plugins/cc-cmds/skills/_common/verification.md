...
A single, generalized definition covering reproduction and verification alike. A carve-out is a *definition*, not an exception — the "NO code modifications" literals stay literally true.

**Which tree the gate measures, named once.** The `cwd`-pinning principle of `### 6.2` binds a **Surface-2 caller only**. For every caller, where assertions 1 and 2b say "main tree" or "this caller's own worktree", they mean **the tree the caller's boundary baselines were captured in**, which stays correct if a caller runs in a linked worktree. A caller takes both boundary observations in that one tree.
...
- **Two-command boundary gate**. At every team-discussion boundary check both, in this order:
    1. main tree `git status --porcelain` == the pre-workflow baseline, **within the caller's declared measurement surface** (§6.0);
    2. the worktree baseline, compared as the **three scoped assertions** below (porcelain does not see records inside `.git/worktrees/`, so the worktree list is a separate gate from assertion 1).

**Assertion 2 is scoped, never a whole-output equality.** `git worktree list --porcelain` is **repository-global**, so a wholesale comparison fails whenever an unrelated sibling worktree commits. All three of these must hold: 2a is the only check that sees a worktree this caller created and leaked, 2b is the only check that sees a content change that was **committed** inside the window, and 2c is the only check that needs **no baseline at all** and therefore still works after a compaction destroyed every baseline string.
...
- **2c — mechanism-owned leak, scoped to this caller.** The count of worktree entries whose path carries **this caller's owner token** under the `cc-design-exp-` prefix is **0**. An absolute predicate, not a baseline comparison: the owner token is re-derived from the caller's own session identity rather than read back from a record, so it survives a compaction that destroyed every baseline string. It never condemns the user's own pre-existing worktrees, nor a **concurrent caller's live experiment worktree**, which carries a different owner token.

    > **옛 표제**(인용 재유도용): 「- **2c — mechanism-owned leak.**」 — 절대 술어는 그대로이고 그 정의역이 접두 전체에서 이 호출자의 소유 토큰으로 좁혀졌다.

**Foreign worktree churn is deliberately not a mismatch — creation, removal and `HEAD` movement alike.** None of it changes a byte of the caller's own tree or of any reviewed text, so whatever the gate protects is untouched, and the callers that hash their input hash it directly. A worktree the caller's record does not claim is outside the assertion, so there is nothing to excuse.

**2a and 2c are split by RECORD-DEPENDENCE.** 2a ranges over the caller's **own creation record**; it is the only check that can condemn a leaked worktree the caller created **outside** the mechanism namespace, and it is **blind if the record is lost**. 2c ranges over the **machine's worktree list** filtered by the **owner token**; it is the only check that still works **after the record and every baseline are gone**.

**The prefix cannot be 2a's domain**: over the `cc-design-exp-` namespace 2c would imply 2a, leaving one assertion and one tautology.

**2c stays absolute while 2a is record-relative**: 2c's value is needing no baseline, and the owner token in the path is what scopes its domain to this caller.
...
**No pattern can work**: worktree names are chosen by a **producer the gate does not constrain**, and a pattern would have to be declared before the caller could see the spellings it must cover.

The record is bounded by three things, and it is not safe without all three:

- **It is recorded, not inferred.** The append happens **at creation time**, in the same Bash call as the `git worktree add`, which is unconditionally before the close. An append made **after the closing observation is not admissible** — the clock `### 6.0` owns.
- **It reaches 2a and nothing else.** A recorded path excuses nothing about tree contents (assertion 1), the caller's own entry (2b), or the mechanism namespace (2c).
- **The teardown guard survives and becomes primary.** Matching a name must never by itself authorize removing a worktree; the caller removes a path only when its **own record** shows that path was created by this run. A hand-made lookalike is excluded because it is neither in the record nor carries the owner token.

**Where the record lives, and who writes it.** The agent that runs `git worktree add` writes it, in the same Bash call — not the lead, not a later step, or there is an interval in which a worktree exists unclaimed. It lives **out-of-tree**, under the caller's existing scratch root (for the audit arms that is the witness directory; for a team it is the per-team `mktemp -d` anchored as each row's `scratchDir`): an in-tree record would change the very `git status` assertion 1 states. Each line carries the absolute worktree path as `git worktree list` will spell it, the owner token, and the creation timestamp. It must outlive the creating agent and must not be deleted before the close.

**What this gives up.** A worktree the caller's own team creates **without** appending it to the record and **without** an owner token is invisible to all five checks; such a creation is a contract violation, and there is **no executable bound**.

### 6.0 Assertion 1's declared measurement surface

**This narrowing has not been observed to fire in a shipped run**, so a caller should expect assertion 1 to be whole-tree equality in practice and treat a narrowed surface as the exception.
...
- **Take both observations with `git status --porcelain -z`.** Default porcelain C-quotes any path `core.quotePath` treats as non-ASCII, so a quoted entry compares unequal to the same path held unquoted in the surface, and the intersection comes back empty. `-z` emits raw path bytes NUL-separated with no quoting, and it also splits a rename's two paths into separate fields instead of one arrow-joined string.
...
An anchor entry that is a symbol, a count, or a numeric claim rather than a path contributes nothing. The frozen document needs no entry: a caller that hashes its input covers those bytes with that hash directly. **The caller's own output paths are a different case** — nothing hashes a caller's output, so an output path a witness cites does land on the surface, which widens the check (the safe direction).

**What this yields is the set of paths the caller CITED, which is strictly narrower than the set it read** — and the contract claims the narrower one, because all three sources key on citation. A caller restating this scoping in its own words restates it this way — **and restates the caveat with it**: on a run where the totality check below put the surface at the whole tree, this narrowing does not hold at all and every porcelain change is inside the surface. A caller wanting the wider set has to record the wider set first.

**The after-the-fact narrowing hazard, and what closes it.** The hazard is shrinking the measurement surface after reading the diff. What closes it is that the surface is **derived rather than selected**, and derived from witnesses that are out-of-tree files carrying a phase nonce — which seals their completeness and their authorship, not their bytes — and fixed before the caller observes the closing porcelain at all. **That ordering is a requirement, not a description**: a surface derived after the closing observation is not a surface, and a caller that finds itself deriving one then must treat it as absent.

**Fail closed unless the derivation is total.** The trigger is a predicate on the **derivation**, not on the witness files, and it ranges over the tokens in the **quantified domain** defined below and over nothing else. The unit is the token the derivation actually extracts, not the typeset cell, and this contract defines the extraction rather than leaving it to the reader: from each source line, (i) the **content** of every markdown code span is one token, (ii) every maximal whitespace-separated run of the text outside those code spans is one token, and (iii) when a code span's content holds whitespace, every maximal whitespace-separated run inside that content is a token as well. **A backtick is not part of a token; it is a token boundary** — a code-span anchor and a bare anchor yield the same token, so the span markup never has to be stripped and never has to be tolerated. (iii) is there because a span holding a command and a path together resolves nothing as a whole while the path inside it is real: without splitting the span, that path is extracted by nothing and contributes zero in silence. **The quantified domain is the union of two limbs, and they ask different questions.** The **resolution limb**: drop a trailing `:<line>` or `:<line>:<col>` suffix, and if what remains does not resolve, drop the trailing punctuation `.,;:)]}"'?!` as well; if either spelling resolves as a repository-relative path of the repository the closing observation is taken in, the token is in the domain and the surface carries that spelling. **This limb does not ask what characters the token is made of** — a path carrying non-ASCII bytes, a path with no extension at all, and a path a sentence has put a comma after all enter here. The **shape limb**: the same suffix-dropped text is composed only of the characters `[A-Za-z0-9._/-]`, **holds a `/` separator**, and **either ends in a dot followed by one to five ASCII letters or the suffix the resolution limb's rewrite dropped from this token was itself a `:<line>` or `:<line>:<col>` suffix**. **The two conjuncts hang on different things on purpose**: the character class and the separator requirement are properties of the rewritten text, while the line-suffix disjunct is a property of the rewrite, which rewritten text can no longer carry. `.` and `..` name no path for this purpose and are in neither limb. **A leading `/` is not a disqualifier on this limb**: an absolute path enters the domain here, resolves against nothing, and so forces the fallback. One cell may yield several tokens and each contributes on its own. **The two limbs are orthogonal and must not be read as one reduction rate** — the resolution limb only adds to the surface and the shape limb only subtracts from the domain. The shape limb's separator requirement **loses no path**: a repository-relative path with no `/` in it is a root file, and a root file resolves, so the resolution limb is already holding it. **The surface is the derived path set only when all three sources above were enumerated from a durable record sealed before the closing porcelain observation and every token in the domain those sources carry resolves, with nothing rewritten beyond the suffix and trailing-punctuation dropping the resolution limb defines, as a repository-relative path of the repository the closing observation is taken in. Every other case is the fallback: if any such token fails to resolve that way for any reason whatever, or the derived set comes back empty, the measurement surface is the whole tree, not the empty set.** An absolute path, a well-formed relative path naming a location outside `CODE_ROOT`, and a well-formed relative path naming something that is not there are **examples and not the extension** — the extension is the complement of the positive condition, and no enumeration narrows it. **The residuals are enumerated by behaviour, not by discovery order**, so a class nobody has named yet still has an axis to stand on; a relaxation argued on one axis does not carry to the other. **Axis 1 — in the domain, resolving against nothing, so the fallback is forced.** The classes named so far on this axis are a partial path citation and an absence-confirming citation. A **partial path citation** is a token such as `autopilot/SKILL.md`, naming a real file by the tail of its path; it *is* a path spelled wrong, which a production-side rule can close. An **absence-confirming citation** is not: a reader that spells a path exactly right and judges it absent leaves an anchor that resolves against nothing **because the file is not there**, and finding absent anchors is the reader's primary duty, so no production-side rule closes it. **This contract names that class and does not close it** — nothing in this section should be read as claiming the class is handled. A branch or lock name spelled with a `/` reaches neither class: it carries no extension and no line suffix, so the shape limb excludes it. **Axis 2 — outside the domain and silent, so nothing forces the fallback and nothing is checked.** The class measured here is a **citation of a real file by its basename alone**, and it is one member of this axis rather than its extent — `SaleManagement.tsx:28` where the file is `src/components/pages/store/sale/SaleManagement.tsx`. It holds no separator and resolves against nothing, so it contributes zero in silence and that file's path never reaches the surface. Any file can be cited by its basename, so the exposure is bounded by the number of files in the repository; dropping the separator requirement would readmit the non-path class it removes, and calling this class back into the domain would take a new decision rule this contract does not make. Neither axis is a hole in any of the three assertions. **The resolution limb also admits a prose word that happens to coincide with a repository path**, which widens the surface rather than narrowing it and is accepted for that reason — a token this contract cannot classify costs a wider equality check and never a narrower one. The records are sealed **before the closing observation**, the only clock a caller can meet, since the reader witnesses are written after the window opens. That sealing is **necessary**, and it is not sufficient: it does not stand in for the requirement above that the surface is **derived rather than selected**, and that a surface derived after the closing observation is not a surface at all. **A partial derivation is not a smaller surface**: the union of whatever happened to resolve is a surface small enough to let assertion 1 pass for free. An empty or under-derived surface makes assertion 1 pass unconditionally, which is an unchecked field wearing the shape of a check.
...
- **Cleanup**: the producer runs `git worktree remove --force "$WT"` + `git worktree prune` + the §6 boundary gate before the verdict ships. **2c is the assertion that condemns a leaked experiment worktree of this caller here** — of this caller specifically, because the count is now filtered by the owner token. Lead backstop: the pre-save sweep and the Step-6-entry state-check add the two-command gate. `prune`'s incidental cleanup of a user's stale worktree is accepted residual (the prefix naming mitigates; remove-own-paths takes priority).
    ...
    **What this gives up: an ORPHAN is now condemned by nobody, and that is a recorded decision rather than an oversight.** An orphan is an experiment worktree minted by a caller that has since died; the token-scoped 2c catches only its own, and the "cleanup sweep" named as the prefix's consumer **does not exist as code**. No sweep is specified here: a sweep is not an assertion.

    The exposure is bounded by two existing mechanisms: the worktree **directory** dies on the host's own schedule under `${TMPDIR:-/tmp}`, and the worktree **record** is removed by the `git worktree prune` every producer's teardown already runs.
...
**Why this is the one carve-out from the file's contracts-only posture.** A pass predicate is a predicate, not a procedure, so it belongs to this file. **Do not add a coupling lint for this section**: with no second copy there is nothing to synchronize.
