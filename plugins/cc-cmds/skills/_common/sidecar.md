...
- **The exit status is the whole of the caller's interface.** `0` means the swap committed; any non-zero status means **nothing was written** and the existing file is intact. The disposition above is therefore not scoped to a failed comparison: an attempt that could not be prepared or committed at all — a failed `mkdir`, a failed `mktemp`, a temp that failed its completeness check, a `rename(2)` that genuinely failed — ends at the same place and takes the same disposition. Every failure path of this contract has exactly one destination. A snapshot that could **not be taken** is never encoded as absence: a failed `mktemp` is a failure to prepare an attempt, not an observation that the target was missing.
- **A restart re-derives byte-keyed material only.** A set keyed on a decision taken outside the bytes is **retained across the restart**, never re-derived. The one such set in this contract is the reader's close set, which is keyed on the user's routing of the blocks the reader surfaced (§2.6 **close**, §2.7): a block that appeared in the file after that routing decision is **not** in it, however the byte predicates score it on the fresh read. This is the reader-side counterpart of what §2.1 already fixes on the writer side for `N`.
- **`$SNAP` is the operand of every read-path predicate.** The provenance guard (§1.2), the version token (§1.5), and every payload-schema derivation over the file's own bytes (§2.1's `max(회차)`, §2.5's tokenizer, §2.5's terminator check) are evaluated against `$SNAP`, **not** against `$TARGET`. A verdict reached on `$TARGET` in a separate read at a separate moment is not carried by the swap — the swap can only preserve a verdict reached on the bytes it compares. Where a tool-call boundary separates the read that fed a guard from the `cp` that takes the snapshot, the guard is re-evaluated against `$SNAP` before the content is built.
...
| `트리 위생` | the five forms of the table below — two of them parameterised |
...
- **File terminator.** The last line of the file is the fixed sentinel `<!-- cc-design-reconverge: end -->`, emitted by every write form, the creation write included. A file whose last line is not that sentinel **outside an open fence** is truncated: fail closed, surface it, and do not write to it. The sentinel is a constant, so **on every write against an existing file** it is byte-identical before and after and therefore never appears in the diff gates of §2.6; on the creation write it is an added line, and §2.6 exempts creation from the table constraint, so no diff gate evaluates that write at all. What is scoped here is the sentinel's *invisibility to the diff gates*, never its emission — every write form emits it, so a writer may always assert it on the bytes it is about to commit. It replaces a fence-parity check, which the variable-length rule above refutes: a payload holding a fenced script — an authorised recipe shape — legitimately contributes fence-looking lines and flips parity on an intact file, and a fail-closed parity check would then reject an intact sidecar wholesale. Parity is also blind to a truncation landing between blocks. The tokenizer's own open-fence-at-EOF signal is kept as a cheap second check, never as the primary one. The truncation paths this defends against are the hand-editing §1.4 invites, a `mv` degraded to copy-then-unlink by a §1.3 violation, and a host-level crash across a rename this contract never `fsync`s (§1.3) — on a filesystem that does not order the data write ahead of the rename, that can leave a zero-length or stale target. Writer *process* death is not one of them, because `rename(2)` is the commit point.
...
| --- | --- | --- |
| **append** — emit one complete new `## 회차 <N>` block: every applicable field, including `상태: 대기` and the settled `사용자 라우팅` value | writer | **0 removed lines**; exactly one new tokenizer-recognised `## 회차` heading, and it is the **last** `## 회차` heading in the written file; every added **non-blank** line lies **at or after** that heading and **before the next tokenizer-recognised `## 회차` heading or the terminator, whichever comes first** |
...
- `surfaceable(block)` — a **further** filter on `사용자 라우팅`, applied only to un-processed blocks. It exists so that a block can be un-processed and still never be shown to the user. **This file fixes the predicate's name and its domain, and does not yet fix its mapping**: which of `사용자 라우팅`'s four values are surfaceable is written down nowhere — not here, and not in any consumer. That omission is **recorded as a debt, not delegated**: until the mapping is fixed, a reader that picks one privately and a writer that assumes another are both conforming, and `미응답` is the value most likely to be read in opposite directions, since it records that no answer was given rather than any routing the user chose. A reader implemented before the mapping is fixed states the mapping it used, so the divergence is visible rather than silent.
...
Group blocks by `대상 항목` **verbatim** — the whole composite anchor, unparsed. Run the empty-range check of §2.8 on a group **before** taking its charter, so a migrated or renumbered item is disposed of without judgment. Then take the charter from the **newest** block in the group, and inject **every** block in the group as evidence strength. ***Newest* is the greatest `회차 <N>` among that group's blocks** — never `관측 일시`, whose value grammar this contract never fixes and which is carried for a human to read rather than to compare. Under §2.6 the appended heading is also the file's last `## 회차` heading, so file order agrees with the numbering; the number is the definition and the agreement is a consequence, not a second rule. Keying on the item alone is what keeps a history like *drift first, refutation later* as one group with one owner — grouping on the item *and* the verdict would split it into two, and the second group would have no knowledge of the first.
