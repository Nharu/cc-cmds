...
- **Witness scratch dir**: before the first spawn, run the protocol's `## Spawn` command — `<plugin root>/orchestrator/cc-team-witness-init.sh <this review's slug>` — and record the **printed path, literally** (not `$WITNESS_DIR`) as each reviewer's `scratchDir`. Out-of-tree under either root, leaving the boundary gate untouched.
