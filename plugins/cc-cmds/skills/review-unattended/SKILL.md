...
4. **Tiered synthesis, per role.** Every role descends the tiers on its own; one role resolving high says nothing about the next.
    ...
    **That seat is no longer minted, and the rule is kept regardless.** The protocol's checkpoint parameter now writes in round 1 and in no later round, so no member spawned under the current contract leaves a checkpoint at round 2 while holding a completed round-1 witness. Two things keep round-first load-bearing anyway: a corpus written before that restriction still presents exactly this shape and this arm still has to read it correctly, and inside round 1 a seat holding a checkpoint and no witness is resolved by the same rule. What the restriction does change is the **count of independent suppressors**. A round-2 checkpoint used to be one of two, the other being the ledger block's column reading higher than where the seat resolved; where that column lags a live resume the checkpoint was the only one standing, and now nothing stands there. The lag itself is not introduced here — it is the conservative under-claim the protocol already documents, and the disclosure below already names the resulting hole. What is new is that the hole is no longer partly covered, and closing it is the flip ordering's business rather than this ladder's.
