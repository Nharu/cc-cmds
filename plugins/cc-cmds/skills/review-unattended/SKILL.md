...
9. **`## 복구 프로버넌스`.** Emit this section in the report — one row per role carrying role / tier / round / file / `seq` / whether `nonce 미검증` applies. The tier cell carries item 4's `조인 미해소` marking wherever that marking applies.
