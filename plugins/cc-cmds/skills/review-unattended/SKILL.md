...
- **Witness scratch dir**: before the first spawn, run the protocol's `## Spawn` three-line fragment in order in one shell, substituting this review's slug for `<slug>` — the fragment assigns `WITNESS_ROOT`, `STAGE_TAG` and `WITNESS_DIR`, and running only part of it silently drops the attempt discriminator. Then stamp the raw stage id into `${WITNESS_DIR}/.attempt`, and record the directory as each reviewer's `scratchDir`. Out-of-tree under either root, leaving the boundary gate untouched.
