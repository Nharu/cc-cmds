...
- **Witness scratch dir**: before the first spawn, `WITNESS_DIR=$(mktemp -d "${CC_PIPELINE_RUN_DIR:-${TMPDIR:-/tmp}}/cc-team-witness-{slug}${STAGE_TAG:+.${STAGE_TAG}}.XXXXXX")` — root and `STAGE_TAG` sanitization per the protocol's Spawn section — stamp the raw stage id into `${WITNESS_DIR}/.attempt`, and record the directory as each reviewer's `scratchDir`. Out-of-tree under either root, leaving the boundary gate untouched.
