...
- **`<run-id>` is re-derived, never passed.** Take the design document's path → document key → `{slug}` (`${CLAUDE_SKILL_DIR}/../_common/sidecar.md` §1.1) → read `<base>/docs/pipeline-grant/{slug}.md` → the `## 인가 <run-id>` block whose `<run-id>` this run owns. `<stage-id>` comes from the driver-exported `CC_PIPELINE_STAGE_ID`; if it is unset, use this skill's own name.
