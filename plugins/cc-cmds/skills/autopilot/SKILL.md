...
- Local execution and collaboration — issues, comments, labels, projects, pull-request conversation, and a merge or push that is not a deploy trigger — proceed on their own. A raw `gh api` call counts only against those endpoints, and only against the target's own `원격 슬러그` repository and host, reading no body file from outside the worktree; a graphql call never counts — its node ids are opaque, so the repository cannot be read from argv and it fails closed. Anything else declared as collaboration parks. **Destructive dev acts, prod writes and deploy triggers park unless a `사전 인가` row names them**, and only that act parks: the run keeps going.
...
- Declare `dev 식별자` and a `dev` claim is checked against argv; a mismatch or nothing to compare against parks. Declare none and the claim is believed and recorded. `domain` is compared on its own; `aws-account` cannot be read from argv, so declare `aws-profile` beside it or every `aws` dev claim on that target parks.
...
| 9 | *(empty slot)* | — |
| 10 | the merge cannot say what it merges, or the dispatch cannot say where it runs | **fix the segment row and call again with the same argv** — do not raise anything |
