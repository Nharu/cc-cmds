...
`<H>` comes from `<run-dir>/gate-digest-router.json`, written by the previous acting call's own `--emit-digest-to`; where that file is absent take it from `snapshot` instead, and where the gate refuses the flag as an unknown argument drop the flag and keep taking it from `snapshot`.
