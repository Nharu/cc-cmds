...
**5f — Cost ceiling, written to the manifest as `비용 천장`.** A figure in USD written as **digits only** — no currency symbol, no unit, no Korean suffix — or leave it undeclared. The spelling is load-bearing rather than cosmetic: the gate coerces this value to a number, so a value it cannot read comes out as zero and the boundary walks away, and the run then holds a ceiling that bounds nothing. Write the number and put the currency in prose if it needs saying. Say what the two thresholds do: at 80% of the declared ceiling the gate opens a boundary approval — which unattended means it waits for the morning rather than stopping — and at 100% it **ends the run**. Say the state this question is changing, too: the gate has always read this field, and until now nothing wrote it, so `gate_b4_cost` returned immediately on every run ever made and the ceiling was a bound in name. Undeclared keeps exactly that behaviour, which is why it stays legal. **Carry this answer into 5g** — these two are the only bounds the gate enforces on a router run, and whether both of them end up undeclared is a question 5g asks once, with both answers in hand.

**5g — Stagnation bound, written to the manifest as `무진전 상한`.** An integer, or `없음`. What it counts is **the router's own acts** — how many of its gate calls graded above `읽기` in a row the progress vector may stand still for before the run ends. Two callers are excluded and each for a different reason: a stage's traffic through the gate is not counted at all, which keeps a healthy stage from spending the bound while it works, and the router's own **reads** are not counted either, because looking around is not what this bound aims at and a night that ended on reconnaissance would have ended for nothing. `없음` is unbounded on this axis and is what every manifest written before this field existed says. Offer a default and make the user move it: the value that makes this bound safe depends on how much the run is expected to do between acts, and only the user knows that.

**If both 5f and 5g are heading for undeclared, stop and say what that means, then take a value for one of them.** Each field's own undeclared answer stays legal — that legality is the whole of what lets a manifest written before these fields existed still be read, and narrowing it would kill runs already in flight. What is not fine is both at once: such a run has no ceiling on either progress axis, and the only bound left is 5e's deadline, which is read by the fixed-graph cycle loop's dispatch and merge gates and never reaches the router's path. Say that, then require a figure or an integer in one of the two. **This requirement lives here and nowhere else** — the driver's manifest check deliberately passes a manifest with both absent, so that a frozen run does not die on its next verb, and the gate only warns once at the first boundary evaluation. This seat is the one place the state can still be avoided rather than merely reported.
...
| `answered_judgments[]` | `id`, `segment`, `answer` — the judgments a person has answered that no stage has consumed yet; the reading rule is below the table |
...
**The re-dispatch prompt must carry BOTH halves, and prose that carries only the first leaves the answer un-spent forever.** The first half is the approval id, so the stage can read the untruncated bytes (`gate.sh answers --manifest <매니페스트> --approval <id>`). The second half is the duty that produces the consumption record: **the stage re-submits the same judgment before it terminates** — same `판단 기준`, same `판단 근거`, so it hashes to the same approval id — and the gate then reads that approval's `승인` state and writes the `자율 승인` row naming `해소 승인=<id>`. Nothing else writes that row. A prompt that says only "read the answer and carry on" produces work and no record, so the same element is still in the array on the next pass, and the same stage is re-dispatched with the same answer for as long as the run lasts. **A cap is not a substitute for the instruction**: the fixed-graph driver bounds its own re-attachments, and what a bound buys is that the loop ends, not that the answer was used.

**`"answer": null` means the sidecar is gone, not that the answer was consumed.** The bytes live under the volatile run directory, so a collected `$RUN_DIR` takes them while the ledger keeps the clipped copy in `답변 문면`. The element stays in the array because it is still unconsumed; what changed is that re-dispatching it cannot hand the stage the full text. Treat it as a candidate whose input has to come from the ledger row instead.

**This mechanism reaches exactly one run.** An approval id is derived by hashing `RUN_ID` among other things, and both the sidecar path and the consumption predicate are keyed on that id — so a new run cannot see, consume, or be blocked by an answer given during an earlier one. An answer that arrives after its run ended is not inherited; the question is asked again by whatever run needs it next.
...
**And this re-read is a real `snapshot` call, not a file read.** `grade` and `plan` perform nothing and reach no acting path, so neither emits a digest however the flag is spelled — which is deliberate rather than an omission: emitting from a verb that answers a question would put a value in the file that no act of this run stands behind. So the emitted file is stale after a dry run in exactly the way it is stale after any other write, and the two paths do not overlap. The dry-run case is one of the places the round trip stays, and the sentence above says what the others are.
