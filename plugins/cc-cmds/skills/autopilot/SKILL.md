...
7. **The deferred variant — freeze here, dispatch later.** A kickoff may end Act 2 **without starting a shift**: freeze the manifest and the authorization record exactly as Step 6 does, append one backlog record naming their paths, and stop. The lane's own `fleet.sh dispatch` job starts the run later. This is the exception written into CFI-3, and it exists because what kept the fleet idle was never the scheduler — it was that there was no way to put work in.
   ...
   **Items 1 through 3 above are NOT performed on this path.** The report stub, the first `snapshot` that creates the run directory, and the liveness watcher all belong to a run, and at freeze time there is no run id and no run directory to hang them on. `fleet.sh dispatch` does those three, in that order, immediately before `run.sh --manifest`. The order is the same one item 2 gives and for the same measured reason.

   **The backlog** is `${XDG_STATE_HOME:-$HOME/.local/state}/cc-cmds/pace/backlog.jsonl`, and **this skill only ever APPENDS to it.** Two writers share the file with disjoint operations: the enqueuer — this path — adds records and never edits one; the dispatcher (`…fleet.dispatch.<lane>`) edits an existing record's state and never adds one. Do not go back and fix up a record you wrote earlier. A record whose premise has moved is **parked by the dispatcher**, which is the only writer entitled to say so.

   The record carries `id`, `enqueued_at`, `authorized_at`, `deadline`, `base_commit`, `doc_sha256`, `manifest_path`, `lane` and `status: pending`; `park_reason` stays empty until a dispatcher parks it. **`deadline` is written, and it is not a priority.** Order is `enqueued_at` FIFO and a near deadline never overtakes — there is no preemption here, so allowing one to overtake starves the head. What a deadline does is park the record once it passes.
