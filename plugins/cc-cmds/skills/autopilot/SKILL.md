...
| `shift` | `n`, `context`, `soft`, `hard`, `over_soft`, `over_hard`, `floor` — the session cap's state. `over_soft` true is the signal to end this shift; `over_hard` true means `context` has reached or passed `hard`, the point at which the shift is already over its ceiling |
