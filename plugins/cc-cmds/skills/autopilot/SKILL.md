...
What the router may NOT do is decide from memory. Its input is the snapshot and nothing else, save the digest value the gate emits from that same state — see Act 2b. The session is compacted repeatedly over a long run, so a router that leaned on conversation history would have its input silently rewritten, and every claim about reproducibility would go with it.
