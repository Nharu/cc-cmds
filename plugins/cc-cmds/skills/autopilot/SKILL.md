...
- `design_required` is true, the tier is `team-2` or `team-4`, and `## 실행 계획` carries no step with `skill` `design` → back to Step 4, insert the step and add its id to every other step's `depends_on`.
