...
- Local execution and collaboration — issues, comments, labels, projects, pull-request conversation, and a merge or push that is not a deploy trigger — proceed on their own. A raw `gh api` call counts only against those endpoints, and a graphql call only when every mutation it names is one of that kind; anything else declared as collaboration parks. **Destructive dev acts, prod writes and deploy triggers park unless a `사전 인가` row names them**, and only that act parks: the run keeps going.
