...
3. **Read `${CLAUDE_SKILL_DIR}/../_common/sidecar.md` `## 1`** and derive, from the **document's own directory** and never the cwd: `CODE_ROOT` (`git rev-parse --show-toplevel`, falling back to the document's directory), the **document key**, and `{slug}`. **Record `CODE_ROOT` and the cwd the boundary observations will be taken in as a pair, in the report.** Step 5's first act compares the two, and that one recorded pair is what discharges the caller-checks-it duty of `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6.0 and lets a morning reader see that a whole-tree fallback fired.
...
1. **Compare the recorded pair, derive the measurement surface, record it — and only then take the closing observation.** First act of this step: read back the `CODE_ROOT` / observation-cwd pair Step 1 recorded and compare them. If the two do not resolve to one path vocabulary, assertion 1 takes the whole-tree fallback of `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6.0, and **that the fallback fired is written into the report**. Then derive assertion 1's measurement surface from the collected witnesses and the report's `## 결정론적 검사` section, and **write the derived surface into the report**. The closing observation is taken after that write and not before. Every witness is sealed at the end of Step 4, so this order is satisfiable; requiring the surface to be *recorded* is what makes the order observable in the artifact instead of merely asserted.
