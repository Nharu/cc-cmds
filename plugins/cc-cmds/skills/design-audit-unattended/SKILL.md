...
**Assertion 1 is scoped to what the readers actually measured.** Its measurement surface is derived from the collected reader witnesses — the repository-relative paths in their anchor comparison tables and their `path:line` citations — plus the paths the deterministic checks read. A `git status --porcelain` change touching none of those is not a mismatch. The reviewed text loses no protection from this, because `FROZEN_SHA256` covers the document's bytes directly. The measured false positive this removes: an unrelated design session, working in the same repository during the window, moved seven files no reader had opened, and the audit ended there with the document hash unchanged.
