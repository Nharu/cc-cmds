...
**The worktree half is the gate's three scoped assertions, not a whole-output equality** — 2a (owned path set), 2b (this audit's own entry), 2c (`cc-design-exp-` count is 0), exactly as `${CLAUDE_SKILL_DIR}/../_common/verification.md` §6 defines them, and identically to the interactive arm's CFI-1. Do not re-derive them here.
...
**Declare `cc-design-exp-` as assertion 2a's ownership prefix before the window opens.** The only worktrees this audit owns are the ones its own verification mechanism creates, and they all carry that prefix; nothing else in the repository is this audit's to account for. That is the whole of the declaration, and it rests on no naming convention holding for worktrees other runs create — which is what the previous form needed and could not get. It assumed every concurrent run's worktree name carried a shared marker, and in measured runs most of them did not, so another run's worktree *creation* halted an audit for something the audit did not do and cannot see. The declaration reaches a *worktree path-set* entry only — 2b and the `cc-design-exp-` zero-count are untouched by it.
...
2. **Re-hash and re-compare both baselines** — the worktree half as the gate's **2a/2b/2c**, with 2a taken over the declared `cc-design-exp-` ownership prefix, and assertion 1 taken within the measurement surface item 1 recorded (the derived set, or the whole tree when item 1's fallback fired). A sibling worktree's `HEAD` move is not a mismatch, and neither is a porcelain change confined to paths no reader cited — unless item 1's fallback fired, in which case the surface is the whole tree and every porcelain change is inside it. **A mismatch is a halt, and the automatic re-audit count is zero.** Write the halt record with `분류: freeze-mismatch`, naming **which** assertion diverged, the baseline value, the observed value, `FROZEN_SHA256`, and — on an assertion 1 mismatch — **the intersecting paths**, so the line itself tells the morning reader what ended the audit; and — in `재호출 명령` — the exact re-invocation command line. Then stop.
