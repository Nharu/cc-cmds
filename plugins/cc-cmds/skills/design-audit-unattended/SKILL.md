...
**Assertion 1 is scoped to the paths the readers cited.** Its measurement surface is derived from the collected reader witnesses — the repository-relative paths in their anchor comparison tables and their `path:line` citations — plus the paths the deterministic checks cited. A `git status --porcelain` change touching none of those is not a mismatch, unless the four checks put the surface at the whole tree. **Expect that fallback rather than the narrowing**: it has fired on every complete run measured so far — 15 of 15 in this repository's audit corpus — so an unattended run should be planned around whole-tree equality, and the false positive described below is removed only on a run where the narrowing actually fires. **Cited, not measured**, and the gap between the two is ordinary reader behaviour rather than an edge: the witness format keys on what a reader quoted, so a reachability check that succeeds against an uncited file produces no row (only negatives become findings), and a tree-wide search for a cited API leaves a symbol in the anchor column — which contributes no path — so every file that search swept sits outside the surface. The claim here is the weaker one the derivation actually yields. The reviewed text loses no protection from the narrowing, because `FROZEN_SHA256` covers the document's bytes directly. The measured false positive this removes: an unrelated design session, working in the same repository during the window, moved seven files no reader had opened, and the audit ended there with the document hash unchanged.
...
1. **Where the two boundary baselines live** — persist them out-of-tree at `${WITNESS_DIR}/baseline.status` and `${WITNESS_DIR}/baseline.worktree` rather than holding them in context across the whole fan-out. An in-tree record changes the very `git status` assertion 1 states, so it has no fixed point; and it is what lets a **later attempt** re-compare after the attempt that opened the window died, which is otherwise the state in which every baseline-requiring assertion is neither pass nor fail.
...
3. **What happens when `CODE_ROOT` is not a repository, or is a bare one** — both git baselines are then undefined. Skip them and **report the skip**; a silently skipped baseline is an unchecked field wearing the shape of a check, and unattended there is nobody who would notice the absence.
...
- **MUST — byte-identical prompts, and one model.** The `READER_COUNT` rendered prompts differ **only** in `{WITNESS_PATH}`, `{WITNESS_NONCE}`, and `{role-slug}` — and the `model` is the same for all of them. Differentiating readers by lens destroys the reinforcement statistic the `미보강 잔여 수` slot reports, and mixing models across them destroys it the same way: the count means something only while the readers are interchangeable.
...
2. **Re-hash and re-compare both baselines** — the worktree half as the gate's **2a/2b/2c**, and assertion 1 taken within the measurement surface item 1 recorded (the derived set, or the whole tree when item 1's fallback fired). Foreign worktree churn is not a mismatch, and neither is a porcelain change confined to paths no reader cited — unless item 1's fallback fired, in which case the surface is the whole tree and every porcelain change is inside it. **A mismatch is a halt, and the automatic re-audit count is zero.** Write the halt record with `분류: freeze-mismatch`, naming **which** assertion diverged, the baseline value, the observed value, `FROZEN_SHA256`, and — on an assertion 1 mismatch — **the intersecting paths**, all of them in `관측 상세`, the field the halt schema carries for exactly these values, so the line itself tells the morning reader what ended the audit; and — in `재호출 명령` — the exact re-invocation command line. Then stop.
    ...
    **On a PASSING run where 2b's `HEAD` moved but its tree did not, write one line into the report** naming both commit shas (12 hex is enough) and stating that the tree was unchanged, so the window held. Without it this narrowing makes that class invisible: there is no halt record on a passing run, and the only symptom the class ever had was the halt this change removes.
