...
- **SYNC** — an inline tool result gated by the `<usage>…duration_ms…</usage>` signature (structural observation: the recording block is unreachable without the return).
- **ASYNC** — the agent-authored, round-N-keyed `## Review Round N` round-summary witness in `review_log.md` — written by the spawned agent, NOT by the main session and NOT a drop-prone notification. The single trustworthy completion primitive in the async case is this **round-N summary line in review_log.md**; the lost TaskGet status-query and the lost handle-binding mis-route defense are both replaced by in-file round-N keying (a `## Review Round M`, M≠N, never satisfies the round-N check).

**fail-closed (verbatim, both branches)** — never counted as observation: (1) a notification arrival (may drop / duplicate / mis-route; it never enters the predicate), (2) the existence of `review_proposals.md` (overwritten at round start — it cannot prove which round authored it), (3) any record written while the round-N witness is absent. On-disk artifacts are the persisted PRODUCT of an observed return, never authored by the main session to manufacture grounding. **Re-spawn is the cheap safe direction** holds in both branches.

**Sole anti-fabrication anchor (ASYNC).** With TaskGet removed there is no cross-check, so the entire async anti-fabrication guarantee rests on one fact — **the `## Review Round N` round-summary header is authored ONLY by the spawned review agent** (it appends the header after completing its review). The main session's own `review_log.md` writes are disposition tags / informational markers and the `## Outer Iteration N` block — it never authors a `## Review Round N` header. If that sole-authorship breaks, the witness becomes forgeable.
...
**Before spawning the review agent, Read `${CLAUDE_SKILL_DIR}/references/06-review-agent-prompt.md`** and substitute `{TEMP_DIR}` (= actual `INNER_TEMP_DIR`), `{USER_NOTE}` (the trailing note when non-empty, else empty), `{BASE_MODE_CONSTRAINT}` (the `--base` block when `BASE_MODE=true`, else empty), and `{CHANGES_MODE_CONSTRAINT}` (the `--changes` block when `CHANGES_MODE=true`, else empty) per the file's substitution contract, then prepend the path context block before calling `Agent()`. **Pass `run_in_background: false` as a request** — a session that honors it reaches the SYNC branch below at zero cost — but do NOT assert that this removes the notification channel; a host that ignores the request lands in the ASYNC branch, which is exactly why Step 12.detect classifies the spawn result instead of assuming synchrony.
...
- **SYNC** ⟺ the tool result carries a `<usage>…duration_ms…</usage>` tail (positive evidence the agent ran inline to completion) AND has no `output_file:` / `Async agent launched successfully.` marker. The inline result IS the observed return — proceed directly to (a)–(j) below with no agentId tracking, no on-disk read, no yield (reconcile 0, wait 0). This is identical to the committed synchronous behavior.
- **ASYNC** ⟺ the tool result contains `output_file:` or `Async agent launched successfully.`. Capture `agentId` and `output_file` from the envelope, then **confirm completion from the on-disk witness — never from the notification** (background task-notifications drop / duplicate / mis-route — confirmed upstream harness bugs; they are demoted to an early-wake hint and never gate):
    - `async_observed_return(round N)` ≡ the agent-authored `## Review Round N` round-summary header exists in `$INNER_TEMP_DIR/review_log.md` (round-N keyed — a `## Review Round M`, M≠N, never satisfies the check, so a mis-routed/duplicate notification is structurally irrelevant; the notification is never consulted).
    ...
    - **witness absent** → **default to waiting (NOT respawn)**: on every re-entry (notification early-wake, an `output_file` mtime/size growth check, or any later turn that returns to this control point) do a single floor read of `review_log.md`. Soft liveness signals (early-wake on arrival; `output_file` existing + growing) are acceleration hints, never a gate. Early respawn wastes a round and races the original agent's `review_proposals.md` overwrite, so bias toward "running". Emit one Korean line to the user on a witness-absent yield ("리뷰 라운드 진행 중 — 완료 신호 대기" 류; informational, NOT a gate). **Only after a bounded number of re-entries with no round-N witness AND a non-growing / absent `output_file`** declare death → `TaskStop(agentId)` (zombie cleanup) → respawn in the **same `inner_round`** (reuse-first; counter NOT incremented). This is an honest degradation vs. a status query (it may respawn a slow agent early or wait longer on a dead one) — it trades bounded extra waiting for the elimination of the stall.
...
- **Neither** (no `<usage>` tail, no async marker) → **malformed / undetermined — NOT consumable**. Write no record; if `agentId` / `output_file` can be recovered take the ASYNC floor path, else surface the error. Consumption requires a positive witness (`<usage>` for SYNC, the round-N header for ASYNC) — "no async marker" alone never authorizes consumption.

**No look-ahead across the round boundary (ASYNC)**: an async spawn returns its envelope immediately, so the structural block SYNC enjoys is gone — enforce the order in prose: confirm round N (witness) → record round N → round N's Edits land on disk → only then `inner_round += 1` and spawn round N+1.
