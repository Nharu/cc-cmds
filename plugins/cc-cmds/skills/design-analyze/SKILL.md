...
The work.json also carries the durable Role↔agentId **`"ledger"`** key (created/updated alongside the work-state writes): an array of behavior-bearing entries `{ agentId, state, round, role, thinReturns, lastReturn, scratchDir }`, where `state ∈ {running, done, aborted}` and `scratchDir` is the analyst's out-of-tree witness directory (recorded at spawn; transient — stripped from every terminal entry on normal completion, retained only on abort). Re-read the ledger from disk before any resume phase (Step 8 follow-up); if the `"ledger"` key is missing or unparseable, **fail closed via `AskUserQuestion`** (never silent-skip). A residual `state: "running"` entry is the leftover-detection signal.
