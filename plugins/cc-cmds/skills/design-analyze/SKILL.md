...
The work.json also carries the durable Role↔agentId **`"ledger"`** key (created/updated alongside the work-state writes): an array of behavior-bearing entries `{ agentId, state, round, role, thinReturns, lastReturn, scratchDir, outputFile, stallMark, epoch, witnessNonce }`, field-isomorphic with the protocol's pipe-row. Entry schema — the column list, the `state` domain, the per-row transient set and its terminal-strip rule (including the `epoch` exemption) — is defined once in `_common/agent-team-protocol.md`'s Role↔agentId ledger v3 and is not restated here. `scratchDir` is the analyst's out-of-tree witness directory, recorded at spawn. Re-read the ledger from disk before any resume phase (Step 8 follow-up); if the `"ledger"` key is missing or unparseable, **fail closed via `AskUserQuestion`** (never silent-skip). A residual `state: "running"` entry is the leftover-detection signal.
