...
This is a **single-pass** analysis (the `review` agent-team engine adapted to prose design docs), NOT an external/internal convergence loop. The source document and its entire source repo are **read-only** — every output is a new file under cwd `docs/analysis/`.
...
There is no shutdown to run — analysts that returned have already self-terminated. Before any Step 8 follow-up and before any team re-composition, the lead MUST (a) apply the **mid-workflow phase-boundary arm** of `${CLAUDE_SKILL_DIR}/../_common/team-cleanup.md` so no ledger `state=running` row survives (`TaskStop` any straggler, then ledger hygiene) — the arm turns on whether anything after this point resumes this team, and a Step 8 follow-up does — and (b) re-read the ledger from disk before dispatching. **The two continuations are not the same dispatch**: a Step 8 follow-up **resumes** the existing rows by `agentId`, while a re-composition **spawns a fresh team** and therefore takes a new `epoch` per the protocol's Spawn section rather than re-using these rows. Calling both "resuming" is what let the re-composition path inherit a resume's assumptions.
...
**Before assigning analysts, Read `${CLAUDE_SKILL_DIR}/../_common/agent-team-protocol.md`** for the completion-signal contract, the Role↔agentId ledger v3 schema, and shared facilitator rules. Embed the **task-assignment header** from that file verbatim at the top of each spawn/resume prompt (do NOT paraphrase — it is the self-contained contract that tells the task to publish its result as its durable witness file before returning, with the return text only an early-wake hint and no completion prefix).
...
- Spawn each analyst as a **nameless background task** with the approved composition: `Agent({ subagent_type: "claude", run_in_background: true, prompt: <self-contained assignment> })`. The members ARE nameless `Agent` background sub-agents — but a one-shot isolated spawn per round is forbidden; the retained-context resume loop (resume by `agentId`, re-injecting load-bearing context each round) is what makes this a team. Record each returned `agentId` in the `"ledger"` key of `docs/analysis/.{doc-slug}.work.json` immediately (see the protocol's Role↔agentId ledger v3). **Stamp `epoch`** (`max(disk epoch, 0)+1`, re-derived from the on-disk `"ledger"` entries — never an in-context counter; this Step-3 analyst team is the first team → `epoch 1`) **and the round-1 `witnessNonce`** on every one of its entries in the same at-spawn recording window as `agentId`/`scratchDir`, per the protocol Spawn section. The `{doc-slug}` still names the work.json. Before the first spawn, also create the witness dir `WITNESS_DIR=$(mktemp -d "${TMPDIR:-/tmp}/cc-team-witness-{doc-slug}.XXXXXX")` (out-of-tree, under `${TMPDIR:-/tmp}`) and record it as each analyst's `scratchDir` in the ledger rows; every Step-4 analysis round is witnessed (`{role-slug}.round-N.md`, sentinel/nonce per the protocol). The witness dir is **orthogonal** to the in-tree `.{doc-slug}.work.json` — witness content lives only in `/tmp` and is **never** written into work.json (work.json carries ledger STATE including `scratchDir`, not witness content).
...
- **The lead acts as facilitator**, actively driving the multi-round analysis. Completion is by **witness collection** (each analyst's result is its durable witness; the return text is only an early-wake hint): after cross-review, resume each analyst once with a convergence prompt (re-injecting current consensus + open conflicts); an analyst's round is converged-and-collected only when its round witness is `witness_present` and the witness body says "no further input". Do **not** inline a completion lifecycle here — escalation (thin/empty witness, never-returns death predicate → `TaskStop` + same-round respawn, non-conforming witness) follows **the protocol's reconcile ladder + failure phenotypes** verbatim. This is a single pass — no external/internal convergence loop.
...
The work.json also carries the durable Role↔agentId **`"ledger"`** key (created/updated alongside the work-state writes): an array of behavior-bearing entries `{ agentId, state, round, role, thinReturns, lastReturn, scratchDir, outputFile, stallMark, epoch, witnessNonce }` (field-isomorphic with the protocol's pipe-row), where `state ∈ {running, done, aborted}`, `scratchDir` is the analyst's out-of-tree witness directory, `outputFile` is the analyst's harness `output_file` path (the liveness-signal source), `stallMark = {reentryCount, lastBytes, lastBytesPathTag, unavailStreak, emptyStreak, growthStreak}` is the durable death-predicate state, `epoch` is the monotone team-generation index (the current team is `max(epoch)` over non-aborted entries — a Step-8 deep re-analysis team increments it), and `witnessNonce` is the per-(analyst, round) `{WITNESS_NONCE}` recovered on re-entry so the self-heal-consume branch stays evaluable across a compaction — all five recorded at spawn, of which four (`scratchDir`, `outputFile`, `stallMark`, `witnessNonce`) are transient and stripped from every terminal entry on normal completion, retained only on abort. **In this skill that first clause is currently unsatisfied, and the gap is disclosed here rather than left to a reader's inference**: both of this skill's cleanup call sites take the mid-workflow phase-boundary arm, which strips nothing, and no call site takes the completion arm — so on the normal path the four transient fields are never stripped. Whether the fix is a completion-arm call site or a re-keying of the arm depends on a decision this skill has not made, namely whether the Step-8 follow-up resumes these analysts or re-composes a fresh team; until that is settled the sentence above states the contract's rule, not this skill's behaviour. **`epoch` is never terminal-stripped on any path** — it is the only durable source the next `max(epoch)` re-derivation can read, so stripping it would leave a completed ledger in partial-`epoch` presence and fail-close the roster on re-entry. Re-read the ledger from disk before any resume phase (Step 8 follow-up); if the `"ledger"` key is missing or unparseable, **fail closed via `AskUserQuestion`** (never silent-skip). A residual `state: "running"` entry is the leftover-detection signal.
...
- **Deep re-analysis** (team re-composition): when the user needs perspectives not covered, propose a fresh analyst team (Step 3 format + re-creation reason + previous coverage + additional scope), require approval, include prior findings in the new package. On approval, spawn fresh nameless background tasks (`Agent`, `subagent_type: "claude"`, `run_in_background: true`) and record their new `agentId`s in the work.json ledger (**stamp `epoch` = `max(disk epoch, 0)+1`, re-derived from the on-disk `"ledger"` entries — never an in-context counter; this fresh re-composition team → `epoch 2` in the normal sequence — and the round-1 `witnessNonce` on every entry in the at-spawn recording window, per the protocol Spawn section**), with a fresh `cc-team-witness-<doc-slug>` nested `mktemp -d` as their `scratchDir` (witnessed per `_common/agent-team-protocol.md`).
...
**Before Step 8 follow-up handling AND before any team re-composition, Read `${CLAUDE_SKILL_DIR}/../_common/team-cleanup.md`** (CFI-5) and apply its **mid-workflow phase-boundary arm**. There is no shutdown to run — returned analysts already self-terminated; cleanup is no-op (normal) / `TaskStop` on any `state=running` straggler (abort) / ledger hygiene. **No strip and no witness-directory removal at this call site**: a Step 8 follow-up is a resume, and a re-composition reads the previous round's ledger, so both need the transient fields the completion arm would take away.

    **That rationale is refuted by this skill's own Step 7 on the normal path, and the gap is not closed here.** Step 7 deletes the work file after a successful render, and that file is the ledger's carrier. So on every normal run the Step-8 re-analysis reaches its fail-closed branch instead of resuming, and `scratchDir` is destroyed before any cleanup arm can read it — the transient fields this call site preserves do not survive to the step that was supposed to need them. Two consequences, and neither is fixed by choosing a different arm here: **(i)** whoever gives this skill a real workflow-completion call site must capture `WITNESS_DIR` from the ledger **before** the work file is deleted, because afterwards there is nothing to read it from; **(ii)** until then the four transient fields are never stripped on any path, so they persist in the rendered artifact. Recorded rather than repaired: adding the completion site is a new behaviour, and the point of writing it down is that the next reader does not have to re-derive the refutation. Then re-read the ledger from disk before resuming. When follow-up triggers team re-composition, ensure no `state=running` row survives the previous round before spawning the next.
...
- **Nameless background-task team**: Step 4 multi-perspective analysis members ARE nameless `Agent` background sub-agents (`subagent_type: "claude"`, no `name`, `run_in_background: true`). Do NOT collapse this into one-shot isolated `Agent()` calls per round — the retained-context resume loop (`SendMessage` to each `agentId`, re-injecting load-bearing context) drives the cross-validation/debate. A task self-terminates when it returns; its result is delivered by its durable witness file (the return text is only an early-wake hint — see `_common/agent-team-protocol.md`).
...
- **Single pass**: no external/internal convergence loop; no termination counter math.
