...
- **Visual-fidelity completion gate (conditional)**: when the design document designates a visual artifact — an HTML/Figma prototype, mockup, or reference screenshot — as the fidelity source-of-truth (a re-skin, pixel-match, or "match the prototype" task), `dart analyze`/lint + structural widget tests + a design-token gate do NOT constitute "done". Those check structure and tokens, never rendered pixels — visual drift (header alignment, field geometry, placeholder/icon styling, spacing rhythm) passes them silently and surfaces only when a human eyeballs the running build, one screen at a time. Before handoff you MUST, for each in-scope screen: (1) render the implementation to an image at the artifact's viewport and render/screenshot the artifact at the same size, place them adjacent, and review the side-by-side yourself — do not delegate the visual comparison to the user; (2) extract the artifact's load-bearing dimensions (heights, colours, type sizes) by measurement and lock them as test assertions so re-drift becomes a test failure, not a later human catch; (3) fix drift at its systemic source — a shared theme/default or a shared component — not per-screen, or the same class recurs on the next screen. The gate does not apply to non-visual work (backend, logic, data); its trigger is strictly a visual artifact named as the fidelity SOT.
