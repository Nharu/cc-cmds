...
- **1.5a — Discovery & classification (read-only)**: search for the `## 구현 시 검증 항목` heading. Absent → no gate; skip to Step 1.6. Enumerate `### R<n>` items; **skip any item already carrying a terminal token** (`검증됨(통과)`/`반증됨(실패)`/`검증불가(드리프트)`), detected with the tolerant `검증 등급` lookup `^(- )?(\*\*검증 등급\*\*|검증 등급): (검증됨\(통과\)|반증됨\(실패\)|검증불가\(드리프트\))$` (`grep -E`, never perl — the balanced-bold idiom of `_common/verification.md` §3.4 with the value arm pinned to the terminal-token set, so a legacy bullet/no-bold flipped item is still recognized as terminal) — idempotency on re-invocation. What the skip rule leaves is then decided by a **positive selector**, so that no `검증 등급` state reaches the gate by default: an item is a gate item **only** when its `검증 등급` is the save-time residual token `구현 시 검증`, matched with the same tolerant idiom pinned to that one value — `^(- )?(\*\*검증 등급\*\*|검증 등급): 구현 시 검증$` (`grep -E`, never perl; byte-identical to Step 3's W1 lookup, which is what makes an item selected here flippable there). Every other remaining state is a **document defect**, not a residual: `미검증` (the frozen vocabulary makes `구현 시 검증` the only save-time residual token and bars `미검증` from a saved document entirely, so W1 would have nothing on it to rewrite), any `검증 등급` value outside the frozen five-token vocabulary, and an R-item carrying no `검증 등급` line at all (the field is required of every verifiable claim). Take one disposition for all three — surface each on its own line in the Step 2 plan as a document defect, run no recipe, create no task, flip no token. Partition by `검증 시점` into **three** buckets — `구현 전` | `구현 중(<phase>)` | `구현 후` — with a **two-step** lookup, because the bucket decision and the malformedness decision need different questions answered. **Step 1 (presence)**: `^(- )?(\*\*검증 시점\*\*|검증 시점): (.+)$` (`grep -E`, never perl) decides only whether the item carries the field at all — `검증 시점` is an **optional** field, so no match means the item is `구현 전`, the contract's declared default, not a fourth bucket. **Step 2 (value)**: on a Step-1 match, the same tolerant key-anchored idiom with the value arm pinned per bucket selects the bucket — `^(- )?(\*\*검증 시점\*\*|검증 시점): 구현 전$` | `^(- )?(\*\*검증 시점\*\*|검증 시점): 구현 중\(.+\)$` | `^(- )?(\*\*검증 시점\*\*|검증 시점): 구현 후$`. A field that Step 1 found but no Step-2 arm matches is an out-of-vocabulary value — reachable **only** because Step 1 separated "no line" from "unrecognized value"; collapsing the two steps would send it to the default bucket instead. A value outside the three-token set is **never silently bucketed**: it is a malformedness axis of `_common/verification.md` §5.2, which names an out-of-vocabulary enum value as one. Take §5.2's disposition — surface it, consume nothing: run no recipe, create no task, flip no token, and route it to neither the gated nor the disclosed-benign path. Surface it on its own line in the Step 2 plan rather than inside the non-executing disclosure list, because the other entries there are normal outcomes and this one is a defect in the design document. Collect each item's `분류` and `실행 주의` flags. **Scope-directive interaction**: `구현 전` items are gated unconditionally regardless of scope (design validity is global — even a partial implementation rests on a refuted design); a `구현 중` item whose phase is outside the scope is disclosed in the consent batch as "이번 범위 밖 — 미실행" with no `TaskCreate`, leaving its `구현 시 검증` token in place for a later invocation's idempotent 1.5a re-discovery. **`구현 후` is a disclosure-only bucket, never a gate.** Its recipe's inputs do not exist until the design has landed and been used, so scope is irrelevant to it — no phase ever contains it. 1.5a marks it for the 1.5b consent batch, where it is disclosed as "관측 창 미개시 — 미실행", issues no `TaskCreate`, does not run its recipe in 1.5c, does not enter it in the Step 2 verdict table as a flip target, and leaves its `구현 시 검증` token in place for the same idempotent re-discovery. Because no recipe runs, **no verdict exists** — the absence of a verdict on a `구현 후` item is not a drift-ladder input (the ladder is entered by running a recipe; Rung 3 requires an attempted execution and a cause) and never reaches the 1.5d failure branch. The disclosure is a plain line and **never triggers an `AskUserQuestion` of its own** — a `구현 후` item is re-disclosed on every invocation by design, so a prompt would re-ask forever. Its unconditional disclosure surface is the Step 2 plan; the 1.5b consent batch carries it only as a disclosure line, and only when that batch is issued for some other item. An invocation none of whose residual items execute — all `구현 후`, or any mix of non-executing outcomes (an out-of-scope `구현 중` item, an item surfaced above as a document defect) — has an empty execution set and is a normal pass, not an ungated design.
- **1.5b — Consent gate (layered; plan-mode semantics "research yes, side effects no" — the design session's consent does NOT carry into implement's pre-approval phase)**: (a)/(b)/(d) read-only-local recipes run without consent. **If any (c) external probe, (e) worktree, or `실행 주의`-flagged item is present *and will execute in this invocation*, issue ONE batched `AskUserQuestion` before executing** — body *"이번 호출에서 실행할 검증 N건 — 외부 probe X건, 워크트리 재현 Y건[, 예외 클래스: …], 예상 소요 …"*, options `실행 / 해당 항목 건너뛰고 위험 수용 / 중단`. An exception-class recipe (flagged, or recognized during read) is **never auto-run**. The consent batch covers **all residual items** regardless of `검증 시점` (a `구현 중` item is disclosed as "phase <p>에서 실행 예정" — closing the gap where a mid-implementation (c)/(e)/execution-caution recipe would otherwise run un-consented; a `구현 후` item is disclosed as "관측 창 미개시 — 미실행", and an item 1.5a surfaced as malformed is disclosed on its own line as a document defect). **Consent is for execution: an item that will not execute in this invocation — a `구현 후` item, an out-of-scope `구현 중` item, or a malformed item — contributes to the disclosure list only, never to the N / X / Y counts of the body above, and neither its category ((c) external probe / (e) worktree) nor its `실행 주의` flag makes it a trigger of the batched AUQ**; a recipe implement belatedly recognizes as exception-class mid-run gets its own AUQ **immediately before that recipe runs** (for a `구현 중` item, phase arrival is that moment — the recognition fallback). An unflagged recipe is killed at 10 min (or 3× the declared `예상 소요`, whichever is larger; a declared >10 min mandates the execution-caution flag → asked up front).
- **1.5c — Execute, zero document writes**: run per the drift ladder; hold verdicts in memory; write nothing to the document. Capture **both baselines** (the **capture format** of `_common/verification.md` §6 + `git worktree list --porcelain`) on entering 1.5c, and gate after each worktree recipe + on exiting 1.5c (the implement-time tree may legitimately be dirty — the gate is "no new change vs. entry", not "clean").
...
- **Binding-tier coverage is total.** Every requirement, decision, and file change in the design document's **binding tier** (see Constraints → *Binding tiers*) must be covered in the plan. Do NOT omit or skip any binding-tier item.
- **Reference-tier material informs the plan; it does not bind it.** Read it, use it, and say so where it changed a choice — but a reference-tier sentence that the plan does not implement is not an omission, and it is not a reason to widen scope. Where the reference tier and the binding tier disagree, the binding tier wins and the divergence is worth one line in the plan.
- Cross-check the plan against the design document's binding tier to ensure nothing is missing before presenting.
...
- **Verdict table in the plan** (the checkpoint of the in-memory verdict window from Step 1.5): include the verbatim verdict table — R-id / current token / verdict-to-record / 1-line observation / waiver marker. Non-executing disclosure items (`구현 후`, an out-of-scope `구현 중`) belong in the plan's disclosure prose, **not** as verdict-table rows — a verdict-table row is by definition a flip target. The malformed-item line that 1.5a surfaces separately stays separate here too, and an **in-scope** `구현 중` item is likewise not a row of this table: its recipe runs and its token flips at phase arrival (Step 3), not in the batch this table drives. Because plan mode blocks Edits, a pre-approval flip is mechanically impossible — this sequencing (execute in 1.5 → table in the plan → batched flip after approval, in Step 3) is the only working form.
...
- **Step 3's first document-write action (after the reverse-scan guard passes, before any other edit): the batched flip write + diff gate.** (The reverse-scan guard retains its literal-first-action status; the flip is the first *edit* after the guard passes.) The write surface is **exactly 2 byte-enumerated forms**, both inside a `### R<n>` of `## 구현 시 검증 항목`:
    ...
    - **diff gate** (snapshot-based, once after the batch): copy the pre-flip document to a temp snapshot (`SNAP=$(mktemp)` + `cp <doc> "$SNAP"`) → apply the batched W1/W2 → every content line (`±`; excluding the file headers `---`/`+++`) of `diff -U0 "$SNAP" <doc>` must match one of (`grep -E`, never perl): `^-(- )?(\*\*검증 등급\*\*|검증 등급): 구현 시 검증$` (removed side **tolerant** — diff dash-doubling `(- )?` plus all four legacy renderings, so flipping a legacy bullet/no-bold line is not a false-fail) / `^\+\*\*검증 등급\*\*: (검증됨\(통과\)|반증됨\(실패\)|검증불가\(드리프트\))$` (added grade, **strict-canonical** — bold·non-bullet enforced; the 3 legacy renderings, `미검증`, and the un-flipped value are all rejected) / `^\+\*\*구현 시 검증 기록\*\*: .+$` (added note, strict-canonical — `.+`, never `.*`: the general value arm of `_common/verification.md` §3.4 does not match an empty value, and an empty-valued note line would satisfy the §3.3 adjacency check while carrying nothing). Any other changed line → revert **only this batch's non-matching changes** against the snapshot + fail-loud. **Per-flagged-item non-empty-diff assertion**: for every R-item the verdict table marked as a flip target, that item's `검증 등급` line MUST appear as a changed line in the diff — a silent no-op flip (empty diff) on a flagged item is a fail-loud, structurally blocking the vacuous pass (a 1.5a-skipped already-terminal item, a non-executing disclosure item — a `구현 후` item or an out-of-scope `구현 중` item — the separately-surfaced malformed item, and an **in-scope** `구현 중` item whose token flips at phase arrival rather than in this batch are none of them flip targets of this batch, so an invocation with an empty target set passes with an empty diff). A `git diff` against HEAD is FORBIDDEN — it vacuous-passes on an untracked document and risks false-failing / destroying the user's uncommitted edits on a dirty document (the implement-time tree may legitimately be dirty — same premise as 1.5c).
...
- **Binding tiers (section identity, not content inspection).** A design document is the faithful record of a multi-perspective discussion *and* the instruction set an implementation consumes, and those two jobs do not agree byte for byte: a complete record includes reasoning that was considered and superseded, alternatives that lost, and open questions. Treating every byte as an instruction is what turns a record into a contradiction. So the contract is keyed to **which section a sentence sits in** — no predicate inspects the sentence itself, the published document format is unchanged, and there is no migration.
    - **Binding** — `## 합의된 아키텍처`; the *decision* sentences of `## 주요 결정사항과 근거`; entries in `## 미해결 이슈 / 트레이드오프` whose `상태` is `해결`; `## 구현 시 검증 항목`; and a `## 재현·근본원인` whose `근거 등급` is `확인됨(재현·관측)`.
    - **Reading `상태` — both renderings, and absence is never a default.** The producer authorizes two: a **sub-section** rendering (`**상태**: 해결`, matched by the tolerant key-anchored idiom of `_common/verification.md` §3.4) and a **table** rendering (a `상태` column whose value is a cell, which that idiom cannot match because the value is not the rest of the line). **Read both**, and say which one the document used. A consumer that checks only the sub-section form silently classifies every entry of a table-form document as unresolved; a consumer that loosens to a bare `상태.*해결` over-matches prose and rationale text. **An absent or unmatched `상태` is undetermined, not `미해결`** — ask rather than assume. The failure direction here is the lenient one and it is silent: mis-reading `해결` as unresolved demotes a binding-tier item to reference tier, which is exactly the tier that needs no user approval, so the missing approval never surfaces anywhere downstream and the drift sidecar records the divergence as one that legitimately needed none.
    - **Reference** — the rationale prose of `## 주요 결정사항과 근거`; unresolved entries of `## 미해결 이슈 / 트레이드오프`; `## 권장 구현 순서`; examples and illustrations anywhere; and completed `### V<n>` entries of `## 검증 기록`, which are a record of verifications already performed, not work to do.
    - `## 권장 구현 순서` is reference because Step 2 already converts it into a user-approved commitment — the approved plan is what binds, not the recommendation it was derived from.
- **Do NOT deviate from the design document's binding tier.** If a binding-tier item seems wrong or unclear, ask the user instead of making assumptions. Deviating from the **reference** tier needs no approval — it is what the tier is for — but record it: append an entry to the drift sidecar `<base>/docs/design-drift/{slug}.md` (see *Design-drift sidecar* below) so the deviation is visible to a later reviewer instead of living only in this session. A binding-tier deviation additionally requires explicit user approval, and is recorded in the same sidecar.
- **Design-drift sidecar** (`<base>/docs/design-drift/{slug}.md` — the base is part of the path, not an omitted default; `## 1` resolves it from the design document's own location, and a design document outside any repo is the ordinary case rather than an edge one. The token is `<base>`, **not** `<레포 루트>`: `## 1` defines `<base>` over *both* derivation branches, so it is the repo root when one resolves and the design document's own directory when none does. Naming the repo root here reads as more specific and is in fact wrong on the branch this very parenthesis calls ordinary — there is no repo root to name, and a reader following the letter has no path at all). **Read `${CLAUDE_SKILL_DIR}/../_common/sidecar.md` `## 1` + `## 3`** before the first write of an invocation that records a deviation. `## 1` owns the slug derivation, the symmetric `owner-doc=` provenance guard (absence is treated as mismatch, fail-closed both ways), the compare-and-swap atomic write, the never-delete lifetime, and the version token; `## 3` owns the block schema — one `## 이탈 <N>` block per deviation, 8 fields in fixed order, append-only, frozen from the instant it lands. Do not re-author either grammar here.
    - **It is a report, not a self-assessment.** `implement` records *that* it diverged and *why*; whether the divergence was acceptable is judged downstream by `review`, which did not write the code. Nothing in the schema lets the implementer mark its own divergence sound, and the `승인` field transports a **user's** approval rather than issuing one.
    - **Why out of document.** The design document's implement-reserved write surface stays exactly W1/W2 (next bullet). A deviation record is durable state a second writer needs, and taking a new in-document section for it would enlarge an enumerated write surface that exists so a sweep can prove no other byte moved. The sidecar sits beside the document and inherits its retention class, so nothing is lost by keeping it out.
    - **Ephemeral-only is not sufficient.** A deviation reported in conversation and nowhere else dies with the session, and the reviewer who needs it is in a different session by construction.
...
3. `docs/visual-drift/` — **이 레포에서 제거할 대상은 없다**: 대상 레포에 런타임으로 생기는 산출물 디렉토리이며 이 레포 트리에는 존재하지 않는다(대상 레포가 `docs/`를 gitignore하면 커밋되지도 않는다). 이 항목은 부재를 확인만 하고 넘어간다.
...
- **atomic write** (torn-write 안전): 같은 디렉토리 `mktemp` + `mv`로 whole-file atomic write. **대상 디렉토리는 writer가 먼저 생성한다** — `mktemp`는 대상 디렉토리가 이미 존재해야 성립하므로, 사이드카를 쓰기 전에 그 디렉토리를 만든다. 이것이 스킬이 대상 레포 트리 안에 만드는 유일한 디렉토리다(그 경로의 부재한 상위 디렉토리도 함께 생성될 수 있다). **쓰기 가드(읽기 가드와 대칭)**: 대상 사이드카가 이미 존재하면 whole-file 덮어쓰기 전에 그 파일의 `owner-doc=`를 현재 설계 문서의 레포 루트 상대경로와 대조한다 — 일치하면 자신 소유이므로 그대로 갱신하고, 불일치면 slug 충돌(다른 문서 소유), `owner-doc=` 필드 부재면 자신 소유가 확인되지 않으므로 — 둘 다 fail-closed로 **clobber하지 않는다**: 그 사이드카를 보존한 채 이번 잔여는 1차 ephemeral 리포트로만 남기고 사용자에게 fail-loud(=surface, abort 아님)로 알린다(타 문서의 `사용자 수용` 기록 소실 방지). 대상 파일이 없으면 신규 생성이므로 그대로 쓴다.
