...
- **Binding tiers (section identity, not content inspection).** A design document is both a record of a discussion and an instruction set, so the contract is keyed to **which section a sentence sits in** — no predicate inspects the sentence itself.
    ...
    - **Reference** — the rationale prose of `## 주요 결정사항과 근거`; unresolved entries of `## 미해결 이슈 / 트레이드오프`; `## 권장 구현 순서`; examples and illustrations anywhere; `## 팀이 정한 요구 결정`, including its `### 확인된 요구와의 대응` block (a record of the requirement-level choices the design team made, which the binding sections already carry — an entry marked `벗어남` is not an instruction); and completed `### V<n>` entries of `## 검증 기록`, which are a record of verifications already performed, not work to do.
...
- **Design-drift sidecar** (`docs/design-drift/{slug}.md`). **Read `${CLAUDE_SKILL_DIR}/../_common/sidecar.md` `## 1` + `## 3`** before the first write of an invocation that records a deviation. `## 1` owns the slug derivation, the symmetric `owner-doc=` provenance guard (absence is treated as mismatch, fail-closed both ways), the compare-and-swap atomic write, the never-delete lifetime, and the version token; `## 3` owns the block schema — one `## 이탈 <N>` block per deviation, 8 fields in fixed order, append-only, frozen from the instant it lands. Do not re-author either grammar here.
    ...
    - **Why out of document.** The design document's implement-reserved write surface stays exactly W1/W2 (next bullet), so a sweep can still prove no other byte moved.
    - **Ephemeral-only is not sufficient.** The reviewer who needs the record is in a different session.
