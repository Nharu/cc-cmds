...
- **Document-present guard → `document-present`.** If anything exists at the given document path, halt — Step 4's save would overwrite it, and whatever it holds is not this dispatch's: the driver's arm fires only on an absent document, so a file there means a person's document, an earlier dispatch's partial one, or a hand dispatch, and all three are the fault this guard exists for. It is judged by existence, not by content, and not by the freeze line, which a person's document does not carry either.
