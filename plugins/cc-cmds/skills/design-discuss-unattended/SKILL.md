...
It follows the landed `design-audit` / `design-audit-unattended` pair: a separate file so that "this arm has no human-question surface" is a whole-file predicate `scripts/lint-unattended-surfaces.sh` can check. Widening the arm to a second dispatcher keeps that predicate intact — **one file still carries one arm**; what varies is who dispatched it, not how many arms the file holds, and a `design-unattended` sibling was not created for exactly that reason.

## Two dispatchers, one arm

**The discriminator is `CC_PIPELINE_RUN_ID`.** The driver exports it to its stage children and nothing else sets it, so it is the dispatcher itself and not a heuristic: non-empty → **driver dispatch** (stage); unset or empty → **seat dispatch** (leg). Evaluate it once at Step 0 and never re-evaluate. Every existing seat-dispatch sentence in this file stays in force byte for byte; the driver-dispatch rules live in this section, in `## Halt record (driver dispatch)`, in CFI-L4 and in Steps 5U–7U, and nowhere else.
...
**Wind-down waits only for tasks the harness still holds, and a member that has RETURNED is not one of them.** The converse of the paragraph above: when every live task has ended, a turn that ends is the process ending — with the turn's own rc, which is 0 for prose — and nothing about that exit says the round was incomplete. Measured: a member spawned helpers in the background and ended its turn to wait for them; the helpers' notifications came here, the member was never resumed, and this arm kept its row `running` through five wakes, ended its sixth turn with "waiting for the last witness", and exited rc=0 with a title-only stub at the document path. The protocol's **Returned-without-witness** arm is what closes this: a member's completion notification with no witness on disk is resolved — respawned into the same round — **in that same turn**, so that no turn ends with a `running` row the harness no longer holds. Under this dispatcher that rule is not hygiene; it is the only thing standing between a returned member and a silent rc=0.
...
Applies only when `CC_PIPELINE_RUN_ID` is set. **(a) A walkthrough transition is one Edit, and the pre-image comes first.** Before any Step 5U item transition, copy the document to `${CC_PIPELINE_RUN_DIR}/preimage/<slug>.<seq>.md` (atomic form). Then apply the item's terminal transition — `상태`, `자율 처분`, the body reflection and every depth-2 child surfaced while processing it — in **one** Edit. Only then may a halt record be written. A halt between two Edits would leave a half-applied transition that the document and the halt record describe differently, and the driver has no lead memory to reconcile them with. **(b) No team is composed.** `팀-구성` is a permanently forbidden auto-adoption class; the roster for Step 3 is the manifest's `설계 로스터` rows when it carries any, and otherwise `### Default roster (driver dispatch)` frozen in this file — in both cases read, never chosen. Step 5U offers no `팀 토론 진행`; Step 6U spawns nothing. **(c) Nothing is dropped.** A drop is the one disposition whose undo evidence lives outside the document; the substitute is `상태: 보류` + `사유`, which re-surfaces on the attended walkthrough's next pass. **(d) One destination.** The stage ends by emitting the freeze literal, the document path and its whole-file `sha256`, then stops — it names neither the audit nor the kickoff, because the driver's step graph already holds the next stage and a hand-off line here would send a reader to run by hand what the run is about to dispatch.
...
- **Document-present guard → `document-present`.** If a *saved* document exists at the given document path, halt — Step 4's save would overwrite it, and whatever it holds is not this dispatch's: a saved file there means a person's document, an earlier dispatch's completed one, or a hand dispatch, and all three are the fault this guard exists for. **Judge by content on the same axis as the target-document guard below** — does the ledger block parse, and does the body carry at least one `##` section? An early ledger stub is passed; a saved document is parked. It used to be judged by existence, on the premise that the driver's arm fired only on an absent document; that premise no longer holds, because a design stage whose only attempt crashed leaves its own spawn-time stub at the path and the driver now dispatches over it rather than reading the crash's own leftover as a finished design. Existence would make that stub halt every retry, which is the state this guard would be protecting nothing from.
- **Roster check → `roster-missing`.** Read the manifest named by `CC_PIPELINE_MANIFEST`. If its `## 인가` section carries one or more rows of the form
    ...
    instantiate the Step 3 roster from exactly those rows, one member per row, in order. If it carries none, instantiate `### Default roster (driver dispatch)` below, verbatim. Either way nothing is chosen by this stage. **Halt** when rows are present but one cannot be instantiated mechanically — a missing `역할`, `범위` or `모델` field, a model outside the three aliases, or two rows with the same `역할` — because a row that leaves a slot to fill by judgment is a row that has quietly delegated `팀-구성`, a forbidden class, to a stage nobody is awake to overrule. Rows outside `## 인가` are not read. Deviation is not tolerated and recorded; it is a stop.

### Default roster (driver dispatch)

The roster this stage instantiates when the manifest carries no `설계 로스터` row. It is frozen here so that reading it is as mechanical as reading the manifest — the forbidden thing is a stage composing a team by judgment, and a roster written verbatim in this file leaves nothing to judge. It follows the base skill's Step 2 shape: domain seats plus the verification seat, counted like any other row. Changing it is an edit to this file, reviewed like one, never a runtime choice.
...
**Target-document guard.** Resolve `docs/{topic-slug}.md` from `## 대상`. No file at that path is an ordinary first dispatch: proceed. Otherwise read it. This dispatch has saved nothing yet, so a file that already exists is an earlier run's — but *which* earlier run's is the whole question, because one of the reachable states is the one a lawful recovery is meant to land in. **Judge by content, never by existence.** The axis is the seat's own artifact predicate: does the ledger block parse, and does the body carry at least one `##` section? **An early ledger stub is passed; a saved document is parked.**
...
Park with `자리 id: ledger-missing`, `분류: precondition-failed`, and put the path, its byte count, its mtime and **which branch above fired** into `관측 상세` — the byte count was never the discriminator on its own, and naming the branch is what spares the next reader from re-deriving it. The boundary gate is no backstop for the saved-document branch: Step 2 below exempts that exact path, so the overwrite would satisfy every assertion it makes.
...
Any check ambiguous → **fail-closed**: the item is skeleton, and the disposition is the `skeleton` halt with the item, the proposed write and which check was ambiguous in `관측 상세`. A disposition that passes all three is a `설계-쟁점` judgment at `등급 1`, emitted per `### Per-grade disposition`. **Classifying by `Category` (UD/UC/UA/UR) was measured and rejected** — the corpus carries out-of-vocabulary values and bracketed tails on about one entry in fifteen, and a router keyed on them fails silently there; the form test has no such input.
...
- **Pre-image retention.** `preimage/` must survive until the run's **last stage** has terminated — every `**판단 되돌리는 법**` this stage emitted points at it, and a `되돌리는 법` that has been swept is not one. The shared contract's "the run directory is deliberately not durable" is a statement about **process handles** (`*.pid`, `*.pgid`, `*.rc`, `*.start`, transcript pointers) and is read at the path level: those may die with their process, `preimage/` may not, and the two live under one directory without contradiction. Bound: one file per transition, at most **64** per document slug; past 64 the oldest is removed and the removal is stated in the terminal message, so an unbounded run does not turn the retention rule into a justification for unbounded growth.

### Step 6U: Pass-through refinement (driver dispatch)

The base skill's Step 6 is a conversation, and there is none. Step 6U exists because it is a **destination** — Step 5U's `재설계` items and Step 4's two escalation options route here — and it redesigns nothing: it leaves every such item at `상태: 보류` + `사유: 재설계 필요 — 라우터 라우팅`, so the router's redesign rung (`/cc-cmds:design-reconverge`) or the morning's person picks it up. No team is spawned (CFI-L4(b)), nothing is asked, and the step ends in the same turn into Step 7U.

### Step 7U: Coherence, slicing self-check, residual ladder, freeze (driver dispatch)

Three jobs in this order, then the freeze. The ladder sits here and not earlier because the R-items must be final and the ladder writes, so it must precede the freeze; and nothing follows the freeze (CFI-3c of the base skill).

1. **Coherence pass.** Resume this session's Step 3 team by `agentId` (the base skill's permitted resume #3 — this is the one session that can address them), phase token `coherence`, `epoch` re-stamp and `witnessNonce` per the base skill's Step 7 and the protocol, one round, collect by witness under CFI-U0. Apply each finding directly to the document. **A finding that contradicts a converged decision → halt `step7-coherence-conflict`**, naming the finding and the decision — the attended seat raises exactly that to the person. Clear each consumed `[Step 7 정합 점검 대상]` note. Then apply `${CLAUDE_SKILL_DIR}/../_common/team-cleanup.md`.
...
3. **Residual ladder.** For every `### R<n>` still at `**검증 등급**: 구현 시 검증`, descend until one rung settles it:
    ...
    - **Rung 3 — reachability**: **off by default**. Every client that could probe an endpoint (`curl`, `aws`, `psql`, …) is graded `외부상태변경` by argv0, and the `design` settings variant denies `WebFetch`, so this rung costs the very cutpoint it would verify. It runs only when the manifest opens it for that item by a `사전 인가` row naming the item; then the result is evidence only.
    - An item no rung settles → halt `ladder-unsettled` **as a `설계-골격` judgment**: the halt record's `질문 문면` is the item and its blocker, `관측 상세` carries the rung-1 and rung-3 evidence, so the person's morning decision is one judgment rather than an investigation. Halt after all items have been walked, not at the first — the ladder is read-only except for W3, so walking on costs nothing and the record then lists every unsettled item at once.
