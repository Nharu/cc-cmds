...
**`leg.json` updates.** On every ledger flip rewrite `heartbeat_at` to now (atomic form; diagnostic only — no step compares it, the seat's progress reading uses the file's mtime). Write `state: parked` after `park.md` and `state: done` after `presentation.md`. Every other field — `session_id`, `redispatch_count`, `brief_sha256`, `leg_out`, `leg_err`, `prior_session_ids` — is the seat's and is never touched here.
