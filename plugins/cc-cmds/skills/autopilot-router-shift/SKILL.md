...
2. **A Step 1.5d halt record binds to it, and it names the item.** Scan the halt directory by content, not by file name, in one read with no shell glob: `grep -rn -e '^<!-- cc-pipeline-halt v1;' -e '^\*\*스킬\*\*: ' -e '^\*\*스텝\*\*: ' -e '^\*\*분류\*\*: ' -e 'cc-cmds:design-reconverge' -e '^<!-- /cc-pipeline-halt v1 -->' "$CC_PIPELINE_RUN_DIR/halt"`. On a call that carries `게이트 통과`, exit 1 or a missing directory means no record. A record qualifies when its lines read `**스킬**: implement-unattended`, `**분류**: precondition-failed`, a `**스텝**:` value beginning with `Step 1.5d`, and it carries the closing fence. **It binds by its header's `stage=` field**: equal to `<segment>#<실행 버전>` of item 1's row, or to `<segment>`. A header whose `stage=` is the skill's own name binds only when **exactly one** segment satisfies item 1 in this snapshot and no `blocked` row already names that record as `근거`; with zero or several candidates write the `blocked` form with `사유=재수렴 라우팅 — 중단 기록 <path> 를 세그먼트에 묶을 수 없다 (<k>개 후보)` and dispatch nothing. **When the record and the row disagree, the record wins** — the contract's read priority is "a halt record present ⇒ halt" — and the dispatch's `--rationale` says so: `row 종단 부류=<class>; halt record <path> bound by <stage=… | single candidate>`.

    **Then take the `R<n>` the record itself names**, from the `cc-cmds:design-reconverge` lines of that record and that record only. The `재수렴` option of a Step 1.5d halt carries the dispatch line `/cc-cmds:design-reconverge <문서> R<n>`, and writing it is the documented obligation of the skill that writes the record — so the value is there rather than inferred. Collect every `R<번호>` those lines carry. **Exactly one distinct number, or nothing is dispatched**: zero, or two that differ, → `blocked` with `사유=재수렴 라우팅 — 중단 기록 <path> 가 지명하는 R<n> 이 하나가 아니다 (<k>개)`. This value is what item 3 selects with; without it that item picks by file order, which is oldest-first on a carrier that is never pruned.
3. **The carrier's open block for *that* item, and the item is not spent.** The carrier is `<home main worktree>/docs/design-reconverge/<slug>.md`, where `<slug>` is the design document's repo-relative path with `.md` stripped and every `/` replaced by `-` (the manifest's `## 요소` names the document; `_common/sidecar.md` §1.1 is the rule). Read it with `grep -n -e '^## 회차 ' -e '^\*\*상태\*\*: ' -e '^\*\*대상 항목\*\*: ' -e '^\*\*처리 기록\*\*: ' -e '^<!-- cc-design-reconverge: end -->' <carrier>`. Take the first block whose `상태` is `대기` **and whose `대상 항목` names the `R<n>` item 2 took from the halt record**. Any other open block belongs to some other refutation and is not this halt's to route — the carrier outlives the run and its blocks are never deleted, so "the first open block" is structurally the oldest one, and re-converging on its frozen observation edits the binding surface from evidence that may be several runs stale. **That `R<n>` must have no block reading `**상태**: 처리됨`** — one item is re-converged at most once per run, and an item refuted again after its re-convergence goes to a person as `blocked` with `사유=재수렴 라우팅 — R<n> 이 재수렴 뒤 다시 반증됐다`. A carrier whose last marker is not the end marker is truncated: `blocked`, `사유=재수렴 라우팅 — 캐리어 잘림`. No such block at all → `blocked`, `사유=재수렴 라우팅 — 1.5d 중단 기록에 대응하는 대기 블록이 캐리어에 없다`.
4. **Exclusion, in both directions.** Dispatch no re-convergence while another `reconverge` stage is live, and dispatch no `implement` stage while one is — the document lock detects a second writer, it does not keep one out. **And dispatch no re-convergence while any `implement` stage is live** — nothing in `live_stages[]` or `orphan_stages[]` for this run. Item 6's parking test can only see a segment that has ALREADY written its plan, and the `plan_sha256` row is written when that stage EXITS, so an implement running at dispatch time passes the test entirely and then merges a plan composed against the pre-edit document. Those segments are not parked here; the re-convergence is simply not dispatched this shift, and the same judgment stands again once they end.
...
6. **After it ends, the segment goes back to planning — or to a person.** Read the re-convergence's `stage-result` row, then two authored facts: its terminal literal in its own stream (`grep -cF '재수렴을 종료합니다. 판정은 여기까지이며 추가 패스는 없습니다.' "$CC_PIPELINE_RUN_DIR/log/<id>#<실행 버전>.json"`), and item 3's block for that `R<n>` now reading `**상태**: 처리됨` with a `**처리 기록**:` line carrying `판정: 재설계 필요` or `판정: 불필요`.
   ...
   - **`판정: 재설계 필요`** additionally parks the unmerged segments whose plan is older than the edit, and them only: a segment whose `implement` row carries a `plan_sha256` that stands **before** the re-convergence's own `stage-result` row in ledger order. Its plan was derived from the binding surface that just moved, and this loop has no way to make a segment that already holds a plan compose a new one. `blocked` with `사유=재수렴 라우팅 — 구속면 이동(재설계 필요)으로 계획이 낡았다`, one row per segment. **Ledger order is the test, not the mere presence of the row.** A plan written after the edit was composed on the edited document and parking it is pure over-refusal; and the window on the other side — a segment still running when the re-convergence was dispatched, whose plan row lands later — is closed by item 4's exclusion rather than here, because at this point that row is indistinguishable from an honest post-edit plan. The two halves are what make the window closed: item 4 shuts the dispatch-time gap, and this test divides what is left by when it was written.
