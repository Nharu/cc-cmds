...
**Every read in this subsection is a gate call.** You write no files (CFI-S5) and every Bash line you issue goes through the gate, so the ledger rows, the termination-predicate and partial-recovery tests, the `.recover.md` existence check, and the witness enumeration with its stamps are each one `gate.sh exec --manifest <매니페스트> --target <alias> --segment <id> --cutpoint <token> --surface 읽기 --snapshot-digest <H> -- <command>` call — one command per call, with no pipe after it. None of them needs `--reach`.
