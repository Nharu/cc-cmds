...
2. **A stage still running holds everything.** Item 1's row is written only when the stage ends, and the stage puts a file at the document path when it spawns its team, hours before its last edit freezes it — so while it runs, item 1 finds no row and item 3 finds a document. Before item 3, look for the step itself: its id as `세그먼트` in the snapshot's `live_stages[]`, its id in `orphan_stages[]`, or a dispatch record with no row — `ls "$CC_PIPELINE_RUN_DIR/<step id>.attempt"` answering present while item 1 found none. Any of the three → **wait and advance nothing**: dispatch no step that depends on the design, and wait on it with `gate.sh wait --manifest <매니페스트> --segment <step id>` (the step id, never `-`) as *Waiting on a stage* describes; when the wait ends, start again from item 1. You are exactly the seat this guard is for: a shift that took over mid-stage never saw the dispatch, and a shift change during a design stage of several hours is the ordinary case, not the exotic one.
