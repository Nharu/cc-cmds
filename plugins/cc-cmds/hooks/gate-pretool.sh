  ...
  Write|Edit|NotebookEdit|MultiEdit)
    ...
    case "$p" in
      ...
      */CLAUDE.md|CLAUDE.md|*/CLAUDE.local.md|CLAUDE.local.md)
        deny "$(jstr "gate: CLAUDE.md 는 git 이 추적하지 않는 라이브 프리픽스라, Write/Edit 로 고치면 원장에 아무 행도 남지 않습니다. 적용은 게이트를 거쳐야 합니다 — ${GATE} exec --manifest \"\$CC_PIPELINE_MANIFEST\" --target \"\$CC_PIPELINE_TARGET\" --segment \"\$CC_PIPELINE_SEGMENT\" --cutpoint 커밋 --surface 트리밖쓰기 --snapshot-digest <스냅숏 해시> --emit-digest-to ${RUN_DIR}/gate-digest.json --rationale '리뷰 채택본 적용' -- cp <제안본> ${p} — <스냅숏 해시> 는 직전 게이트 호출이 ${RUN_DIR}/gate-digest.json 에 방출한 H 필드이고(Read 도구로 열면 됩니다), 그 파일이 없으면 ${GATE} snapshot --manifest \"\$CC_PIPELINE_MANIFEST\" | jq -r .H 로 받습니다. --emit-digest-to 가 '알 수 없는 인자' 로 거부되면 그 플래그만 빼고 다시 실행하세요")" ;;
    esac
