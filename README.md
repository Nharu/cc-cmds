...
- **반복 알림** — 사용자가 지정한 이벤트(커밋, 테스트 통과 등)가 관측될 때마다 모델이 알림을 발송합니다. hook이 아닌 모델 판단 기반이라, 모델이 어떤 인스턴스를 놓치면 그 알림이 누락될 수 있습니다. **스스로 멈추는지는 그 이벤트를 누가 일으키는가에 달렸습니다** — 모델이 직접 수행한 작업의 계열이면 계열이 끝날 때 스스로 종료하고, 사용자 본인의 커밋이나 CI처럼 모델이 직접 일으키지 않는 이벤트면 스스로 종료하지 않으므로 `"알림 취소"`로 중단해 주세요. 어느 쪽이든 `"알림 취소"`는 언제나 듣습니다.
...
| `"각 커밋마다 알림 줘"` | 반복 — 커밋마다 발송; `"알림 취소"`로 중단 |
| `"각 커밋마다, 각 배포마다 알림 줘"` | 반복 — 두 이벤트를 한 사이클로 묶어 발송; `"알림 취소"`로 중단 |
| `"매 단계마다 알림 줘"` | 반복 — 작업 단위 완료마다 발송; `"알림 취소"`로 중단 |
| `"30분 후에 알림 줘"` | 시계 — 예약 1회 발송 후 자동 삭제; 세션이 끝나면 예약도 사라짐 |
| `"5분마다 알림 줘"` | 시계 — 5분 간격 예약; 첫 알림은 다음 5분 경계부터, 7일 후 만료 |
| `"알림 취소"` | 반복·단발·예약 모두 취소 |

※ **모델 판단으로 발송되는 행(단발·반복)은 어느 것도 자동 종료를 약속하지 않습니다.** 스스로 멈추는지는 위 산문의 분기 규칙 하나로만 정해지며, 표의 예시 발화만으로는 그 이벤트를 누가 일으키는지가 정해지지 않기 때문입니다. 시계 행의 삭제·만료는 모델이 아니라 스케줄러가 처리하므로 이 규칙의 대상이 아닙니다.
...
**3단계 — 최초 macOS 권한 승인** (1단계 완료 후): 첫 알림 시 macOS 권한 다이얼로그가 표시됩니다. 미리 트리거하려면 `"알림 테스트 한 번 해줘"`로 발화하여 테스트 알림을 받고 **허용**을 클릭하세요. (이 발화가 실행하는 호출은 플러그인의 PreToolUse hook이 자동 승인하므로 Claude Code의 Bash 권한 다이얼로그가 뜨지 않습니다 — macOS 알림 권한 다이얼로그만 1회 응답하면 됩니다. 자동 승인은 각각 자기 명령줄에 단독으로 오는 호출에 한하며, 다른 명령과 이어 붙인 형태나 플러그인 경로에 공백이 있는 경우는 다이얼로그를 거칩니다.) 다이얼로그를 놓쳤다면 시스템 설정 → 알림 → terminal-notifier에서 수동 활성화. 권한 거부 후 복구는 셸에서 `terminal-notifier -message 'cc-cmds permission test' -title '[cc-cmds] test' -group cc-cmds-active-notify -execute ':'` 직접 실행으로 재트리거 (스킬 bypass와 동일 형식이라 banner 외관이 일치).
