...
- **반복 알림** — 사용자가 지정한 이벤트(커밋, 테스트 통과 등)가 관측될 때마다 모델이 알림을 발송합니다. hook이 아닌 모델 판단 기반이라, 모델이 어떤 인스턴스를 놓치면 그 알림이 누락될 수 있습니다. 해당 이벤트가 더 나오지 않게 되면 모델이 스스로 종료하며, `"알림 취소"`로 언제든 직접 중단할 수 있습니다.
...
| `"각 커밋마다 알림 줘"` | 반복 — 커밋마다 발송; 커밋이 끝나면 자동 종료 |
