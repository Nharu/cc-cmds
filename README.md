...
| `--dry-run` | off (실제 쓰기 수행) | 처분 판정과 미리보기까지만 수행하고 공개 트래커 쓰기·산출물 저작·원장 기록을 전부 건너뛴다. 게이트 평가는 그대로 하므로 미리보기가 실물과 어긋나지 않는다. |

**Safety** — 처분 판정과 미리보기까지만 수행하고 공개 트래커 쓰기·산출물 저작·원장 기록을 전부 건너뛴다. 게이트 평가는 그대로 하므로 미리보기가 실물과 어긋나지 않는다. (`--dry-run`):
...
- `docs/remediation/{slug}.md` 산출물과 그 안의 `## 판정 원장`을 갱신한다.
- `--dry-run`은 처분 판정과 미리보기까지만 하고 위 넷을 전부 건너뛴다. **게이트 평가는 한다** — 하지 않으면 미리보기가 실물과 달라진다.
