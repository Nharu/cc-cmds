...
| `<task-sentence>` | _(optional)_ | 드라이버 파견에서만 — 매니페스트 `## 의도` 의 첫 줄. 인터뷰 브리프가 없을 때 과제 문면이 되는 한 문장. |

> _Parsing (`<brief-or-doc-path>`): `$ARGUMENTS`의 첫 `.md` 토큰. 어느 파견인지는 이 인자가 아니라 `CC_PIPELINE_RUN_ID` 의 유무로 가른다._

> _Parsing (`<task-sentence>`): 첫 `.md` 토큰 이후의 모든 내용. 좌석 파견에서는 무시한다._
