#!/usr/bin/env bash
# Mutation harness for the subcommand lint. Applies each declared mutation singly and
# requires the observed red set to EQUAL the declared one.
#
# Manifest contract (tests/fixtures/lint-active-notify-subcommands-mutations/<id>/):
...
#
# Usage: test-lint-active-notify-subcommands-mutations.sh [--self-check]
...
manifest_root="$repo_root/tests/fixtures/lint-active-notify-subcommands-mutations"
# The corpus lives in a SIBLING tree, not under the fixture root: this lint's
# driver iterates every directory it finds there with no name filter, so a
# manifest placed inside would be read as a fixture and run as one.
fixtures_root="$repo_root/tests/fixtures/lint-active-notify-subcommands"
lint="$script_dir/lint-active-notify-subcommands.sh"
driver="$script_dir/test-lint-active-notify-subcommands.sh"
...
export CC_CMDS_SUBCMD_LINT_UNDER_TEST="$target"
...
all_fixtures=$(find "$fixtures_root" -mindepth 1 -maxdepth 1 -type d -name 'T-SUBCMD-*' -exec basename {} \; | sort)
...
observe_red() {
  ...
  set -e
  summary=$(grep -E '^test-lint-active-notify-subcommands: [0-9]+ passed, [0-9]+ failed$' "$out" | tail -1)
...
mutation_pre_measurement_check "$manifest_root" "$fixtures_root" "T-SUBCMD-" || exit 2
...
echo "test-lint-active-notify-subcommands-mutations: $passed passed, $failures failed"
