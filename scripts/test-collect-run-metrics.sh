...
# 회차는 게이트가 띄운 분리 자식에서 돌고, 게이트가 수집기 전체에 한 시간 기한을 건다. 이
# 상한과 예산은 그와 별개로 한 회차의 새 수집을 나눈다. 상한이나 예산에 걸린 런은 수집하지
# 않고 미수집으로 세어, 그 회차가 판정 없이 차단되고 미룬 런이 다음 회차에 다시 들게 한다 — 새 수나 어휘를
