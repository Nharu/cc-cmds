...
#   - Model-invocable policy references (github-ops / clickup-ops /
#     fanout-policy / browser-policy / local-html) — these carry no workflow
#     at all. They are procedure text the model opens at a decision point and
#     applies in place, so there is no loop, no counter, and no phase
#     transition for compaction to summarize away. Rule (A) has nothing to protect here, and requiring a
...
EXEMPT_SKILLS=("active-notify" "design-upgrade" "review-upgrade" "implement" "review" "design-lite" "review-lite" "design-system" "design-prompt" "design-ingest" "design-apply" "github-ops" "fanout-policy" "browser-policy" "local-html" "clickup-ops")
