...
# stage was resumed.
sl_sid=$(grep '^- `stage-result` ' "$FX_LEDGER" | { grep -F '세그먼트=SL' || true; } | tail -1 \
...
# --- section: 14k | group: base | covers: snapshot, plan, act | anchors: 런이 종단하면 스테이지 디스패치가 거부된다, 종단 뒤 머지도 거부된다 ---
#
...
grant_field_set() {
  ...
  local key="$1" val="$2" line out="$FX_MANIFEST.gf" inserted=0
  : > "$out"
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      "**$key**: "*) continue ;;
...
unend_run() {
  ...
  rm -f "$RD/done"
  { grep -v '결정=종료' "$FX_LEDGER" || true; } > "$FX_LEDGER.ue"
  mv "$FX_LEDGER.ue" "$FX_LEDGER"
}
...
if grep -q '기준=B5' "$FX_LEDGER"; then
...
while [ "$i" -lt 4 ]; do
  gate exec --manifest "$FX_MANIFEST" --target front --cutpoint 커밋 --surface 읽기 \
       --snapshot-digest "$(HH)" --rationale "B5-stage" -- grep -q . "$FX_MANIFEST"
  i=$((i + 1))
done
unset CC_PIPELINE_STAGE_ID
...
while [ "$i" -lt 4 ]; do
  gate exec --manifest "$FX_MANIFEST" --target front --cutpoint 커밋 --surface 읽기 \
       --snapshot-digest "$(HH)" --rationale "B5-router-read" -- grep -q . "$FX_MANIFEST"
  i=$((i + 1))
done
if [ ! -e "$RD/stagnation-repeat" ] && [ ! -s "$RD/done" ]; then
...
while [ "$i" -lt 3 ]; do
  gate exec --manifest "$FX_MANIFEST" --target front --cutpoint 커밋 --surface 읽기 \
       --snapshot-digest "$(HH)" --rationale "B5-mix-stage" -- grep -q . "$FX_MANIFEST"
  i=$((i + 1))
done
unset CC_PIPELINE_STAGE_ID
...
gate exec --manifest "$FX_MANIFEST" --target front --cutpoint 커밋 --surface 읽기 \
     --snapshot-digest "$(HH)" --rationale "B5-mix-router-read" -- grep -q . "$FX_MANIFEST"
n=$(cat "$RD/stagnation-repeat" 2>/dev/null || printf '(없음)')
...
if grep -q '구속 튜플=B4' "$FX_LEDGER" && [ ! -s "$RD/done" ]; then
...
grep -v '^- `cost`' "$FX_LEDGER" > "$FX_LEDGER.nc" && mv "$FX_LEDGER.nc" "$FX_LEDGER"
...
DL_ORIG=$( { grep -m1 '^\*\*벽시계 마감\*\*: ' "$FX_MANIFEST" || true; } | sed 's/^\*\*벽시계 마감\*\*: //')
...
gate plan --manifest "$FX_MANIFEST" --kind propose-done --target infra --segment SD --cutpoint 머지 -- true
case "$msg" in
  *"런이 이미 종단했습니다"*)
...
LBAK22="$WORK/ledger.22b.bak"; cp "$FX_LEDGER" "$LBAK22"
...
# `$FX_LEDGER`, so the run this section drives is the cone run and nothing else.
# ---------------------------------------------------------------------------
...
if [ -n "$ij" ]; then
  ...
  aj_json=$(cd "$WT" && XDG_STATE_HOME="$STATE_CONE" gate_inproc snapshot --manifest "$NM" 2>/dev/null \
...
H5() {  # H5 [교대 id] — the digest AS THE ACT'S OWN ENVIRONMENT SEES IT.
  ...
  ( cd "$WT" && XDG_STATE_HOME="$STATE_CONE" \
    ...
    CC_PIPELINE_SHIFT_ID="${1:-}" \
    gate_inproc snapshot --manifest "$NM5" 2>/dev/null ) | jq -r .H
}
