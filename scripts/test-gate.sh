...
# less, so a wrong mapping costs time and never coverage.
#
...
#
# `ANC` IS THE ONLY BANNER FIELD ANYTHING READS BACK. `group:` and `covers:` are
# parsed off the id and discarded; `anchors:` is a comma-separated list of
# assertion labels that a cut of this section must still contain, and it is
# checked against the cut before the cut runs. It exists because the id, the
# title and the number of assertions can all survive a section being truncated,
# and the label of the assertion that went missing cannot. Anchors are matched as
# fixed substrings, so they may be a prefix of the label as written; they must
# not contain `,` or `|`.
#
  ...
  awk '
    {
      if ($0 ~ /^# --- preamble-end ---$/)        { pre = NR }
        ...
        nid = nid + 1; idline[nid] = NR; idval[nid] = id; ancval[nid] = anc
      }
    ...
    END {
      ...
      # `bound[i]` is the opening rule line of the box and the title is the line
      # after it, so a marker of that box is at `bound[i] + 2`. Range containment
      # was the earlier rule and it is not the same thing: a marker written one
      # line ABOVE its own rule line lands on the LAST line of the PREVIOUS
      # section, which a range test accepts, and `--sections <id>` then cuts the
      # neighbour, runs it and exits 0 while the named block never runs at all.
      # That is coverage lost under a named id, so it is a hard error rather
      # than a fallback.
      for (i = 1; i <= nb; i++) {
        if (bound[i] <= pre) continue
        for (j = 1; j <= nid; j++) {
          if (idline[j] == bound[i] + 2) { owner[i] = j; claimed[j] = 1 }
        }
      }
      for (j = 1; j <= nid; j++) {
        if (!(j in claimed)) {
      ...
      for (i = 1; i <= nb; i++) {
        ...
        if (e >= epi) e = epi - 1
        id = "-"; il = 0; anc = ""
        if (i in owner) { j = owner[i]; id = idval[j]; il = idline[j]; anc = ancval[j] }
...
SECEOF
    ...
    if [ -n "$sec_anc_miss" ]; then
      ...
      printf 'test-gate: 절 안에 경계 모양(`# ---` 줄 + `# <번호>. ` 줄)이 새로 생겼는지 보세요 — 그 절은 id 로는 여전히 해소되지만 끝 줄이 당겨집니다\n' >&2
      rm -rf "$sec_dir"
      exit 2
    fi
...
SECEOF
    ...
    while read -r sec_a sec_b; do
      [ -n "$sec_a" ] || continue
      printf '%s\n' "$sec_idx" \
        | sed -n "s/^SEC $sec_a $sec_b \([^ ]*\) [0-9]* \(.*\)\$/test-gate:   id=\1 줄 $sec_a-$sec_b 「\2」/p" >&2
    done <<SECEOF
$(printf '%s' "$sec_ranges" | sort -n -u)
SECEOF
...
cp "$MANIFEST" "$WORK/manifest-clean.md"
{
  printf '\n## 룰 설정\n'
  printf '**절단점-준수**: 끔\n**사전-인가-대조**: 끔\n**인가-자기확장-금지**: 끔\n**리뷰-후-머지**: 끔\n'
} >> "$MANIFEST"
...
# ---------------------------------------------------------------------------
# 30. 슬라이스 A 회귀 집합 — 리뷰 정책 축이 실제로 게이트에 도달하는가
#
...
# --- 1. 음성 대조군 — 첫 머지가 아니라 두 번째 머지로 잰다 -------------------
#
...
# --- 2. #569 의 핵심 — 룰이 켜진 채 cycle 행 0 건으로 머지가 통과한다 --------
...
# --- 3. 같은 세그먼트의 두 번째 머지, 의무가 미이행인 채 → 거절 --------------
nb=$(sa_ob_count)
sa_merge S2
...
# --- 4. 의무를 닫은 뒤의 같은 머지 — 미착지 갈래이고, 통과가 재발행을 부른다 --
#
...
# --- 4f. 착지하지 않은 머지의 거짓 의무는 리뷰 없이 닫힌다 --------------------
#
...
# --- 4g. 판정 불가는 통과가 아니고, 문면이 「덮지 않는다」와 다르다 -----------
#
...
# --- 7. 상속된 리뷰 정책이 뒤따르는 세그먼트 행에서도 살아남는다 -------------
#
...
# --- 11. 해소기의 상한 거절이 행 기록 시점의 것과 같은 코드로 실패한다 -------
#
...
# --- 16. 원격을 통해 실제로 착지한 머지는 「착지」로 판정된다 -----------------
#
...
# --- 17. 형제 세그먼트의 열린 의무가 이 세그먼트의 재발행을 억제하지 않는다 ---
#
...
# --- 19. 이행 판정 은 세 값 중 하나가 언제나 실리고, 라우터가 고를 수 없다 ---
#
...
# --- 20. 원격이 없는 앵커 저장소는 로컬 ref 로 판정된다 -----------------------
#
...
# --- 21. fetch 는 성공했는데 베이스의 원격 추적 ref 가 없으면 거절이다 -------
#
...
# --- 22. 로컬에 이미 착지한 머지는 네트워크를 건드리지 않는다 ----------------
#
...
# --- 24. 발행 앵커는 세그먼트 워크트리의 HEAD 이고 메인 워크트리의 것이 아니다 -
#
...
# ---------------------------------------------------------------------------
# 31. 선머지후리뷰 는 리뷰를 없애지 않고 미룬다 — 그 유예가 기록으로 남는다
#
...
# ---------------------------------------------------------------------------
# 32. 리뷰 의무는 `이행` 으로 옮겨질 수 있고, 근거에 대해서만 그렇다
#
...
# ---------------------------------------------------------------------------
# 34. 읽기 크레딧·억제 흔적·철회 (슬라이스 B)
#
# 테스트 설계 신규 1·8 과 철회 회귀가 여기 산다. 셋 다 동사가 없는 술어를 재므로
# 31an2 와 같은 소스 전용 seam 으로 직접 물린다.
#
# 이 절은 픽스처를 자기가 만든다 — 원장·런 디렉터리·매니페스트·전사를 매번 새로
# 뜬다. 앞 절의 `$LEDGER2` 를 물려받으면 `--sections 34` 로 잘라 돌릴 때 그 변수가
...
# ---------------------------------------------------------------------------
B34=$(mktemp -d "$WORK/b34.XXXXXX")

b34_fixture() {
  # b34_fixture <읽기 행 수> [등급 필드] [결과 행 수] — 새 픽스처를 뜨고, 실제 런이
  ...
  B34_DIR=$(mktemp -d "$B34/s.XXXXXX")
  B34_LEDGER="$B34_DIR/led.md"; B34_RD="$B34_DIR/rd"; B34_MAN="$B34_DIR/man.md"
  mkdir -p "$B34_RD"
  printf '## 인가\n- **종료 지점**: 테스트\n- **비용 천장**: 없음\n' > "$B34_MAN"
...
b34_launch_rows() {
  # b34_launch_rows <서수> — 게이트가 띄우는 두 번째 이후 교대의 기동 접두. 앞
...
b34_judgment_rows() {
  # b34_judgment_rows <건수> — 자동 채택 바닥을 통과한 판단 그만큼. 채택 하나는 행
...
b34_boundary_count() {
  # 이 픽스처에서 발행된 경계 승인 수.
  { grep -F '`승인`' "$B34_LEDGER" || true; } | { grep -cF '절단점=경계' || true; }
}
b34_balance() {
  # 마지막 억제 흔적의 크레딧 잔량, 흔적이 없으면 빈 값.
  b34_field "$( { grep -F '`경계 억제`' "$B34_LEDGER" || true; } | tail -1)" '크레딧 잔량'
}
b34_eval4() {
  # 판정 네 번 — 셋이 카운터를 문턱까지 올리고 넷째가 넘는다.
  b34_seam 'gate_b1_stagnation; gate_b1_stagnation; gate_b1_stagnation; gate_b1_stagnation' >/dev/null 2>&1
}

b34_seam() {
  # b34_seam <셸 조각> — 이 픽스처에 대고 seam 안에서 그 조각을 돈다. 경로는
  # 환경변수로 넘긴다: 인용을 세 겹으로 쌓으면 조각 안의 한국어 문면이 깨진다.
  ( cd "$WT" && CC_GATE_SOURCE_ONLY=1 CC_CMDS_AUTOPILOT_NOTIFY=0 \
      B34_LEDGER="$B34_LEDGER" B34_RD="$B34_RD" B34_MAN="$B34_MAN" \
      B34_WT="$WT" B34_CFG="${B34_CFG:-}" B34_BODY="$1" \
      bash -c '
        . "'"$GATE"'"; unset CC_GATE_SOURCE_ONLY CC_ORCH_SOURCE_ONLY
        MANIFEST="$B34_MAN"; LEDGER="$B34_LEDGER"; RUN_DIR="$B34_RD"
        RUN_ID=B34; BASE="$B34_WT"
        if [ -n "$B34_CFG" ]; then CLAUDE_CONFIG_DIR="$B34_CFG"; export CLAUDE_CONFIG_DIR; fi
        set +e
        eval "$B34_BODY"' )
}

b34_rows() { { grep -cF "\`$1\`" "$B34_LEDGER" || true; } ; }
b34_field() {
  # b34_field <행> <키> — 파이프 필드 분해로 그 행의 값 하나. 부분 문자열 grep 은
...
#
# 판정 네 번을 돌린다 — 셋이 카운터를 문턱까지 올리고 넷째가 넘는다.
b34_fixture 4 '축2=읽기' 1
b34_eval4
check "34: 기동 행·결과 행이 낀 읽기 4개(크레딧 미만)로는 경계 승인이 발행되지 않는다" \
  "$(b34_boundary_count)" "0"
check "34: 그 대신 억제 흔적이 남는다" "$(b34_rows '경계 억제')" "1"
check "34: 흔적은 새 계열에 남고 blocked 는 건드리지 않는다" "$(b34_rows 'blocked')" "0"
# 억제는 리셋이 아니다. 카운터를 그대로 두므로 읽기가 끝나고 진짜 정체가 이어지면
# 다음 판정들에서 문턱에 도달한다 — 바로 위 `gate_live_stages` 억제와 같은 모양이다.
check "34: 억제해도 카운터는 리셋되지 않는다" "$(cat "$B34_RD/progress-repeat" 2>/dev/null || printf '?')" "3"
check "34: 흔적 행의 크레딧 잔량은 기동 행과 결과 행을 뺀 값이다 (8-1-4-1)" "$(b34_balance)" "2"
# 결과 행이 정확히 한 단위를 쓴다 — 같은 구간에서 결과 행만 빼면 잔량이 하나 는다.
b34_fixture 4
b34_eval4
check "34: 결과 행이 없으면 잔량이 한 단위 늘어난다 (8-1-4)" "$(b34_balance)" "3"
# 새 계열은 슬라이스 B 에서 생기므로 R3 의 행 길이 측정이 덮지 못한 유일한 종류다.
b34_len=$( { grep -F '`경계 억제`' "$B34_LEDGER" || true; } \
           | LC_ALL=C awk 'BEGIN{m=0}{if(length($0)+1>m)m=length($0)+1}END{printf "%d", m}')
b34_cap=$(sed -n 's/^readonly GATE_ROW_MAX=\([0-9][0-9]*\)$/\1/p' "$GATE")
if [ -n "$b34_cap" ] && [ -n "$b34_len" ] && [ "$b34_len" -lt "$b34_cap" ]; then
  ok "34: 경계 억제 행이 GATE_ROW_MAX 아래다 (${b34_len}B < ${b34_cap})"
else
  bad "34: 경계 억제 행 길이" "행 ${b34_len}B · 상한 '${b34_cap}'"
fi

# 크레딧 값 자체는 게이트 상수에서 읽는다 — 리터럴로 박으면 상수를 바꿔도 테스트가
# 초록인 채 다른 것을 시험한다.
b34_credit=$(sed -n 's/^readonly B1_READ_CREDIT=\([0-9][0-9]*\)$/\1/p' "$GATE")
check "34: 크레딧 상수를 게이트에서 읽는다" "$b34_credit" "8"
# 포화 지점은 구간의 인가 행 수가 크레딧에 닿는 곳이고 기동 행도 그 하나다 — 그래서
# 읽기 일곱에 기동 하나로 여덟이다. 양쪽에서 고정한다: 하나 모자라면 억제, 닿으면 발행.
b34_fixture "$((b34_credit - 2))"
b34_eval4
check "34: 기동 행 + 읽기 6개(합 7, 크레딧 미만)는 억제되고 잔량이 하나 남는다" "$(b34_balance)" "1"
check "34: 그 구간에서는 경계 승인이 발행되지 않는다" "$(b34_boundary_count)" "0"
b34_fixture "$((b34_credit - 1))"
b34_eval4
check "34: 기동 행 + 읽기 7개(합 8, 포화 지점)에서는 경계 승인이 발행된다" "$(b34_boundary_count)" "1"
check "34: 포화 지점에서는 억제 흔적이 남지 않는다" "$(b34_rows '경계 억제')" "0"
...
printf -- '- `자율 승인` | 교대=1 | kind= | 결정=exec | 대상=t | 세그먼트=S1 | 절단점=커밋 | 자격=주변 | 근거=등급 미상\n' \
  >> "$B34_LEDGER"
b34_eval4
check "34: 등급 미상 exec 는 구간을 무효로 만들지 않고 한 단위를 쓴다 (8-1-3-1)" "$(b34_balance)" "3"
check "34: 그 구간에서는 경계 승인이 발행되지 않는다" "$(b34_boundary_count)" "0"
b34_fixture 3 ''
b34_eval4
check "34: 등급 미상 exec 만으로는 읽기 바닥을 넘지 못하므로 발화한다" "$(b34_boundary_count)" "1"
# 빈 구간도 사면되지 않는다 — 판정만 돌리는 라우터가 바로 이 경계가 존재하는 이유다.
# 기동 행 하나만 있는 구간이 그것이다: 소모된 단위는 있으나 읽기는 없다.
b34_fixture 0
b34_eval4
check "34: 읽기가 하나도 없는 빈 구간도 사면되지 않는다" "$(b34_boundary_count)" "1"
...
b34_judgment_rows 2
b34_eval4
check "34: 읽기 1개 + 채택 판단 2건(합 6)은 억제된다" "$(b34_boundary_count)" "0"
check "34: 그 억제의 잔량은 판단 행 넷을 뺀 값이다 (8-1-1-4)" "$(b34_balance)" "2"
b34_fixture 1
b34_judgment_rows 3
b34_eval4
check "34: 읽기 1개 + 채택 판단 3건(합 8)에서는 경계 승인이 발행된다" "$(b34_boundary_count)" "1"
...
b34_eval4
check "34: handoff 와 두 번째 기동 뒤 읽기 3개는 억제된다" "$(b34_boundary_count)" "0"
check "34: 그 잔량은 세 기동 관련 행을 뺀 값이다 (8-3-3)" "$(b34_balance)" "2"
...
b34_fixture 7
b34_before=$(b34_seam 'printf "%s %s %s %s %s\n" "$(gate_boundary_binding B1)" "$(gate_boundary_binding B2)" "$(gate_boundary_binding B3)" "$(gate_boundary_binding B4)" "$(gate_b1_read_run)"' 2>/dev/null)
b34_seam 'i=1; while [ "$i" -le 5 ]; do gate_append "경계 억제" "경계=B1" "사유=읽기 크레딧" "크레딧 잔량=$i" "기록 시각=2026-09-12T00:00:0${i}Z"; i=$((i + 1)); done' >/dev/null 2>&1
check "34: 픽스처에 억제 행이 실제로 쌓였다" "$(b34_rows '경계 억제')" "5"
...
b34_reset_case() {
  ...
  if [ "$want" = "이동" ]; then
    check "34: [$label] 벡터가 움직였으므로 읽기 구간이 리셋된다" "$nread" "0 0"
  elif [ "$nread" = "0 0" ]; then
    bad "34: [$label] 읽기 구간" "벡터가 움직이지 않았는데 구간이 리셋됐다"
  else
    ok "34: [$label] 벡터가 그대로이므로 구간도 그대로다 (n m=$nread)"
  fi
}
...
check "34: 판단 인가는 읽기로 세이지 않고 그 밖의 인가로 소모된다 (2 2)" \
  "$(b34_seam 'gate_b1_read_run' 2>/dev/null | tail -1)" "2 2"
# 축2 가 읽기인 결과 행도 같다 — 결정이 exec 가 아니므로 읽기가 아니다.
b34_reset_case '실패한 읽기의 결과 행'   불변 '' '- `자율 승인` | 교대=1 | kind= | 결정=결과 | 대상=t | 세그먼트=S1 | 절단점=커밋 | 유도 절단점=- | 축2=읽기 | 근거=rc=1'
check "34: 결과 행은 읽기로 세이지 않고 그 밖의 인가로 소모된다 (2 2)" \
  "$(b34_seam 'gate_b1_read_run' 2>/dev/null | tail -1)" "2 2"
...
check "34: (가) 다이얼로그 취소가 관측되면 철회는 거부된다" "$(b34_state)" "대기"
# (나) 중단과 (다) 원인 미관측은 사람이 있었다는 증거가 아니다. 셋을 둘로 접으면
# 「원인 미관측 건에는 적용되지 않는다」는 단서가 사라진다.
b34_withdraw_fixture 중단
b34_seam 'gate_withdraw_boundary_approval "'"$B34_AID"'" "진전 재개"' >/dev/null 2>&1
check "34: (나) 중단은 철회를 막지 않는다" "$(b34_state)" "철회"
check "34: 철회 행이 사유를 싣는다" \
  "$(b34_field "$( { grep -F '상태=철회' "$B34_LEDGER" || true; } | tail -1)" '사유')" "진전 재개"
check "34: 철회 행은 응답 토큰을 싣지 않는다 (대조할 답이 없다)" \
  "$(b34_field "$( { grep -F '상태=철회' "$B34_LEDGER" || true; } | tail -1)" '응답 토큰')" ""
b34_withdraw_fixture 미관측
b34_seam 'gate_withdraw_boundary_approval "'"$B34_AID"'" "진전 재개"' >/dev/null 2>&1
check "34: (다) 원인 미관측도 철회를 막지 않는다" "$(b34_state)" "철회"
...
check "34: 경계 평가는 결속값이 움직인 승인을 철회한다" "$(b34_state)" "철회"
b34_withdraw_fixture 중단
b34_cur=$(b34_seam 'gate_boundary_binding B1' 2>/dev/null | tail -1)
b34_withdraw_fixture 중단 "$b34_cur"
b34_seam 'gate_withdraw_stale_boundaries' >/dev/null 2>&1
check "34: 결속값이 그대로인 승인은 열린 채로 둔다" "$(b34_state)" "대기"
