...
# 훑는 대상은 이 프로세스의 소스가 아니라 원본 파일이다. 창의 끝 표식은 33번 절에
# 있으므로, 이 절을 자른 컷 사본을 훑으면 끝을 찾지 못해 실패한다. 전량 실행에서는
# 둘이 같은 파일이다.
...
# `snapH`, the digest re-read before every act below, is defined in `pre_base`.
...
# drives a real act. `TAB` is defined in `pre_base`.
drain_act() {
