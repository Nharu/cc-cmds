...
# --- 8e. B5 counts the exemption, which is the disposition that writes no row -
#
# Counting only the two new verbs inverts the incentive: today's evasion runs
# entirely on the exemption, so the count stays 0 while the boundary catches only
# the honest mechanism. That form is a fine levied on the honest path.
LEDGER="$WORK/b5.md"; : > "$LEDGER"
RUN_DIR="$WORK/b5-run"; mkdir -p "$RUN_DIR"
n_b5() { { grep -c '승인 id=B5-' "$LEDGER" 2>/dev/null || true; } ; }
o_segment SX park
o_problem SX P0-면제1 읽기
o_problem SX P0-면제2 읽기
o_problem SX P0-면제3 읽기
gate_b5_disposition_volume
check "처분 셋에서는 볼륨 경계가 침묵한다 (임계 미만)" "$(n_b5)" "0"
o_problem SX P0-면제4 읽기
gate_b5_disposition_volume
if [ "$(n_b5)" != "0" ]; then
  ok "면제만으로 임계까지 쌓아도 볼륨 경계가 발화한다 (행 없는 경로가 세어진다)"
else
  bad "볼륨 경계" "면제 넷을 쌓았는데 침묵한다 — 정직한 새 기제에만 걸리는 벌금이 된다"
fi
check "그 넷이 원장에 처분 행을 하나도 남기지 않았다 (행을 세는 형태로는 0 이다)" \
  "$( { grep -c '^- `의무 ' "$LEDGER" 2>/dev/null || true; } )" "0"
case "$( { grep '승인 id=B5-' "$LEDGER" || true; } | tail -1)" in
  *"처분된 의무가 누적 4건"*) ok "그 승인이 누적 처분 수를 질문 문면에 싣는다" ;;
  *) bad "볼륨 경계 문면" "$( { grep '승인 id=B5-' "$LEDGER" || true; } | tail -1)" ;;
esac
