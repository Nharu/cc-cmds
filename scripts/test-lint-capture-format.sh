#!/usr/bin/env bash
# Test scripts/lint-capture-format.sh against
# tests/fixtures/lint-capture-format/.
#
# Each fixture is a SKILLS_ROOT-shaped directory. Convention: the fixture
# directory name encodes the expected exit code.
#   T-CAPFMT-OK-*   → expected exit 0
#   T-CAPFMT-FAIL-* → expected exit 1
#
# Two ways the one-spelling property breaks: a consumer respells the format,
# or a second definition appears. The second is the one a per-line check cannot
# see, because both lines are individually correct.
#
...
fixtures="$repo_root/tests/fixtures/lint-capture-format"
...
for fixture in "$fixtures"/*/; do
  ...
  case "$fixture_name" in
    T-CAPFMT-OK-*)   want=0 ;;
    T-CAPFMT-FAIL-*) want=1 ;;
    *)
      echo "test-lint-capture-format: fixture '$fixture_name' has unrecognized prefix" >&2
      failures=$((failures + 1))
      continue
      ;;
  esac
  ...
  set +e
  output=$(SKILLS_ROOT="$fixture" bash "$script_dir/lint-capture-format.sh" 2>&1)
  ec=$?
  set -e
...
echo "test-lint-capture-format: $passed passed, $failures failed"
